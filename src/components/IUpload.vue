...
const selectFile = async (e) => {
  ...
  for (let file of e) {
    ...
    if (_.find(uploadFiles.value, { fileName: file.name })) {
      ...
      return
    } else if (props.maxFileSize > 0 && file.size >= props.maxFileSize * 1024 * 1024) {
      Message.err(
        props.maxFileSize +
...
const deleteFiles = async () => {
  ...
  await commonDeletFilesApi(data)
}
...
<template>
  <VContainer :class="props.gridOnly ? 'pa-2' : 'pa-0'" style="height: 100%">
    <VRow v-if="!props.gridOnly" no-gutters :style="{ height: `${props.fixHeight ? '38px' : '70px'}` }">
    ...
    <VRow
      ...
      :style="{
        height: props.gridOnly
          ? '100%'
          : props.isVisibled
          ? `calc(100% - 70px - 48px ${props.fixHeight ? '+ 32px' : ''})`
          : 'calc(100% - 70px - 0px)',
      }"
    >
      <VCol>
