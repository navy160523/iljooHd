...
const paramBsnsCd = ref(null) //2025-05-07 박성학 파라메터에 사업부 코드가 있을때 처리.
...
//코드리스트 셋팅
const initCodeList = () => {
   console.log('EmpPopup initCodeList')
  Promise.all([
    getCompanyList(), // 회사구분
    commonSearchApi({
      queryId: 'searchBSNS',
      // 2025.09.12 장석천 인사정보 팝업 재오픈시 작동오류 수정
      param: { CMPNY_DIV: options.values.CMPNY_DIV ? options.values.CMPNY_DIV : userStore.cmpnyDiv, USE_DIV: 'N' },
    }), // 사업부
    commonSearchApi({
      queryId: 'searchDept3',
      ...
      param: {
        // 2025.09.12 장석천 인사정보 팝업 재오픈시 작동오류 수정
        CMPNY_DIV: options.values.CMPNY_DIV ? options.values.CMPNY_DIV : userStore.cmpnyDiv,
        BSNS_CD: paramBsnsCd.value,
        USE_DIV: 'Y',
      },
    }), // 부서
  ]).then((res) => {
    ...
    console.log('EmpPopup codeList.deptCd',codeList.deptCd)
...
const grdMainProps = reactive({
  ...
  field: [
    ...
    { fieldName: 'GRP_ENT_DATE', dataType: 'text', visible: false },
    ...
    // 2025.09.08 과명 추가
    { fieldName: 'GWA_NM', dataType: 'text', visible: false },
    { fieldName: 'OFFI_RES_DATE', dataType: 'text', visible: false },
    { fieldName: 'NATN_CD', dataType: 'text', visible: false },
  ],
  columns: [],
})
...
const openPopup = (popupParam) => {
  // console.log('팝업파람', popupParam)
  if(popupParam.BSNS_CD !== undefined) //2025-05-07 박성학 사업부 코드 파라메터가 있을때.
    paramBsnsCd.value = popupParam.BSNS_CD ? popupParam.BSNS_CD : userStore.bsnsCd
  ...
  const arr = Array.from(options.values.DEPT_NM)
  const result = !arr[0] ? arr.slice(1) : arr
  options.values.DEPT_CD = result.length > 0 ? result.join(',') : '' 
  ...
  // 2025.09.12 장석천 추가
  // 업무상질병 신청에서 인원정보 판업 오픈시 목록 컬럼에 생년월일까지 나와야함
  if (popupParam.MENU_ID = 'HLTJB0010') {
    grdMainProps.field[5].visible = true //생년월일 
    grdMainProps.columns = grdMainProps.field
    grdMain.value.getGridView().setColumns(grdMainProps.columns)
  }
}
    ...
    if (!options.onlyDisabled.BSNS_CD_D) {
      ...
      if (oldValue !== newValue) {
        commonSearchApi({
          ...
          //codeList.deptCd.unshift({ DEPT_NM: "전체", DEPT_CD: "" })
          // options.values.DEPT_CD = ''
          // 2025-06-04 부서선택 활성화 후 팝업실행시 조건값으로 검색이 되나 선택부서가 초기화되서 값이 있으면 
          // 값을 보여주고 없으면 초기화 하도록 수정(마환구)
          options.values.DEPT_CD = options.values.DEPT_CD ? options.values.DEPT_CD : ''
          options.values.DEPT_NM = options.values.DEPT_NM ? options.values.DEPT_NM : ''
...
const beforeSearch = async () => {
  if (!options.ALL_SEARCH) {
    ...
    if(result.length > 10) {
      return Message.err('부서는 최대 10개까지 선택 가능합니다.')
    }
...
const afterSearch = (res) => {
  let data = res.ORESULT_CUR.map(item => {
    if(isEmpty(item.BIRTH_DAY)) {
      item.BIRTH_DAY = item.SANJE_BIRTH_DAY
    }
    return item
  })
          ...
          <v-sheet class="searchArea flex-column">
            ...
            <div class="mt-2 d-flex">              
              ...
              <i-input
                ...
                required
              />
              <span class="mt-3" style="margin-left:400px;font-size :14px; color:#1a40c7"> 
                *부서 최대 10개까지 선택가능
              </span>
            </div>
            <div class="mt-2 d-flex">
              ...
              <i-select
                :label="$t('부서')"
                labelWidth="50px"
                v-model="options.values.DEPT_NM"
