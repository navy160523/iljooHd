...
const openPopup = popupParam =>
{
  ...
  if (popupParam.CLRLINE === 'CLR_15') {
    ...
    popupParams.CLRLINE = ''
    popupParams.CLRLEVELMSG = '부서장(단독 과의 경우 직책과장)'
  }
    ...
    popupParams.CLRLINE = ''
    //popupParams.CLRLEVELMSG = '직책과장'
    popupParams.CLRLEVELMSG = '부서장(단독 과의 경우 직책과장)' //20250626 한민규매니저 요청
    safja.value = true
  }
  else {
  ...
  if(!isEmpty(popupParams.CLRLEVEL)) {
    ...
    if(grdApprovalLineData.length > 0) {
      for(let i=0; i <grdApprovalLineData.length;i++)
      {
        ...
        if(['A526166','A512811'].includes(val.EMP_NO)){
          IsMaster = true
        }

        console.log('val.EMP_NO', val.EMP_NO)
    ...
    console.log('[saveApprovalData] IsMaster : ', IsMaster)
    if(!IsMaster && ClrLevelCnt == 0) {
