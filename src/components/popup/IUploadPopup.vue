...
let searchParams = reactive({
  CMPNY_DIV: props.cmpnyUseYn ? userStore.cmpnyDiv : '',
  FILE_ID: '',
})
  ...
  if (btn.id === 'btnSearch') {
    ...
    new deleteFlowHelper(vm, t)
      ...
      .setAfter(() => {
        ...
        files.value = []
        uploadFiles.value = []
        commonSearchApi({ queryId: 'searchFile', param: searchParams }).then((res) => {
          grdMain.value.getDataProvider().setRows(setSearchData(res.ORESULT_CUR))
          emit('uploaded', {
            fileId: props.guid2 ? props.guid2 : guid.value,
            STATUS: 'delete',
            rowCnt: grdMain.value.getGridView().getJsonRows().length,
          })
          emit('deleted', {
            fileId: props.guid2 ? props.guid2 : guid.value,
            rowCnt: grdMain.value.getGridView().getJsonRows().length,
...
const selectFile = (e) => {
  ...
  for (let file of e) {
    ...
    if (!_.includes(['PDF'], fType) && props.pdfOnly) {
      const msg = props.pdfOnly ? t('pdf 파일만 업로드 가능합니다.') : t('사용할 수 없는 파일입니다.')
      Message.err(msg + '<br>' + file.name)
      clearGrid()
...
const getFileCount = async () => {
  ...
  await commonSearchApi({ queryId: 'searchFile', param: searchParams }).then(res => {
    cnt = res.ORESULT_CUR.length
  })
