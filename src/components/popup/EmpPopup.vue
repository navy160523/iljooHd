  ...
  Promise.all([
    ...
    commonSearchApi({
      ...
      param: {
        ...
        // 2025.09.23 장석천 인사정보 팝업 사업부가 전체일시 부서도 전체부서 가져오도록
        BSNS_CD: options.values.BSNS_CD ? options.values.BSNS_CD : '',
        USE_DIV: 'Y',
      },
    }), // 부서
  ]).then((res) => {
...
const openPopup = (popupParam) => {
  ...
  if(popupParam.BSNS_CD !== undefined) //2025-05-07 박성학 사업부 코드 파라메터가 있을때.
    paramBsnsCd.value = popupParam.BSNS_CD
  ...
  if (['HLTJB0010', 'HLTJB0020', 'HLTJB0030'].includes(popupParam.MENU_ID)) {
    grdMainProps.field[5].visible = true //생년월일 
    // 생년월일이 없을시(퇴직자), 산재데이터에서 생년월일 가져오기
    grdMainProps.field[5].displayCallback = function (grid, index, value) {
      return value ? value : grid.getValue(index.itemIndex, "SANJE_BIRTH_DAY");
    }
    grdMainProps.columns = grdMainProps.field
    grdMain.value.getGridView().setColumns(grdMainProps.columns)
  }
