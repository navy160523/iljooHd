<template>
  <div>
    <div :id="id" />
    <Preview :toggle="previewDialog" :data="previewData" />
  </div>
</template>
...
watch(
  () => props.isReadOnly,
  newValue => {
    newValue == "true" ? ckEditor.enableReadOnlyMode(props.id) : ckEditor.disableReadOnlyMode(props.id)
  },
)
...
onMounted(() => {
  ...
  ClassicEditor.create(document.getElementById(props.id), {
    ...
    .finally(() => {
      ...
      //읽기전용설정
      props.isReadOnly == "true" ? ckEditor.enableReadOnlyMode(props.id) : ckEditor.disableReadOnlyMode(props.id)
      ...
      const getHtmlModeButton = ckEditor.ui.view.element.getElementsByClassName("ck-source-editing-button")[0]

      if(getHtmlModeButton !== undefined){
        getHtmlModeButton.addEventListener("click", () => {
          const htmlEl = ckEditor.ui.view.element.getElementsByClassName("ck-source-editing-area")[0]
