...
const grdProps = reactive({
  ...
  fields: [ 
    ...
    { fieldName: 'RISK_TYPE_NM', dataType: 'text', width: '130', header: { text: t('평가유형') }, button: 'action',
      ...
      editable: false },
    { fieldName: 'WORK_STANDARD_ACT_COUNT',  dataType: 'number',  numberFormat: "#,##0",  width: '70', styleName: 'right-column', header: { text: t('작업행동') }, editable : false },
    { fieldName: 'WORK_STANDARD_RISK_COUNT', dataType: 'number',  numberFormat: "#,##0", width: '70', styleName: 'right-column', header: { text: t('위험요인') }, editable : false },
    { fieldName: 'WORK_STANDARD_RISK_OVER_COUNT', dataType: 'number',  numberFormat: "#,##0", width: '70', styleName: 'right-column', header: { text: t('중요위험') }, editable : false },
    { fieldName: 'WORK_STANDARD_AVG_RISK',   dataType: 'number',  numberFormat: "#,##0", styleName: 'right-column ', header: { text: t('평균\n위험성'), styleName: 'multiline-editor-pre' }, editable : false },
    { fieldName: 'WORK_STANDARD_IMG_COUNT',  dataType: 'number',  numberFormat: "#,##0", styleName: 'right-column', header: { text: t('사진등록') }, editable : false },
    ...
    { fieldName: 'SAGO_SN',                  dataType: 'text', width: '100', header: { text: t('사고번호') }, editable : false },
    { fieldName: 'EDU_FILE_COUNT',           
      dataType: 'text', 
      width: '100', 
    ...
    // 숨기는 필드  
    { fieldName: 'EDU',                      dataType: 'text', width: '100', header: { text: t('교육이수자 확인(명)') }, button : 'action', buttonVisibility : 'always', editable : false, visible : false },  
    ...
    { fieldName: 'RISK_TYPE', dataType: 'text', width: '100', header: { text: t('RISK_TYPE') }, visible : false },
    { fieldName: 'EDU_FILE_ID', dataType: 'text', width: '100', header: { text: t('EDU_FILE_ID') }, visible : false },
  ],
  columns : [],
})
...
const onCellButtonClicked = (grid, index, column) => {
  ...
  else if(column.fieldName == 'EDU_FILE_COUNT')
  {
    openPopup('EDU_FILE_COUNT', focusedRow.EDU_FILE_ID)
  }
...
const openPopup = (gubun, param) => {
  ...
  else if(gubun == 'EDU_FILE_COUNT')
  {
    console.log('fileUploadEduPopup param : ', param)
    fileUploadEduPopup.value.openPopup(param)
  }
  else if(gubun == 'EVAL')
  {
...
const uploadedFile = list => {
  ...
  let focusedRow = grdMain.value.getFocusedRowData()  
  let sEDU_FILE_ID = focusedRow.EDU_FILE_ID

  if(!focusedRow.EDU_FILE_ID){
    sEDU_FILE_ID = list.fileId
  }

  let params = [{
    WORK_STANDARD_ID: focusedRow.WORK_STANDARD_ID,
    EDU_FILE_ID: sEDU_FILE_ID,    
  }]
  commonExecuteApi({ queryId : 'RSKBA0020_SAVE04', list: params }).then( res => {
...
const initCodeList = async () => {
  ...
  await Promise.all([
    ...
    if(window.history.state.data){
      ...
      searchParams.COMPANY_CD = data.COMPANY_CD
      searchParams.BSNS_CD = data.BSNS_CD
      await setDeptList(searchParams.BSNS_CD)
      searchParams.DEPT_CD = data.DEPT_CD_N ? data.DEPT_CD_N : data.DEPT_CD 
...
const afterSearch = res => {
  ...
  filterData = res.ORESULT_CUR.filter(row => row.SAGO_SN != null)

  //console.log('filterData', filterData)
  if(searchParams.SAGO_YN === '1'){
...
const standardsearch = async ()=>{
  grdMain.value.getGridView().filterPanel.clearInput()
    ...
    new queryFlowHelper(vm, t)
      ...
      /*if(searchParams.PROGRESS_STATUS == '3')
        // 작업표준복사 버튼 숨기기
        menuTitle.value.setBtnProperty('btnStandardCopy', 'visible', true)
      else
        menuTitle.value.setBtnProperty('btnStandardCopy', 'visible', false)
      */
...
const standardDelete = async () => {
  ...
  for (let i of checkedRows) {
    ...
    if(data.RISK_TYPE === 'RISK_TYPE-02'){
      cntRISK_TYPE02++
    }else{
      saveParams.push({
  ...
  if (userStore.authGrpCd.includes('GRP00377')){
    ...
    if(cntRISK_TYPE02 > 0){
      return Message.warn(t('평가유형이 수시(안전사고)인 항목은 삭제 불가능합니다.'))
    }
  }
...
const onButtonsClick = async btn => {
  ...
  // 폐지표준복구
    ...
    let dialogResult = await vm.$swal({ 
      title: t('선택한 작업표준을 복구(복사) 하시겠습니까?'), 
      showCancelButton: true,
    })
    ...
    let data = grdMain.value.getDataProvider().getJsonRow(focusedRow[0])

    //console.log('data = ', data)

    let listParam = [{
      WORK_STANDARD_ID: data.WORK_STANDARD_ID,
      WORK_STANDARD_NM: '(복구)' + data.WORK_STANDARD_NM,
    ...
    commonRskApi({ 
      queryId: "RSKBA0010_SAVE_05", //PKG_RSKBA0010.TRANS_MASTER_M
      UP_DEPT_CD: upDeptCd,
      DEPT_CD: deptCode,
      USR_ID: userStore.userId,
      IMG_CHECK:'Y',
  ...
  // 개정(위험성평가)
    ...
    if (checkedRows.length === 0) {
      ...
      return Message.warn(t('최대 10개까지 출력 가능합니다.'));
    }

    checkedRows.forEach((row) => {
      const rowData = grdMain.value.getDataProvider().getJsonRow(row)
  ...
  /*else if (btn.id === 'btnStandardEval') {
    ...
    if(searchParams.PROGRESS_STATUS != '3'){
      return Message.warn(t('진행상태를 작성/평가중으로 선택 후 클릭하시기 바랍니다.'))
    }
...
<template>
  <v-card class="pa-0 fill-height">
    <v-card-title class="pa-3 pb-0">
      ...
      <IMenuTitle
        ...
        :button-list="['btnSearch', 'btnWorkStandardCopy', 'btnStandardCopy', 'btnNew', 'btnRevision', 'btnDelete', 'btnSimplePrint', 'btnDetailPrint', 'btnListPrint']"
      ...
      <div class="d-flex flex-column fill-height">
        <v-sheet class="searchArea flex-column">
          ...
          <div class="d-flex mt-2">
            ...
            />
            <v-checkbox
              v-model="searchParams.SAGO_YN"
              true-value="1"
              false-value="2"
              :label="t('안전사고 Hi-Standard 개정 대상')"
            />             
          </div>
        </v-sheet>
        <v-sheet height="50%">
  ...
  <RSKBA0010popup5 ref="rSKBA0010popup5" @closed="onButtonsClick({id: 'btnSearch'})" />
