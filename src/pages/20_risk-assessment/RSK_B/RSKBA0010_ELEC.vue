...
import { commonSearchApi, commonExecuteApi, commonRskSaveApi, commonExcelWrkDetailApi, commonWordExportApi, getCodeList, getCompanyList } from '@hiway/api/commonApi'
...
import RSKBA0010popup1 from './RSKBA0010popup1_ELEC.vue'
...
import SAFAB0010       from '@/pages/30_safety/SAF_A/SAFAB0010.vue'
...
const dialogSAFAB0010 = ref(false)
...
const initCodeList = () => {
  ...
  Promise.all([
    // getCompanyList(), // 회사구분
    commonSearchApi({ queryId : 'RSKBA0010_ELEC_SEARCH_02', param : { TYPE: 'DETAIL', COMM_CD: 'RISK_TYPE' } }), // 평가유형
  ]).then(res => { 
    //console.log('코드 가져오는데?', res[0].ORESULT_CUR)
    codeList.riskType = res[0].ORESULT_CUR   
    // 일렉일 경우 공정변경, 물질변경 추가함.
    codeList.riskType.push({CODE_ID: 'RISK_TYPE-06', CODE_NM: '수시(공정변경)'})
    codeList.riskType.push({CODE_ID: 'RISK_TYPE-07', CODE_NM: '수시(물질변경)'})
  })
}
...
const selectedEmpData = (val) => {
  ...
  commonSearchApi({ 
      queryId : 'RSKBA0010_ELEC_SEARCH_33', 
...
const onButtonsClick = async btn => {
  ...
  }else if (btn.id === 'btnReqApply') { // 결재
    approvalPopUp('R')
  }else if(btn.id === 'btnPartnerApply') { // 협력사 최종 결재
  ...
  }else if(btn.id === 'btnPartnerReject') { // 협력사 최종 결재
  ...
  else if (btn.id === 'btnStandardCopy') {  // 표준복사
  /*
    if(!isMaster.value) {
      let dialogResult = await vm.$swal({ 
  ...
  */  
  ...
  else if (btn.id === 'btnWordPrint') {  // 워드출력    
...
const openPopup2 = (gbn, id) => {    
  ...
  if (gbn === '위험요인') {
    ...
    // 계략도를 선택해야함.
    if(sItemLevel.value != '1')
    {
      // Message.err(t("파일 첨부할 [작업행동]을 선택해주세요."))
      Message.err(t("파일 첨부할 [계략도]를 선택해주세요."))
    ...
    console.log('WORK_STANDARD_UNIT = ', WORK_STANDARD_UNIT)

    let dd = WORK_STANDARD_UNIT.find(x => x.id == sItem)
...
const afterSearch = (res) => {
  ...
  */   
  ...
  router.push('/20_risk-assessment/RSK_B/RSKBA0020_ELEC')
}
...
const setTrans = res => {
  ...
  return commonRskSaveApi({
    queryId: "RSKBA0010_ELEC_SAVE_02",
...
const validationCheck = () => {
  if (isAbolition.value) {
    ...
    if (WORK_STANDARD_MASTER.RISK_TYPE === '' || WORK_STANDARD_MASTER.RISK_TYPE === null || WORK_STANDARD_MASTER.RISK_TYPE === undefined) {
      Message.warn('평가유형을 선택하세요.')
      return false
    }

    // 일력은 필수입력 해제함. (2024.12.03 by kykim)
        ...
        actList.forEach(act => {
          ...
          if(act.child){
            ...
            riskList.forEach(risk => {
              ...
              if(risk.AFTER_FRE == '' || risk.AFTER_MAT === undefined) {
                after1 = false
              }

              if(risk.RM_NM == '' || risk.RM_NM === undefined) {
                after2 = false
              }
              ...
              if(beforeSum >= 8) {
                ...
                if((risk.AFTER_FRE * risk.AFTER_MAT) == 0)
                  isAfter = false
    ...
    if(!before2){
      Message.warn(t('현재의 위험성 [분류] 미입력 상태입니다.<br/>내용 입력후 저장해주세요.'))
      return false
    }
...
const deleteData = () => {  
  ...
  return commonExecuteApi({ queryId : 'RSKBA0010_ELEC_DELETE_01', list: param })
}
...
const init = gbn => { // gbn - 1 : 신규
  ...
  commonSearchApi({ queryId: "RSKBA0010_ELEC_SEARCH_03", param: { DEPT_CD : deptCode }  }).then( res => {  
     
    WORK_STANDARD_MASTER.WORK_STANDARD_ID = res.OUT_DATA + '-' + '01'            
    WORK_STANDARD_MASTER.WORK_STANDARD_NO = res.OUT_DATA
    WORK_STANDARD_MASTER.WORK_STANDARD_REVISION = '01'
...
const committeeSearch = async () => {
  ...
  await commonSearchApi({ queryId: "RSKBA0010_ELEC_SEARCH_23", param: params  }).then( res => {          
...
const revisionHistory = async () => {
  ...
  await commonSearchApi({ queryId: "RSKBA0010_ELEC_SEARCH_24", param: { WORK_STANDARD_NO : WORK_STANDARD_MASTER.WORK_STANDARD_NO }  }).then( res => {    
...
// 위험요인 분류 조회
const riskCdInit = () => {
  return commonSearchApi({ queryId : 'RSKBA0010_ELEC_SEARCH_02', param : { TYPE : 'RISK_CD' } })
}

// 위험요인 분류 조회
const rmCdInit = () => {
  commonSearchApi({ queryId: "RSKBA0010_ELEC_SEARCH_02", param: { TYPE: 'RM_CD', COMM_CD: 'RM_CD' } }).then( res => {
    if(res.length > 0){
...
const transInit = async id => {  
  ...
  // 언어 목록 조회
  await commonSearchApi({ queryId : 'RSKBA0010_ELEC_SEARCH_32', param: { WORK_STANDARD_ID : id }}).then(res => {
    ...
    res.ORESULT_CUR.forEach(el => {
      ...
      if (el.IN_TRANS === 'Y') {
        cmbTrans.value.push({ TRANS_CD: 'in', TRANS_NM: '인도네시아어' })
      }
...
watch( () => workId.value, (oldValue,newValue) => {
...
const searchPartnerAppr = () => {
  ...
  if(WORK_STANDARD_MASTER.WORK_STANDARD_ID == null)
    return
  ...
  return commonSearchApi({ 
    queryId : 'PKG_CSX02_WORK_GET_APPR_TARGET', param : obj 
  }).then( res => {
...
const search = async id => {
  ...
  if (id !== '' && id !== undefined) {
    ...
    await commonSearchApi({ queryId: "RSKBA0010_ELEC_SEARCH_04", param: { WORK_STANDARD_ID : id, TRANS : WORK_STANDARD_MASTER.TRANS_CD }  }).then( async res => {    
      ...
      if(data != null)
      {
        ...
        // 재해 발생 현황
        await commonSearchApi({ queryId: "RSKBA0010_ELEC_SEARCH_31", param: params_31  }).then( async res2 => {
      ...
      // WORK_STANDARD_MASTER.RISK_NM = isRevision.value ? '' : data.RISK_TYPE_NM    
      ...
      if (data.U_DATE === null && data.C_DATE === null) {
        ...
        if (['CC_AC', 'UC_AC'].includes(data.PROGRESS_STATUS)) {
          ...
          if(workType.value == 'E' && WORK_STANDARD_MASTER.DEPT_CD == deptCode && ['CC_AC', 'UC_AC'].includes(data.PROGRESS_STATUS)) {
...
const searchReportList = async () => { 
  ...
  //임시 하드코딩
  //WORK_STANDARD_MASTER.WORK_STANDARD_ID = 'HHIS-CW-A0000032-031-01'
  await commonSearchApi({ queryId: "RSKBA0010_ELEC_SEARCH_08", param: { WORK_STANDARD_ID: WORK_STANDARD_MASTER.WORK_STANDARD_ID } }).then(async res => {
...
const searchReportTree = () => { 
  ...
  return commonSearchApi({ queryId : 'RSKBA0010_ELEC_SEARCH_09', param : { WORK_STANDARD_ID : WORK_STANDARD_MASTER.WORK_STANDARD_ID } })
...
const meetingSearch = () =>{
  ...
  return commonSearchApi({ queryId : 'RSKBA0010_ELEC_SEARCH_21', param: params })  
}

const isInputState = () => {
  if (['CC_AC', 'UC_AC', 'CC_AD', 'UC_AD', 'CC_AP', 'UC_AP', 'DC_AD', 'DC_AP'].includes(this.WORK_STANDARD_MASTER.PROGRESS_STATUS)) {
    if (this.isAbolition) {
      return 'isInput'
    }
    else {
...
const bindActList = async (data) =>{  
  ...
  await commonSearchApi({ queryId: "RSKBA0010_ELEC_SEARCH_10", param: { COMPANY_CD: cmpnyDiv, WORK_STANDARD_ID: wsid } })
  .then(async res => {
...
const OnSelectedGubun = row =>{  
  if(row){
    // BIZUNIT 구분 코드에 따라서 표준번호를 채번함.
    // HES-WX50(부서)-00(공정)-001(표준번호)-01(개정 횟수)로 자동 생성되도록 개선
    console.log("row.BIZUNIT=" + row.BIZUNIT)
...
const getRevision = async id => {  
  ...
  await commonSearchApi({ queryId: "RSKBA0010_ELEC_SEARCH_22", param: params  }).then(res => {   
...
const getCheckRevision =  () => {  
  ...
  // GET_CHECK_REVISION
  commonSearchApi({ queryId: "RSKBA0010_ELEC_SEARCH_01", param : { WORK_STANDARD_NO: WORK_STANDARD_MASTER.WORK_STANDARD_NO }  }).then( res => {   
...
const revision = res => {
  ...
  if (bRevision.value) {
    ...
    if (revstatus === 'UP_AW' || revstatus === 'UC_AR' || revstatus === 'UC_AB' ) {
      return Message.warn(t("현재 개정 중인 작업표준입니다. 작업표준/위험성 평가 - 작성중' 화면에서 확인하시기 바랍니다."))
    } else if (revstatus === 'UC_AD' || revstatus === 'UC_AP') {  // UC_AD: 결재상신,  UC_AP: 결재중
      return Message.warn(t("현재 개정하여 결재 진행중인 작업표준입니다.작업표준/위험성 평가 - 결재진행중' 화면에서 확인하시기 바랍니다."))
    }
  } 
  else {
    ...
    commonSearchApi({ queryId: "RSKBA0010_ELEC_SEARCH_01", param: { WORK_STANDARD_NO: WORK_STANDARD_MASTER.WORK_STANDARD_NO }  }).then( res => {
      ...
      if(parseInt(WORK_STANDARD_MASTER.WORK_STANDARD_REVISION) === rev){   // true
        checkRevision(true)                
      }
      else if (parseInt(WORK_STANDARD_MASTER.WORK_STANDARD_REVISION) < rev) {   
        vm.$swal({
          title: t("현재 개정 중인 작업표준이 존재합니다. <br> 다시 개정하시겠습니까? 기존에 작성중인 내용은 삭제됩니다."),
          showCancelButton: true,
        }).then((swalRes) => {
...
const Addselect = (type, pid, id) => {    
  if (type === 'unit') {
    ...
    WORK_STANDARD_UNIT.find(
      t =>
        t.UNIT_ID ===
        WORK_STANDARD_ACT.find(x => x.ACT_ID === pid).UNIT_ID
    ).IS_SELECT = true
...
const searchUnit = async id => {
  ...
  // PKG_RSKBA0010.WORK_STANDARD_UNIT_S
  await commonSearchApi({ queryId: "RSKBA0010_ELEC_SEARCH_05", param: { WORK_STANDARD_ID : id }  }).then(async res => {   
    let maxId 
    let maxIdx 
    let SAVE_STATUS 
...
const searchAct = async id => {
  ...
  await commonSearchApi({ queryId: "RSKBA0010_ELEC_SEARCH_06", param: { WORK_STANDARD_ID : id, TRANS : WORK_STANDARD_MASTER.TRANS_CD }  }).then(async res => {    
    let maxId 
    let maxIdx 
    let newId
    let SAVE_STATUS
...
const searchRisk = async (id, data) => {
  ...
  await commonSearchApi({ queryId: "RSKBA0010_ELEC_SEARCH_07", param: { WORK_STANDARD_ID : id, TRANS : WORK_STANDARD_MASTER.TRANS_CD }  }).then(async res => {    
    // 선택된 소분류 찾기
    let maxId 
    let maxIdx 
    let newId 
...
const fileSave = () => {
  ...
  if(fileId == null || fileId == '') {
    ...
    commonSearchApi({ queryId : 'RSKBA0010_ELEC_SEARCH_20', param: params }).then( res => {          
...
const fileSave2 = () => {
  ...
  ArrayList.value.forEach(item => {
    ...
    // }
      ...
      if(item.id === sItem.value){
          unitId = item.UNIT_ID
          fileId = item.FILE_ID
          console.log('Item.fileId = ', item.FILE_ID)
        }  
  })
  ...
  if(fileId == null || fileId == '') {
    ...
    commonSearchApi({ queryId : 'RSKBA0010_ELEC_SEARCH_20_1', param: params }).then( res => {          
      
      if(res.ORESULT_CUR.length > 0)
      {
      
        // 기존 Hi-Standard는 File_ID없고, ACT_ID로 관리하기 때문에 분기 처리
        let fid = res.ORESULT_CUR[0].FILE_ID != null ? res.ORESULT_CUR[0].FILE_ID : res.ORESULT_CUR[0].UNIT_ID
...
const uploaded = param => {
  ...
  return commonExecuteApi({ queryId : 'RSKBA0010_ELEC_SAVE_10', list: params })  

}

// 파일 업로드 후 저장 (UNIT)
const uploadedUnit = param => {
  ...
  return commonExecuteApi({ queryId : 'RSKBA0010_ELEC_SAVE_10_1', list: params })  
...
const deleted = list => {
  ...
  commonSearchApi({ queryId : 'RSKBA0010_ELEC_SEARCH_20', param: { WORK_STANDARD_ID : params.WORK_STANDARD_ID, ACT_ID : params.ACT_ID }  }).then( res => {              
    if(res.ORESULT_CUR.length == 0)
    {
      return commonExecuteApi({ queryId : 'RSKBA0010_ELEC_SAVE_11', list: params })  
    }  
...
const approvalPopUp = (taskType) => {
  ...
  if (isEmpty(WORK_STANDARD_MASTER.IS_SAVE)) {
    ...
    return Message.warn(t('표준서 정보의 사업장 안전보건정보(사고, 현장위험성평가, 안전작업요구권, 안전리스크) 중 하나를 확인해야 결재상신이 가능합니다.'))
  }
  else 
  {
...
const infoSet = taskType => {
  let contents = ''
  let date = WORK_STANDARD_MASTER.APPROVAL_DATE === null ? '' : dayjs(WORK_STANDARD_MASTER.APPROVAL_DATE).format('YYYY-MM-DD')

  switch (taskType) {
  case 'WD':
    contents = '<div class="d-flex flex-row">' + '다음 작업표준에 대한 폐지 결재를 상신합니다. 확인 후 승인하여 주시기 바랍니다' + '</div>' +
    ...
    '<td style="border: 1px solid black; border-collapse : collapse;">' + WORK_STANDARD_MASTER.GUBUN_NM + '</td>' +
    '</tr>' +
    '<tr>' +
    '<th style="border: 1px solid black; border-collapse : collapse;">표준번호</th>' +
    '<td style="border: 1px solid black; border-collapse : collapse;">' + WORK_STANDARD_MASTER.WORK_STANDARD_NO +
    '-' + WORK_STANDARD_MASTER.WORK_STANDARD_REVISION +
    '</td>' +
    '</tr>' +
    '<tr>' +
    '<th style="border: 1px solid black; border-collapse : collapse;">표준서명</th>' +
    '<td style="border: 1px solid black; border-collapse : collapse;">' + WORK_STANDARD_MASTER.WORK_STANDARD_NM + '</td>' +
    '</tr>' +
    '<tr>' +
    ...
    '<td style="border: 1px solid black; border-collapse : collapse;">' + WORK_STANDARD_MASTER.USER_INFO + '</td>' +
    '</tr>' +
    '<tr>' +
    '<th style="border: 1px solid black; border-collapse : collapse;">평가유형</th>' +
    '<td style="border: 1px solid black; border-collapse : collapse;">' + WORK_STANDARD_MASTER.RISK_TYPE_NM + '</td>' +
    '</tr>' +
    '<tr>' +
    '<th style="border: 1px solid black; border-collapse : collapse;">제/개정사유</th>' +
    '<td style="border: 1px solid black; border-collapse : collapse;">' + WORK_STANDARD_MASTER.WORK_STANDARD_DESC + '</td>' +
    '</tr>' +
    '<tr>' +
    '<td colspan="2">' +
  ...
  contents = `다음 작업표준에 대한 폐지 결재를 상신합니다. 확인 후 승인하여 주시기 바랍니다
    ...
    */
    ...
    return {
      apprFormName:
        '[Hi-STANDARD] ' + WORK_STANDARD_MASTER.WORK_STANDARD_NM,
      apprSubject:
        '[위험성평가] ' + WORK_STANDARD_MASTER.WORK_STANDARD_NM,
  ...
  case 'R': 
    ...
    contents = `
      ...
      contents = '<div class="d-flex flex-row">' + '아래와 같이 위험성평가 결과를 보고드리니, 확인 후 승인하여 주시기 바랍니다' + '</div>' +
      '<table class="table_10" style="margin-bottom:5px; border-collapse : collapse; width: 400px;">' +
      '<colgroup>' +
      '<col style="width:30%">' +
      '<col style="width:70%">' +
...
const onLoad = async () => {
  ...
  if(wtype != null)
  {
    ...
    menuTitle.value.disableBtn('btnRevision', wtype == 'U' ? false : true) // 개정
    cnt = 0
  }
  ...
  <v-card class="pa-0 fill-height">
    <v-card-title class="pa-3 pb-0">
      <IMenuTitle
        ...
        :button-list="['btnStandardCopy', 'btnNew', 'btnUpdate', 'btnReqApply', 'btnPartnerApply', 'btnPartnerReject', 'btnRevision', 'btnRskDel', 'btnCancelRSKDel', 'btnSimplePrint', 'btnDetailPrint', 'btnWordPrint']"
          ...
          <div class="h-grow">
            ...
            <v-sheet class="h-auto pr-1">
              <div class="fill-height">
                ...
                <v-sheet height="116px" class="mb-3">
                  <table class="mb-2">
                    <tr class="tr-data" style="height: 50px; background-color: #e4e4e4;">
                      ...
                      <td width="34%">{{ SAGO.HUE_CNT }}건<br/>휴업사고(3일이상)</td>
                      <td width="33%">{{ SAGO.IL_CNT }}건<br/>휴업사고(3일미만)</td>
                    </tr>
                  </table>
                  <table>
                ...
                <div v-if="userStore.bsnsCd == 'AK00'">
          ...
          <div class="d-flex flex-column fill-height">
            ...
            <div class="d-flex pr-1 mb-1">
              <v-sheet class="d-flex justify-space-between" width="300px">
                ...
                <span>
                  <VIcon color="info" icon="mdi-image" @click="openPopup2('addFile2')"/>
                    ...
                    @mousedown="selectRow(element)" v-if="element.STATUS == 'C' || element.STATUS == 'U'">
                      <v-sheet class="d-flex flex-column" width="300px">
                        <v-sheet :id="'card1_' + element.id" class="d-flex justify-space-between align-center todo-card pa-1" color="white"
                          ...
                          <i-textarea v-model="element.UNIT_NM" :row="6" width="270px" margin="0px"/>
                        </v-sheet>
                      </v-sheet>
    ...
    <div class="title-bar" @mousedown="startDragging">안전사고</div>
      <SAFAB0010 dialog="Y" :val1="WORK_STANDARD_MASTER" :half="getHalfDate()" @close="dialogSAFAB0010 = false"/> 
    </VDialog>
    <VDialog
    ...
    <IUploadPopup ref="fileUploadPopup" @uploaded="uploadedUnit($event)" @deleted="deleted($event)"
    />     
