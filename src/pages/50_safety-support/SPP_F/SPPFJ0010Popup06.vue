...
const openPopup = (isNew, data) => {
  ...
  Promise.all([
    ...
    codeList.ITEM_CD = res[0].ORESULT_CUR
  })
  // 변수 초기화 작업
  initSaveFiles()
  initSaveParams()
  ...
  watch(
    ...
    (newVal, oldVal) => {
      ...
      if (newVal !== oldVal) {
        data.ITEM_CD = newVal
        commonSearchApi({
          queryId: 'SPPFJ0010_SEARCH_12',
          param: {
            BSNS_CD: data.BSNS_CD, //사업부
            DEPT_CD: data.DEPT_CD, //부서
            PROCESS_CD: data.PROCESS_CD, //공정설비
          ...
          if (res.ORESULT_CUR.length > 0) {
            ...
            const newData = res.ORESULT_CUR[0]

            data.BSNS_CD = newData.BSNS_CD
            data.DEPT_CD = newData.DEPT_CD
            data.PROCESS_CD = newData.PROCESS_CD
            data.ITEM_CD = newData.ITEM_CD
            data.ITEM_SEQ = newData.ITEM_SEQ
            data.UPDATE_DETAIL = newData.UPDATE_DETAIL
            data.UPDATE_REASON = newData.UPDATE_REASON
            data.LAST_INSERT_DATE = newData.INSERT_DATE
            data.LAST_INSERT_USER_ID = newData.INSERT_USER_ID
            data.LAST_INSERT_USER_NM = newData.INSERT_USER_NM
            data.LATEST_INSERT_SEQ = '1'
            ...
            commonSearchApi({
              ...
              data.ITEM_SEQ = res.ORESULT_CUR[0].ITEM_SEQ
              data.UPDATE_DETAIL = ''
              data.UPDATE_REASON = ''
              data.LAST_INSERT_DATE = ''
              data.LAST_INSERT_USER_ID = ''
              data.LAST_INSERT_USER_NM = ''
              data.LATEST_INSERT_SEQ = ''
...
const searchRegisterData = param => {
  ...
  commonSearchApi({ queryId: 'SPPFJ0010_SEARCH_14', param: param }).then(res => {
...
const searchModifyData = param => {
  ...
  Promise.all([
    commonSearchApi({ queryId: 'SPPFJ0010_SEARCH_15', param: param }),
    commonSearchApi({ queryId: 'SPPFJ0010_SEARCH_16', param: param })
  ]).then(res => {
...
const setInfo = (isNew, data) => {
  ...
  console.log(saveParams.SAVE_YN)
  ...
  //   console.log("v2")
  //   saveParams.ITEM_SEQ = ''
  //   saveParams.LATEST_INSERT_SEQ = ''
  //   saveParams.LAST_INSERT_USER_ID = ''
  //   saveParams.LAST_INSERT_USER_NM = ''
  //   saveParams.LAST_INSERT_DATE = ''
  //   saveParams.UPDATE_REASON = ''
  //   saveParams.UPDATE_DETAIL = ''
  // }
...
const saveFile = async (index) => {
  ...
  commonExecuteApi({ queryId: 'SPPFJ0010_SAVE_16', list: arr }).then(res => { 
  })
}
...
const saveData = () => {
  ...
  // const res = await 
  console.log([saveParams])
  return commonExecuteApi({ queryId: 'SPPFJ0010_SAVE_15', list: [saveParams] })
...
<template>
  <VDialog
    ...
    height="800"
  >
    <div class="title-bar" @mousedown="startDragging">공정안전자료 변경이력 등록</div>
                ...
                <VRow>
                  <VCol cols="6">
                    <i-select
                      ...
                      label-width="40px"
                      placeholder="항목"
                      v-model="saveParams.ITEM_CD"
                      :items="codeList.ITEM_CD"
                      item-value="COD"
                      item-title="TXT"
                    />
                  </VCol>
                  ...
                  <VCol cols="5" style="display: flex;">
                    ...
                    <div class="d-flex align-center" v-if="!isDisabled">
                      <v-file-input 
                        ...
                        ref="fileInput" 
                      />            
                      <v-btn @click="openFileInput(index)" :disabled="isDisabled || popupRead">업로드</v-btn>
                    </div>
                    <div class="d-flex align-center" v-if="isDisabled || popupRead">           
