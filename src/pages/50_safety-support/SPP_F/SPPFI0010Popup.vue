...
const uploadBtn = reactive(['btnDownLoad', 'btnDelete'])
const isAdmin = ref(0)
...
const initParams = (data) => {
  ...
  saveParams.GUBUN = data.GUBUN
  saveParams.CONTENT = data.CONTENT
  saveParams.REMARKS = data.REMARKS
  ...
  write.value = data.DIV == 'C' || (data.DIV == 'M' && data.WRITE == 'Y') ? true : false
  saveParams.REV_SEQ = data.DIV == 'C' ? 0 : data.REV_SEQ
  saveParams.INSPECT_DATE = data.DIV == 'C' ? dayjs().format('YYYY-MM-DD') : dayjs(data.INSPECT_DATE).format('YYYY-MM-DD')
  saveParams.FILE_ID = data.DIV == 'C' || isEmpty(data.FILE_ID) ? create_UUID() : data.FILE_ID
...
const initCodeList = () => {
  Promise.all([ 
    commonSearchApi({ queryId: 'SPPFE0010_COMBO_01', param: saveParams }),
    commonSearchApi({ queryId: 'SPPFE0010_COMBO_02', param: saveParams }),
    commonSearchApi({ queryId: 'SPPFE0010_COMBO_03', param: saveParams }),
  ]).then(res => {    
...
const saveData = () => {
  ...
  return commonExecuteApi({ queryId: 'SPPFI0010_SAVE_01', list: [params] })
}
