...
const initParams = (data) => {
  ...
  titleInfo.bsns = data.BSNS_NM
  titleInfo.dept = data.DEPT_NM
  ...
  {
    if(data.USER_ID == userStore.userId)
    {
      userNM.value=userStore.userName + ' ' + userStore.deptNm
    }
    else
    {
      userNM.value=data.KOR_NM
    }
  }
  
  write.value = data.DIV=="C" || (data.DIV=="M" && data.WRITE=="Y") ? true : false
  revSeq.value = data.DIV=="C" ? 0 : data.REV_SEQ
  revDate.value = data.DIV=="C" ? dayjs().format("YYYY-MM-DD") : data.REV_DATE
  rev_Div.value= data.DIV=="C" ? 'A' : data.REV_DIV
  revContent.value= data.DIV=="C" ? null : data.REV_CONTENT
  remarks.value = data.DIV=="C" ? null : data.REMARKS
  fileID.value = data.DIV=="C" || isEmpty(data.FILE_ID) ? create_UUID() : data.FILE_ID
...
const saveData = async () => {
  ...
  return commonExecuteApi({ queryId: 'SPPFE0010_SAVE_01', list: [saveParams] })
