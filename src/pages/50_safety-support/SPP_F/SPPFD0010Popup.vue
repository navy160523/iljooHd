...
const openPopup = async (data) => {
  ...
  Object.assign(saveParam, {
    ...
    IRR_CONTENT: data.IRR_CONTENT,
    EVAL_TYPE: data.EVAL_TYPE,
    REG_TYPE: '0',
    REG_TECH: data.REG_TECH,
    // REG_DT: data.REG_DT,
    REG_DT: dayjs().format('YYYY-MM-DD'),
    REG_FILE_ID: data.REG_FILE_ID,
    IRR_TYPE: '1',
    IRR_TECH: data.IRR_TECH,
    IRR_DT: data.IRR_DT,
    IRR_FILE_ID: data.IRR_FILE_ID,
    REMARK: data.REMARK,
    USER_ID: userStore.userId,
    // 서브그리드 구분자
    DIV: data.DIV,
...
const saveData = () => {
  ...
  return commonExecuteApi({ queryId : 'SPPFD0010_SAVE_01', list: [saveParam] }) 
}

const saveData2 = () => {
  // console.log("정확한 값2:", JSON.parse(JSON.stringify(saveParam)));
  return commonExecuteApi({ queryId : 'SPPFD0010_SAVE_02', list: [saveParam] })
}
        ...
        <v-container fluid class="searchArea fill-height pa-0">
          <VCol style="height: 100%;">
            ...
            <div class="mb-3">
              ...
              item-value="COD"
              item-title="TXT"
              :disabled="saveParam.DIV === 'Y' || saveParam.DIV === 'N'"
              />
            </div>
              ...
              <p v-if="saveParam.ADD_DIV === 'Y'" class="text-red">※ 내용 저장 후 파일을 업로드 하실 수 있습니다.</p>
                ...
                <div style="height: 320px;">
                  ...
                  @dataLength="onDataLength1"
                    ...
                    :isVisibled="(saveParam.DIV =='Y' || saveParam.ADD_DIV == 'Y') ? false : true"
                    />
            ...
            <div v-if="saveParam.EVAL_TYPE === '1' || saveParam.IRR_TYPE === '0'" class="d-flex flex-wrap">
                ...
                <div style="width: 100%; height: 320px;">
                  <IUpload ref="fileUpload2" :guid2="saveParam.IRR_FILE_ID" @uploaded="uploaded2" @dataLength="onDataLength2" :isVisibled="(saveParam.DIV =='Y' || saveParam.ADD_DIV == 'Y') ? false : true"/>
                </div>
            </div>
          </VCol> 
        </v-container>
