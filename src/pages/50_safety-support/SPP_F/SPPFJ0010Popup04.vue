...
onMounted(()=>{
  ...
  const reviewerEmails = params.get('REVIEWER_EMAILS');
  searchParams.EMAIL = reviewerEmails ? reviewerEmails.split(',') : [];
...
const saveReview = async() => {
  ...
  const res = await commonExecutenoAuthApi({ queryId: 'SPPFJ0010_SAVE_09', list: saveParams })
  const confirmCheck = res.list[0]

  if (confirmCheck.CONFIRM_YN == 'Y') { 
    const reviewLink = `${window.location.origin}/#/50_safety-support/SPP_F/SPPFJ0010Popup05?EMP_NO=&KEY=&BSNS_CD=${tab2ConfirmParams.BSNS_CD}&DEPT_CD=${tab2ConfirmParams.DEPT_CD}&PROCESS=${tab2ConfirmParams.PROCESS}&CHG_NO=${tab2ConfirmParams.CHG_NO}&CHG_NM=${encodeURIComponent(searchParams.CHG_NM)}`;

    // 메일 콘텐츠 작성
    const mailContent = `
      변경관리위원회 변경검토 결과 승인 안내입니다.<br><br>
      아래 링크로 접속하셔서 승인결과 확인 바랍니다.<br><br>
      <a href="${reviewLink}">변경검토 작성 (바로가기, 클릭)</a>
    `;

    searchParams.EMAIL.forEach(email => {
      const sendMailParam = {
        EMAIL: [email],
        SUBJECT: '변경관리위원장 승인 결과 메일',
        CONTENT: mailContent,
      };
      
      commonSendnoAuthApi(sendMailParam);
    });
  }
...
<template>
                      ...
                      <table class="custom-table">
                        ...
                        <tr>
                          <td colspan="2" style="height: 80px;">
                            <v-radio-group inline v-model="tab2ConfirmParams.CONFIRM_YN">
  ...
  <IUploadPopup ref="iUploadPopup" :readOnly="true" :isVisibled="false" :buttonList="['btnClose']"/>
</template>
<style scoped>
.page-wrapper {
  width: 1300px;
