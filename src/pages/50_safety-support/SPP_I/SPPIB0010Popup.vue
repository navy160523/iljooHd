...
const onButtonsClick = async (btn) => {
  if (btn.id === 'btnCalculate') {
    ...
    new saveFlowHelper(vm, t)
      ...
      .setBefore(beforeConfirmSave)
      .setQuery(saveConfirmData)
      .setAfter(afterConfirmSaveData)
      .run()
  } else if (btn.id === 'btnClose') {
    if (searchParams.CONFIRM_FLAG >= 1 && searchParams.CONFIRM_YN == 'N') {
      Message.warn('확정되었던 산출데이터는 반드시 다시 확정이 되어야합니다.');
      return false;
    }
    closePopup()
  }
    ...
    <v-card class="pa-3 fill-height" style="background-color: white">
      <v-card-title class="pa-2 py-0">
        <IMenuTitle
          ...
          :button-list="['btnCalculate', 'btnConfirm', 'btnClose']"
          :hiddenManuel="true"
