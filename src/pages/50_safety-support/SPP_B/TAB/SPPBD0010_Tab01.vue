...
defineOptions({
  name: '50_safety-support-SPP_B-TAB-SPPBD0010_Tab01',
})
...
const grdMainProps = reactive({
  ...
  fields : [ 
    ...
    { fieldName: 'CNT_ASGN_CD', dataType: 'number',styleName: "right-column",numberFormat: "#,##0", width: '100', editable: false, header: { text: t('부서수') }
    ,footer: {
      numberFormat: "#,##0",
    ...
    { fieldName: 'CNT_NONE', dataType: 'number',styleName: "right-column", numberFormat: "#,##0",width: '100', editable: false, header: { text: t('미작성') }
    ,footer: {
      numberFormat: "#,##0",
      valueCallback: function (grid, column, footerIndex, columnFooter, value){
        var sum = 0;
    ...
    { fieldName: 'CNT_CNFM', dataType: 'number',styleName: "right-column", numberFormat: "#,##0",width: '100', editable: false, header: { text: t('완료') }
    ,footer: {
      numberFormat: "#,##0",
      valueCallback: function (grid, column, footerIndex, columnFooter, value){
        var sum = 0;
...
const onButtonsClick1 = async() => {
  ...
  checkedRows.forEach(e=>{
    ...
    commonSearchApi({ 
      ...
      // console.log('rse',res[0].ORESULT_CUR)
  ...
  // console.log('문자 목록', data)
    ...
    data.forEach((e) => {
      let sendSMSParams = {
        ...
        TITLE : '[피난안내도 등록요청]피난안내도 등록바랍니다. -안전경영실- ' // 변수명은 TITLE 이지만 문자 내용을 뜻한다
      }
...
const onCellClicked = async (grid,clickData) => {
  ...
  Promise.all([ 
      // 디테일 데이터 조회 (상단 그리드)
      commonSearchApi({ 
        queryId : 'SPPBD0010_SEARCH_02',
        param: tempParam }),
  ]).then(res => {
      console.log('grdsub',res[0].ORESULT_CUR)
...
const onCellDblClicked = (grid,clickData) => {
  ...
  // 미작성, 완료가 Y인 셀 클릭 시 작동 
  if((clickData.column === 'CNT_CNFM' && focusedRow.CNT_CNFM === 'Y')
  || (clickData.column === 'CNT_NONE' && focusedRow.CNT_NONE === 'Y')){
  window.sessionStorage.TAB_ID = 'PlanRegister'
  window.sessionStorage.ASGN_CD1 = focusedRow.ASGN_CD
  window.sessionStorage.ASGN_NM1 = focusedRow.ASGN_NM
  window.sessionStorage.BSNS_CD1 = focusedRow.BSNS_CD
  window.sessionStorage.YEAR_DATE1 = searchParams.YEAR_DATE 
  window.sessionStorage.USE_YN1 = 'Y'
