...
// 리포트
const reportName = ref("/manage/hse/SPPCB0020.ozr")
const params = ref([])
...
const searchData = async () => {
  ...
  if (isAdmin.value) {
    searchParams.ACTION_DEPT_CD = ''
  } else {
    searchParams.ACTION_DEPT_CD = userStore.deptCd
  }

  await commonSearchApi({
    queryId : 'SPPCB0020_POPUP_SEARCH_01', 
...
const saveData = async () => {
  ...
  commonExecuteApi({
    queryId: 'SPPCB0020_POPUP_SAVE_01',
    list: [saveParams]
  }).then(res => {
...
const selectedReviewDept = val => {
  searchParams.ACTION_DEPT_CD = val.ASGN_CD
  searchParams.ACTION_DEPT_NM = val.ASGN_SHRT_NM
}
...
const FileUpload_Quarter = () => {
  fileUploadPopup2.value.openPopup(searchParams.FILE_ID_QUARTER)
}

const popClosed = async val => {
  let saveParams = {
  ...
  commonExecuteApi({
    queryId: 'SPPCB0020_POPUP_SAVE_02',
    list: [saveParams]
  }).then(res => {
    if (res.result.status === '200') {
      Message.success(t('저장 되었습니다.'))
...
const popClosed2 = async val => {
  let saveParams = {
    CMPNY_DIV: userStore.cmpnyDiv,
    YEAR: searchParams.YEAR,
    QUARTER: searchParams.QUARTER,
    FILE_ID_QUARTER: searchParams.FILE_ID_QUARTER,
    USER_ID: userStore.userId
  }
  
  commonExecuteApi({
    queryId: 'SPPCB0020_POPUP_SAVE_03',
    list: [saveParams]
  }).then(res => {
    if (res.result.status === '200') {
      Message.success(t('저장 되었습니다.'))
...
const uploaded2 = async val => {
  searchParams.FILE_ID_QUARTER = val.fileId
}
...
<template>
  ...
  <v-card class="pa-3 fill-height rounded-b-5">
    <v-card-title class="pa-0">
      <IMenuTitle
        ref="menuTitle"
        :button-list="isAdmin ? ['btnUpdate', 'btnPrint', 'btnClose'] : ['btnPrint', 'btnClose']"
    ...
    <v-card_text class="pt-0 content-area">
      <div class="d-flex flex-column fill-height">
        <v-sheet class="searchArea flex-column">
          ...
          <div class="d-flex mt-2">
            <i-textarea 
              v-model="searchParams.IMPROVEMENT"
              :label="$t('개선요구안')"
              label-width="70px"
              width="850px"
              :readonly = !isAdmin
          ...
          <div class="d-flex mt-2" v-if="isAdmin">
            <i-input
              ...
              @click:appendInner="FileUpload"
            />
            <i-input
              v-model="searchParams.FILE_COUNT_QUARTER"
              :label="$t('첨부파일(분기)')"
              width="415px"
              label-width="90px"
              readonly
              :bgColor="isAdmin ? '#FFFFFF' : '#EEEEEE'"
              append-inner-icon="mdi-magnify"
              @click:appendInner="FileUpload_Quarter"
            />
          </div>

        </v-sheet>
        <v-sheet class="searchArea flex-column" v-if="isAdmin">
          <div class="d-flex">
            ...
            <i-input 
              :label="$t('조치부서')"
              v-model="searchParams.ACTION_DEPT_NM"
              label-width="60px"
              width="300px"
