...
import SPPCB0010Popup01 from '@/pages/50_safety-support/SPP_C/popup/SPPCB0010Popup01.vue'

defineOptions({
  name: '50_safety-support-SPP_C-SPPCB0010',
})
...
// 팝업
const sPPCB0010Popup01 = ref(null)
...
const grdMainProps = reactive({
  ...
  fields : [
    ...
    { fieldName: 'HALF', dataType: 'text', width: '70', editable: false, header: { text:'분기' },
      displayCallback: function (grd, idx, val) {
        return val.substring(0, 1) + '분기'
      },
      mergeRule: { "criteria": "value" }
    }, 
    { fieldName: 'AGENDA_ITEM', dataType: 'text', width: '220', styleName: "multiline-editor left-column", editable: false, header: { text:'상정안건항목' }, mergeRule: { "criteria": "value" } },
    { fieldName: 'PROPOSER', dataType: 'text', width: '110', editable: false, header: { text:'상정주체' }, mergeRule: { "criteria": "value" } },
    { fieldName: 'CONTENT', dataType: 'text', width: '220', styleName: "multiline-editor left-column", editable: false, header: { text:'안건내용' }, mergeRule: { "criteria": "value" } },
    { fieldName: 'FILE_COUNT', dataType: 'text', width: '60', editable: false, header: { text:'보충자료' }, visible: isAdmin.value ? true : false },
    { fieldName: 'REVIEW_DEPT_NM', dataType: 'text', width: '110', editable: false, editable: false, header: { text:'검토부서' } },
    { fieldName: 'REVIEW_OPIN', dataType: 'text', width: '220', styleName: "multiline-editor left-column", editable: false, header: { text:'검토의견' }, mergeRule: { "criteria": "value" } },
    { fieldName: 'CONSULTATION_RESULT', dataType: 'text', width: '220', styleName: "multiline-editor left-column", editable: false, header: { text:'협의결과' }, mergeRule: { "criteria": "value" } },
    { fieldName: 'ACTION_RESULT', dataType: 'text', width: '220', editable: false, styleName: "multiline-editor left-column ", header: { text:'조치결과' },
      editor: {
        type: 'multiline',
    ...
    { fieldName: 'FINAL_CHECK', dataType: 'text', width: '110', editable: false, header: { text:'안전최종확인' },
      lookupDisplay: true,
      lookupData: { value: "COD", label: "TXT", list: codeList.PROGRES_STATUS },
      editor: { type: 'dropdown' },
    },
    { fieldName: 'INS_DT', dataType: 'text', width: '110', editable: false, header: { text:'등록일자' }, mergeRule: { "criteria": "value" } },
    { fieldName: 'UPT_DT', dataType: 'text', width: '110', editable: false, header: { text:'수정일자' }, mergeRule: { "criteria": "value" } },
    
    // 숨기는 필드 visible:'false'
    { fieldName: 'CMPNY_DIV', dataType: 'text' , visible: false },
    { fieldName: 'TASK_SEQ', dataType: 'text' , visible: false },
    { fieldName: 'REVIEW_DEPT_CD', dataType: 'text' , visible: false },
  ...
  if (btn.id === 'btnSearch') { // 조회
    ...
    grdMain.value.getGridView().exportGrid({
      ...
      exportLayout:[
        ...
        { column: 'HALF', header: {text: '분기'} },
        { column: 'AGENDA_ITEM', header: {text: '상정안건항목'} },
        { column: 'PROPOSER', header: {text: '상정주체'} },
        { column: 'CONTENT', header: {text: '안건내용'} },
        { column: 'REVIEW_OPIN', header: {text: '검토의견'} },
        { column: 'CONSULTATION_RESULT', header: {text: '협의결과'} },
        { column: 'ACTION_RESULT', header: {text: '조치결과'} },
        { column: 'PROGRES_STATUS', header: {text: '진행상태'} },
        { column: 'FINAL_CHECK', header: {text: '안전최종확인'} },

        // 설명 추가
        {
          column: 'EXCEL_DISCRIPTION', // 실제 데이터 컬럼과 연결되지 않음
          header: { text: `1. 진행상태는 코드로 입력해주세요.\nA: 진행전\nB: 진행중\nC: 조치완료\n\n2. 안전최종확인은 코드로 입력해주세요.\nA: 확인전\nB: 확인완료` }, // 헤더 부분 공백
          width: 400,
          textAlign: 'left'
        }
      ]
...
const searchData = () => {
  // 관리자, 부서장, 안전보건지원부-안전지원과는 전체 조회가능, 일반 사용자는 본인 부서만 조회 가능
  if (isAdmin.value || userStore.asgnCd === Admin_ASGN.value) {
...
const excelUploadCallback = async excelData => {
  ...
  excelData.forEach((item, index) => {
    let newArr = {
      YEAR: item.년도,
      HALF: item.분기,
      AGENDA_ITEM: item.상정안건항목,
      PROPOSER: item.상정주체,
      CONTENT: item.안건내용,
      STATUS: item.현황,
      // REVIEW_DEPT_: item.검토부서,
      REVIEW_OPIN: item.검토의견,
      CONSULTATION_RESULT: item.협의결과,
      ACTION_RESULT: item.조치결과,
      PROGRES_STATUS: item.진행상태,
      FINAL_CHECK: item.안전최종확인
    }
...
const saveData = async () => {
  ...
  for (let rowIdx in checkedRows) {
    ...
    data.HALF = data.HALF.substring(0, 1)
...
const onCellDblClicked = (grid, clickData) => {
  ...
  if (data.TASK_SEQ) {
    sPPCB0010Popup01.value.openPopup({ TASK_SEQ: data.TASK_SEQ })
  }
}
...
<template>
  <v-card class="pa-0 fill-height">
    ...
    <!-- 상세팝업 -->
    <SPPCB0010Popup01 ref="sPPCB0010Popup01" />
