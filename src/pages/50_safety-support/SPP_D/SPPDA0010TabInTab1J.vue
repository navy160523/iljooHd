<!-- SPPDA0010Tab1위험기계기구 안전검사 기본정보/곤돌라 탭-->
<!-- SI2팀 김성근 2024-04-03  -->
<script setup>
...
onMounted(() => {
  Promise.all([
    ...
    getCodeList("HHIFB150"), //4
  ]).then((res) => {
    codeList.STATUS = res[0].ORESULT_CUR;
    codeList.PART_DIVISION = res[1].ORESULT_CUR;
  })
  setClickData()
})
...
watch(() => props.codeList, (newValue, oldValue) => {
  if(!isEmpty(newValue)) {
    ...
    codeList.PART_DIVISION = props.codeList.FB150
  }
}, { deep: true })
...
<template>
  ...
  <div class="d-flex mt-3">
    ...
    <i-select
      ...
      width="250px"
      label-width="100px"
      v-model="inputData.PART_DIVISION"
      :items="codeList.PART_DIVISION"
      item-value="COD"
      item-title="TXT"
    />
    <i-input
  ...
  <div class="d-flex mt-3">
    <i-input
      ...
      number
      label-width="100px"
      v-model="inputData.JOINT_CNT"
    />

    <i-input
      :label="t('공구중심점(TCP)최대속도')"
      width="350px"
      margin="4px"
      label-width="150px"
      v-model="inputData.TCP_SPEED"
    />

    <i-input
      :label="t('각 구동 모터 정격출력')"
      width="415px"
      margin="10px"
      label-width="150px"
      v-model="inputData.MOTER_OUTPUT"
    />
    <i-input
      :label="t('최대 동작 영역')"
      width="250px"
      margin="10px"
      label-width="100px"
      v-model="inputData.WORKING_AREA"
    />
  </div>
