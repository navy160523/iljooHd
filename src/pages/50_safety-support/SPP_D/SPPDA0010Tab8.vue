<!-- SPPDA0010Tab1위험기계기구 안전검사 Tab8(항만크레인)-->
<!-- SI2팀 김성근 2024-04-03  -->
<script setup>
...
const grdMainProps = reactive({
  ...
  fields: [
    ...
    },
    {
      fieldName: "TRMS_NO",
      dataType: "text",
      header: { text: t("설비번호(TRMS)"), styleName: 'multiline-editor-pre' },
      width: 80
    },
    {
      fieldName: "STATUS",
...
watch(() => props.codeList, (newValue, oldValue) => {
  if(!isEmpty(newValue)) {
    ...
    grdMain.value.setBindingColumn("EQUIP_KIND_DETAIL", props.codeList.FB140, "COD", "TXT")
...
onMounted(() => {
  ...
  if (props.intData) {
    let data = props.intData.filter(
      (x) => x.EQUIP_KIND === "A" && x.PORT_CRANE_YN === "Y"
    )
