<!-- SPPDA0010Tab1위험기계기구 안전검사 Tab2(크레인)-->
<!-- SI2팀 김성근 2024-04-03  -->
<script setup>
...
const grdMainProps = reactive({
  ...
  fields: [
    ...
    {
      fieldName: "EQUIP_KIND_TEST",
      lookupDisplay: true,
      dataType: "text",
      header: { text: t("검사기준분류") },
      width: 80
    },
    {
      fieldName: "EQUIP_KIND_DETAIL",
      lookupDisplay: true,
      dataType: "text",
      header: { text: t("일반기준분류") },
      width: 120
    },
    {
      fieldName: "EQUIP_NO",
...
watch(() => props.codeList, (newValue, oldValue) => {
  if(!isEmpty(newValue)) {
    ...
    grdMain.value.setBindingColumn('EQUIP_KIND_TEST',props.codeList.F810A,'COD','TXT')
    grdMain.value.setBindingColumn('EQUIP_KIND_DETAIL',props.codeList.F812,'COD','TXT')
    grdMain.value.setBindingColumn('DRIVE_METHOD',props.codeList.F815,'COD','TXT')
  }
}, { deep: true })
...
onMounted(() => {
  ...
  console.log("props.initCodeList", props.codeList)
  ...
  if (props.intData) {
    let data = props.intData.filter(
      (x) => x.EQUIP_KIND === "A" && x.PORT_CRANE_YN === "N"
    )
  ...
  <v-card class="pa-0 fill-height">
    <div class="content-area">
      <v-sheet height="100%" class="mb-3">
        <RealGrid
          ref="grdMain"
          :style="{ height: exPanVal === 0 ? 'calc(100% - 370px)' : 'calc(100% - 120px)' }"
