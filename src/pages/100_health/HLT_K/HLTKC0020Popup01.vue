    ...
    if(type == 'A') {
      ...
      commonSearchApi({ queryId: 'HLTKC0020_SEARCH_05', param: searchParams }).then(r => {
        if (r.ORESULT_CUR.length !== 0) {
          ...
          // 입사일이 누락되면 목록 조회데이터에서 가져오기
          if(isEmpty(searchParams.JOIN_DT)) {
            searchParams.JOIN_DT = param.JOIN_DT 
          }
...
/* ********** 저장 ********** */
const saveData = () => {  
  return commonExecuteApi({ queryId: 'HLTKC0020_SAVE_01', list: [ searchParams ] })
}
...
watch(workMngModel, (val) => {
  // 없음 선택시 없음만 체크
  if(val.includes("G") && val.length > 1) {
    val = ['G']
    workMngModel.value = val
    return
  }
...
watch(mediTrmModel, (val) => {
  // 없음 선택시 없음만 체크
  if(val.includes("H") && val.length > 1) {
    val = ['H']
    mediTrmModel.value = val
    return
  }
...
watch(protectModel, (val) => {
  // 없음 선택시 없음만 체크
  if(val.includes("K") && val.length > 1) {
    val = ['K']
    protectModel.value = val
    return
  }
                ...
                <table ref="content1" border="1" cellspacing="1" cellpadding="5" style="width: 100%; text-align: center;">
                  ...
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td>( <input type="number" v-model="searchParams.SMOKE_FRE" min="0" maxlength="2" style="width: 30px" :readonly="searchParams.SMOKE_YN != 'Y' || complete" @input="checkInputNumberLength($event, 'SMOKE_FRE')" /> )회/일</td>
                      <td>( <input type="number" v-model="searchParams.SMOKE_AMT" min="0" maxlength="2" style="width: 40px" :readonly="searchParams.SMOKE_YN != 'Y' || complete" @input="checkInputNumberLength($event, 'SMOKE_AMT')" /> )개피/일</td>
                      <td>-</td>
                      <td colspan="3">
                        <v-text-field
                    ...
                    <tr>
                      ...
                      <td>( <input type="number" v-model="searchParams.DRUNK_FRE" min="0" maxlength="2" style="width: 30px" :readonly="searchParams.DRUNK_YN != 'Y' || complete" @input="checkInputNumberLength($event, 'DRUNK_FRE')" /> )회/주</td>
                      <td>( <input type="number" v-model="searchParams.DRUNK_AMT" min="0" maxlength="2" style="width: 40px" :readonly="searchParams.DRUNK_YN != 'Y' || complete" @input="checkInputNumberLength($event, 'DRUNK_AMT')" /> )잔/회</td>
                      <td>
                    ...
                    <tr>
                      ...
                      <td>( <input type="number" v-model="searchParams.EXRC_FRE" min="0" maxlength="2" style="width: 30px" :readonly="searchParams.EXRC_YN != 'Y' || complete" @input="checkInputNumberLength($event, 'EXRC_FRE')" /> )회/주</td>
                      <td>( <input type="number" v-model="searchParams.EXRC_AMT" min="0" maxlength="2" style="width: 40px" :readonly="searchParams.EXRC_YN != 'Y' || complete" @input="checkInputNumberLength($event, 'EXRC_AMT')" /> )분/회</td>
                      <td>
                        <v-text-field
                    ...
                    <tr>
                      ...
                      <td>-</td>
                      <td>( <input type="number" v-model="searchParams.MEAL_FRE" :readonly="complete" min="0" maxlength="2" style="width: 30px" @input="checkInputNumberLength($event, 'MEAL_FRE')" /> )회/일</td>
                      <td class="title1">식사 시간</td>
                      <td>( <input type="number" v-model="searchParams.MEAL_AMT" :readonly="complete" min="0" maxlength="2" style="width: 50px" @input="checkInputNumberLength($event, 'MEAL_AMT')" /> )분/회</td>
                      <td colspan="3">
                        <v-text-field
                    ...
                    <tr>
                      ...
                      <td>( <input type="number" v-model="searchParams.SLEEP_AMT" :readonly="complete" min="0" maxlength="2" style="width: 50px" @input="checkInputNumberLength($event, 'SLEEP_AMT')" /> )시간/일</td>
                      <td colspan="3">
                        <v-text-field
                      ...
                      <td colspan="7" class="left-align">
                        <div class="checkbox-group" data-code-list="WORK_MNG">
                          <v-checkbox
                            ...
                            :key="idx"
                            :disabled="complete || (searchParams.WORK_MNG.includes('G') && item.COD != 'G')"
                      ...
                      <td colspan="7" class="left-align">
                        <div class="checkbox-group" data-code-list="MEDI_TRM">
                          <v-checkbox
                            ...
                            :key="idx"
                            :disabled="complete || (searchParams.MEDI_TRM.includes('H') && item.COD != 'H')"
                      ...
                      <td colspan="7" class="left-align">
                        <div class="checkbox-group" data-code-list="HEALTH_CARE">
                          <v-checkbox
                            ...
                            :key="idx"
                            :disabled="complete || (searchParams.HEALTH_CARE.includes('G') && item.COD != 'G')"
                      ...
                      <td colspan="7" class="left-align">
                        <div class="checkbox-group" data-code-list="PROTECT">
                          <v-checkbox
                            ...
                            :key="idx"
                            :disabled="complete || (searchParams.PROTECT.includes('K') && item.COD != 'K')"
