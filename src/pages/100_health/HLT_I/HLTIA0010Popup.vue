<script setup>
import { ref, reactive, getCurrentInstance, onMounted, defineEmits, defineExpose } from "vue"
...
const popupData = ref({ BSNS_CD: "", DEPT_CD: "", BSNS_NM: "", DEPT_NM: "" });
...
const initCodeList = () => {
  ...
  //   // 회사구분 리스트 저장
  //   // codeList.CMPNY.splice(0);
  //   // codeList.CMPNY.push({ CMPNY_NM: "전체", CMPNY_DIV: "" }); // 전체 추가
  //   // codeList.CMPNY.push({ CMPNY_NM: bsnsRes.CMPNY_DIV, CMPNY_DIV: bsnsRes.CMPNY_DIV }); // API에서 온 값 추가
...
const searchData01 = () => {
  console.log("조회전서치파람 : ", searchParam);
...
const grdMainProps = reactive({
  ...
  fields : [ 
    { fieldName: 'USER_DIV', dataType: 'text', width: '70',
      header: { text: t('소속구분') } },
    { fieldName: 'BSNS_NM', dataType: 'text', width: '100',
      header: { text: t('소속') } },
...
const openPopup = (data) => {
  ...
  const companyName = getCompanyName(data.CMPNY_DIV);

  // 변환된 데이터 searchParam에 저장
  Object.assign(searchParam, {
  CMPNY_CD: data.CMPNY_DIV, // 회사 코드
  CMPNY_NM: companyName, // 매핑된 회사명
  ...
  console.log("저장된서치파람! :", searchParam);
...
const getCompanyName = (code) => {
  const company = codeList.CMPNY.find(item => item.CMPNY_DIV === code);
  return company ? company.CMPNY_NM : "알 수 없음";
};
