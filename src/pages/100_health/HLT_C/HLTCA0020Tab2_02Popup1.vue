...
const searchData = () => {
  ...
  return commonSearchApi({
    queryId: 'HLTCA0020_SEARCH2_02_POPUP',
    param: searchParam,
  })
}
...
const afterSearch = (res) => {
  ...
  // 숫자로 들어온 값을 그대로 매핑
  saveParams.TGT_CNS   = row.TGT_CNS ?? null
  saveParams.TGT_GEN    = row.TGT_GEN ?? null
  saveParams.PART_CNS = row.PART_CNS ?? null
  saveParams.PART_GEN  = row.PART_GEN ?? null
  saveParams.DONE_CNS = row.DONE_CNS ?? null
  saveParams.DONE_GEN  = row.DONE_GEN ?? null
}
...
const saveData = () => {
  ...
  // 변환이 필요한 키 목록
  const numberKeys = ["TGT_CNS", "TGT_GEN", "PART_CNS", "PART_GEN", "DONE_CNS", "DONE_GEN"];
  ...
  return commonExecuteApi({ queryId : 'HLTCA0020_SEARCH2_02_SAVE', list: [params] })
}
          ...
          <v-window v-model="tab" class="h-window">
            <v-window-item value="tab1">
              ...
              <v-sheet class="pa-0 mr-3">
                  ...
                  <i-input
                    v-model="saveParams.TGT_CNS"
                    :label="$t('사후관리 상담 대상자 뇌심질환')"
                    label-width="180px" 
                    width="260px"
                    style="margin-bottom: 10px"
                  />
                  <i-input
                    v-model="saveParams.TGT_GEN"
                    :label="$t('사후관리 상담 대상자 일반질환')"
                    label-width="180px" 
                    width="260px"
                    style="margin-bottom: 10px"
                  />
                  ...
                  <i-input
                    v-model="saveParams.PART_CNS"
                    :label="$t('사후관리 상담 참여자 뇌심질환')"
                    label-width="180px" 
                    width="260px"
                    style="margin-bottom: 10px"
                  />
                  <i-input
                    v-model="saveParams.PART_GEN"
                    :label="$t('사후관리 상담 참여자 일반질환')"
                    label-width="180px" 
                    width="260px"
                    style="margin-bottom: 10px"
                  />
                  <i-input
                    v-model="saveParams.DONE_CNS"
                    :label="$t('사후관리 상담 완료자 뇌심질환')"
                    label-width="180px" 
                    width="260px"
                    style="margin-bottom: 10px"
                  />
                  <i-input
                    v-model="saveParams.DONE_GEN"
                    :label="$t('사후관리 상담 완료자 일반질환')"
                    label-width="180px" 
                    width="260px"
                    style="margin-bottom: 10px"
                  />
