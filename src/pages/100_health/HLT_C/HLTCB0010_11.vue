...
const searchParam = reactive({
  ...
  IND_CLS: 'B', //지표분류(건강행태)
  IND_NM: 'B05', //지표명(협력사 건강관리위험대상자 개선 비율)
  BSNS_CD: '', //사업부
  DEPT_CD: '', //전체
...
const initCodeList = () => {
  codeList.IND_NM = props.codeArray2.filter((x) => x.COD.includes('B'))
...
const btnSearch = () => {
  ...
  commonSearchApi({
    ...
    for (let i = 0; i < columnName.length; i++) {
      let splitedColumn = columnName[i].split('_')
      if (splitedColumn.length > 1) {
        year.push(splitedColumn[0])
      } else {
        grdMainProps.columns.push({
          fieldName: splitedColumn[0],
          dataType: 'text',
          width: '50',
          editable: false,
          header: { text: t('구분') },
        ...
        columnLayout[0].items.push(splitedColumn[0])
      }
    }
    let uniqueColumn = [...new Set(year)] //연도 중복제거

    for (let i = 0; i < uniqueColumn.length; i++) {
      grdMainProps.columns.push({
        fieldName: uniqueColumn[i],
        dataType: 'text',
        width: '50',
        editable: false,
        header: { text: t(uniqueColumn[i]) },
      })
      grdMainProps.columns.push({
        fieldName: uniqueColumn[i] + '_1분기',
        dataType: 'text',
        width: '50',
        editable: false,
    ...
    for (let i = 0; i < columnName.length; i++) {
      if (columnName[i].includes('분기')) {
        chartsOptions.xAxis.data.push(columnName[i])
        let value = res.ORESULT_CUR[0][columnName[i]]
        if (!isNaN(value)) {
          chartsOptions.series[0].data.push(Number(value))
        }
      }
    }
    ...
    grdMain.value.getGridView().layoutByColumn('CATEGORY').spanCallback = function (grid, layout, itemIndex) {
      let valueGbun = grid.getValue(itemIndex, 'GUBUN')
      let valueCategory = grid.getValue(itemIndex, 'CATEGORY')
      if (valueCategory === ' ') {
        grdMain.value.getDataProvider().setValue(itemIndex, 'CATEGORY', valueGbun) //가로병합은 오른쪽으로 밖에 안돼서 구분 컬럼에 데이터 넣고 병합시킴
        return 2 //가로 병합 수
      } else {
        return 1
      }
  ...
  commonSearchApi({
    ...
    defineIns.NUMER = res.ORESULT_CUR[0].NUMER
...
<template>
  <v-card class="pa-0 fill-height" v-if="ifValue == 'B05'">
