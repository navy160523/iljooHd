...
const afterSearch = (res) => {
  ...
  if (!res.ORESULT_CUR || res.ORESULT_CUR.length === 0) return

  const years = [...new Set(res.ORESULT_CUR.map(item => item.YEAR))]
  const ages = [...new Set(res.ORESULT_CUR.map(item => item.AGE))]
  ...
  for(let i = 0; i < ages.length; i++) {
    ...
    if(age === '00') {
      tempRow['구분'] = tempRow['AAA'] = '운동실천 직원 수'
    } else if(age === '60') {
      tempRow['구분'] = '(운동실천) 60세 이상'
      tempRow['AAA'] = '연령별'
    } else {
      tempRow['구분'] = `(운동실천) ${age}~${age * 1 + 9}세`
      tempRow['AAA'] = '연령별'
    }
    ...
    for(let j = 0; j < years.length; j++) {
      ...
      tempRow[`${year}_남`] = _.find(result, { YEAR: year, AGE: age }).EXER_MALE_CNT
      tempRow[`${year}_여`] = _.find(result, { YEAR: year, AGE: age }).EXER_FEMALE_CNT
      tempRow[`${year}_전체`] = _.find(result, { YEAR: year, AGE: age }).EXER_ALL_CNT
    }
  ...
  for(let i = 0; i < ages.length; i++) {
    ...
    for(let j = 0; j < years.length; j++) {
      ...
      tempRow[`${year}_남`] = _.find(result, { YEAR: year, AGE: age }).EXER_MALE_RATE
      tempRow[`${year}_여`] = _.find(result, { YEAR: year, AGE: age }).EXER_FEMALE_RATE
      tempRow[`${year}_전체`] = _.find(result, { YEAR: year, AGE: age }).EXER_ALL_RATE
    }
