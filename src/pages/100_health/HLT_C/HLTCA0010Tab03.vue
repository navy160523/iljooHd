...
const grd1Props = reactive({
  ...
  fields: [
    ...
    {
      fieldName: 'CHK_RES_KR',
      header: { text: t('검진판정값(한글)') },
      width: '100',
      dataType: 'text',
      editable: false,
    },
    {
      fieldName: 'CHK_RES_EN',
      header: { text: t('검진판정값(영어)') },
      width: '100',
      dataType: 'text',
      editable: false,
    },
...
const MaindeleteData = () => {
  ...
  return commonExecuteApi({ queryId: 'HLTCA0010_DELETE_FORM_03', list: deleteParams })
}

// 1. 키 매핑 사전 (정규화된 키 => DB 컬럼명)
const fieldMapForm1 = {
  EMPNUM: 'EMP_NO', // 사번
  DEPARTNAME: 'E_DEPT_NM', // 부서명
  WORKCLASS: 'E_UNT_DUTY_CD', // 직무
  AGE: 'AGE', // 나이
  SEX: 'GENDER', // 성별
  EXADATE: 'EXADATE', // 검진 일자
  f_2: 'CHK_RES_KR', // 검진판정값(한글)
  f_1: 'CHK_RES_EN', // 검진판정값(영어)
  OPINION: 'OPINION', // 소견
  ACTION: 'ACTION', // 사후조치
  DECFCD: 'DECFCD', // 소견코드
  DECSCD: 'DECSCD', // 검진판정값 코드
  CONSENT: 'CONSENT', // 개인전보 동의 1:동의, 0: 비동의
}
...
const excelUploadCallbackForm1 = (excelData) => {
  ...
  const normalizedFieldMap = Object.fromEntries(Object.entries(fieldMapForm1).map(([k, v]) => [k.toUpperCase(), v]))
  ...
  normalizedData.forEach((item) => {
    ...
    for (const [normalizedKey, dbColumn] of Object.entries(normalizedFieldMap)) {
      const foundKey = Object.keys(item).find((key) => key.toUpperCase() === normalizedKey)
      if (foundKey) {
        rowData[dbColumn] = item[foundKey]
      }
    }
    // console.log(rowData)
  ...
  commonExecuteApi({ queryId: 'HLTCA0010_SAVE_FORM_03', list: saveList }).then((res) => {
    onButtonsClick({ id: 'btnSearch' })
  ...
  <div class="d-flex flex-column fill-height">
    ...
    <div style="text-align: end">
      <span style="font-size: 15px; font-weight: bold; color: red">
        ※ 엑셀 업로드 전 (검진결과), DECFCD 컬럼 값이 없는 경우 ‘Z0000’을 입력 후 업로드하십시오. (소견: 특수검진 모든
        유해인자 정상 / 그 외 특수검진 유해인자 정상 해당)
      </span>
    </div>
