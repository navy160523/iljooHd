...
const searchParam = reactive({
  ...
  IND_CLS: 'E', //지표분류(건강행태)
  IND_NM: 'E02', //지표명(유해인자별 작업환경측정 초과율)
  BSNS_CD: '', //사업부
  DEPT_CD: '', //전체
  ...
  commonSearchApi({
    ...
    for (let i = 0; i < columnName.length; i++) {
      if (columnName[i] === 'GUBUN' || columnName[i] === 'CATEGORY') {
        ...
        chartsOptions.series[7].data.push(res.ORESULT_CUR[3][columnName[i]]) //산 및 알칼리류
      }
    }
    for (let i = 0; i < columnName.length; i++) {
      if (columnName[i] === 'GUBUN' || columnName[i] === 'CATEGORY') {
        ...
        chartsOptions.series[0].data.push(res.ORESULT_CUR[4][columnName[i]]) //소음
      }
    }
    for (let i = 0; i < columnName.length; i++) {
      if (columnName[i] === 'GUBUN' || columnName[i] === 'CATEGORY') {
        ...
        chartsOptions.series[2].data.push(res.ORESULT_CUR[5][columnName[i]]) //유기화합물
      }
    }
    for (let i = 0; i < columnName.length; i++) {
      if (columnName[i] === 'GUBUN' || columnName[i] === 'CATEGORY') {
    ...
    for (let i = 0; i < columnName.length; i++) {
      if (columnName[i] === 'GUBUN' || columnName[i] === 'CATEGORY') {
        ...
        chartsOptions.series[3].data.push(res.ORESULT_CUR[7][columnName[i]]) //금속류 및 기타분진
      }
    }
...
<template>
  <v-card class="pa-0 fill-height" v-if="ifValue == 'E02'">
