  ...
  commonSearchApi({
    ...
    for (let i = 0; i < columnName.length; i++) {
      if (columnName[i] === 'GUBUN' || columnName[i] === 'CATEGORY') {
        ...
        chartsOptions.series[0].data.push(res.ORESULT_CUR[0][columnName[i]]) //직업병
      }
    }
    for (let i = 0; i < columnName.length; i++) {
      if (columnName[i] === 'GUBUN' || columnName[i] === 'CATEGORY') {
        ...
        chartsOptions.series[1].data.push(res.ORESULT_CUR[1][columnName[i]]) //일반질병
      }
    }
  })
  commonSearchApi({
...
<template>
  <v-card class="pa-0 fill-height" v-if="ifValue == 'E07'">
