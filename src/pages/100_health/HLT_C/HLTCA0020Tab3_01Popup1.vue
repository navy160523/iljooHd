...
const searchData = () => {
  ...
  return commonSearchApi({
    queryId: 'HLTCA0020_SEARCH3_01_POPUP',
    param: searchParam,
  })
}
...
const afterSearch = (res) => {
  ...
  // 숫자로 들어온 값을 그대로 매핑
  saveParams.STD_EMP_DIR = row.STD_EMP_DIR ?? null
  saveParams.STD_EMP_PART = row.STD_EMP_PART ?? null
  saveParams.PPE_COST = row.PPE_COST ?? null
  saveParams.SAFE_MAT_COST = row.SAFE_MAT_COST ?? null
  saveParams.SUPPLY_UNIT_COST = row.SUPPLY_UNIT_COST ?? null
}
...
const beforeSave = () => {
  ...
  // if (year > 2021) {
  //   Message.warn("2022년 이전 데이터만 저장할 수 있습니다.");
  //   return false;
  // }
...
const saveData = () => {
  // 변환이 필요한 키 목록
  const numberKeys = ['STD_EMP_DIR', 'STD_EMP_PART', 'PPE_COST', 'SAFE_MAT_COST', 'SUPPLY_UNIT_COST']
  ...
  return commonExecuteApi({ queryId: 'HLTCA0020_SEARCH3_01_SAVE', list: [params] })
}
...
<template>
  <v-dialog v-model="dialog" eager persistent width="300" class="p-absolute user-select-none">
            ...
            <v-window-item value="tab1">
              <div class="h-grow">
                <v-sheet class="pa-0 mr-3">
                  ...
                  <i-input
                    v-model="saveParams.STD_EMP_DIR"
                    :label="$t('기준인원(직영)')"
                    label-width="180px"
                    width="260px"
                  ...
                  />
                  <i-input
                    v-model="saveParams.STD_EMP_PART"
                    :label="$t('기준인원(협력사)')"
                    label-width="180px"
                    width="260px"
                    style="margin-bottom: 10px"
                  ...
                  <i-input
                    v-model="saveParams.PPE_COST"
                    :label="$t('안전보호구 소요 비용(원)')"
                    label-width="180px"
                    width="260px"
                    style="margin-bottom: 10px"
                  />
                  <i-input
                    v-model="saveParams.SAFE_MAT_COST"
                    :label="$t('안전소모품 소요 비용(원)')"
                    label-width="180px"
                    width="260px"
                    style="margin-bottom: 10px"
                  />
                  <i-input
                    v-model="saveParams.SUPPLY_UNIT_COST"
                    :label="$t('1인당 안전보호구/안전소모품 지급 비용(원)')"
                    label-width="180px"
                    width="260px"
                    style="margin-bottom: 10px"
                  />
