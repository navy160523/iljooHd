...
const searchData = () => {
  ...
  return commonSearchApi({
    queryId: 'HLTCA0020_SEARCH5_01_POPUP',
    param: searchParam,
  })
}
...
const afterSearch = (res) => {
  ...
  // 숫자로 들어온 값을 그대로 매핑
  saveParams.MEA_HI = row.MEA_HI ?? null
  saveParams.MEA_LO = row.MEA_LO ?? null
  saveParams.LOW_HI = row.LOW_HI ?? null
  saveParams.LOW_LO = row.LOW_LO ?? null
  saveParams.OVER_HI = row.OVER_HI ?? null
  saveParams.OVER_LO = row.OVER_LO ?? null
}
...
const saveData = () => {
  ...
  // 변환이 필요한 키 목록
  const numberKeys = ["MEA_HI", "MEA_LO", "LOW_HI", "LOW_LO", "OVER_HI", "OVER_LO"];
  ...
  return commonExecuteApi({ queryId : 'HLTCA0020_SEARCH5_01_SAVE', list: [params] })
}
