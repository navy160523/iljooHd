...
const chartsOptions = reactive({
  ...
  yAxis: {
    ...
    min: function(value) {
      ...
      // return Math.floor(value.min) - (Math.floor(value.min) % 5)
      return Math.floor(value.min)
    },
    max: function(value) {
      // return Math.ceil(value.max / 10) * 10
      // return Math.floor(value.max) - (Math.floor(value.max) % 5) + 5
...
onMounted(() => {
  console.log('props.codeArray2', props.codeArray2)
  checkAuth()
  initCodeList()
...
<template>
  <v-card class="pa-0 fill-height" v-if="ifValue == 'A01'">
