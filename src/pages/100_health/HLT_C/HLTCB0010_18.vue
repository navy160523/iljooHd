...
const chartsOptions = reactive({
  ...
  yAxis: [
    { type: 'value', name: '건수', min: 0 }, // 0번 축: 막대
    {
      type: 'value',
      name: '비율(%)',
      min: function(value) {
  ...
  commonSearchApi({
    ...
    for (let i = 0; i < columnName.length; i++) {
      if (columnName[i] == 'GUBUN' || columnName[i] === 'CATEGORY') {
        ...
        chartsOptions.series[0].data.push(res.ORESULT_CUR[3][columnName[i]]) //노출기준 초과건수 차트y축 데이터
      }
    }
    for (let i = 0; i < columnName.length; i++) {
      if (columnName[i] === 'GUBUN' || columnName[i] === 'CATEGORY') {
        ...
        chartsOptions.series[1].data.push(res.ORESULT_CUR[2][columnName[i]]) //노출기준 미만건 차트 y축데이터
      }
    }
    for (let i = 0; i < columnName.length; i++) {
      if (columnName[i] === 'GUBUN' || columnName[i] === 'CATEGORY') {
        ...
        chartsOptions.series[2].data.push(res.ORESULT_CUR[0][columnName[i]]) //노출기준 미만건 차트 y축데이터
      }
    }
  })
  commonSearchApi({
...
<template>
  <v-card class="pa-0 fill-height" v-if="ifValue == 'E01'">
