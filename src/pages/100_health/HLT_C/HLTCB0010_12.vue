<script setup>
//보건자원 및 기타 - 직원당 안전보호구/안전소모품 소요비용
...
const searchParam = reactive({
  ...
  IND_CLS: 'C', //지표분류(건강상태)
  IND_NM: 'C01', //지표명(직원당 보호구 및 안전 소모품 비용)
  BSNS_CD: '', //사업부
  DEPT_CD: '', //전체
...
const initCodeList = () => {
  Promise.all([
    ...
    codeList.IND_NM = props.codeArray2.filter((x) => x.COD.includes('C'))
  })
}
...
const btnSearch = () => {
  ...
  commonSearchApi({
    ...
    for (let i = 0; i < chartValue.length; i++) {
      if (!isNaN(Number(chartValue[i])) && chartValue[i] !== ' ') {
...
watch(
  ...
  (newValue, oldValue) => {
    //부서조회(일반직원이 아니면 부서조회)
    if (newValue && checkAuthFlag.GROUP_A) {
      commonSearchApi({
        queryId: 'searchDept3',
        param: { CMPNY_DIV: userStore.cmpnyDiv, BSNS_CD: newValue, USE_DIV: 'Y' },
      }).then((res) => {
...
<template>
  <v-card class="pa-0 fill-height" v-if="ifValue == 'C01'">
    ...
    <v-card-text class="pa-3 pt-0 content-area">
      <div class="d-flex flex-column fill-height">
        ...
        <v-sheet class="pa-0 h-auto">
          ...
          <div style="text-align: end">[단위:원]</div>
          <div>
            <v-sheet class="pa-0 h-auto">
              <RealGrid
                ref="grdMain"
