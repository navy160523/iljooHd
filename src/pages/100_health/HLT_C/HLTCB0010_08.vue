...
const btnSearch = () => {
  ...
  commonSearchApi({
    ...
    for (let i = 0; i < uniqueColumn.length; i++) {
      ...
      grdMainProps.columns.push({
        fieldName: uniqueColumn[i] + '_뇌심질환',
        dataType: 'text',
        width: '50',
        editable: false,
      ...
      grdMainProps.columns.push({
        fieldName: uniqueColumn[i] + '_일반질환',
        dataType: 'text',
        width: '50',
        editable: false,
        styleName: 'right-column',
      ...
      columnLayout.push({
        name: uniqueColumn[i],
        direction: 'horizontal',
        items: [uniqueColumn[i] + '_뇌심질환', uniqueColumn[i] + '_일반질환', uniqueColumn[i] + '_소계'],
      })
      chartsOptions.xAxis.data.push(uniqueColumn[i]) //차트 x축
    }
    ...
    for (let i = 0; i < res.ORESULT_CUR.length; i++) {
      // 1) 소계만 추출 (객체 그대로)
      //const filteredObj = Object.fromEntries(Object.entries(res.ORESULT_CUR[i]).filter(([key]) => key.includes('소계')))

      const filteredKeys = Object.keys(res.ORESULT_CUR[i]).filter((key) => key.includes('소계'))
      filteredKeys.sort((a, b) => {
        return parseInt(a.split('_')[0]) - parseInt(b.split('_')[0])
      })

      const filteredValues = filteredKeys.map((key) => res.ORESULT_CUR[i][key])
      if (i === 0) {
        ...
        for (let i = 0; i < filteredValues.length; i++) {
          chartsOptions.series[0].data.push(filteredValues[i])
        }
      } else {
        chartsOptions.series.push({
          name: '완료율',
        ...
        for (let i = 0; i < filteredValues.length; i++) {
          chartsOptions.series[1].data.push(filteredValues[i])
        }
      }
    }
  })
...
<template>
  <v-card class="pa-0 fill-height" v-if="ifValue == 'B02'">
