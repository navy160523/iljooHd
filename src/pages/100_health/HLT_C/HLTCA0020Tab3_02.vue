...
// import Popup1 from './HLTCA0020Tab3_02Popup1.vue'
...
// });
  ...
  // AAAA 컬럼
  fields.push({ fieldName: 'AAAA', dataType: 'text' })
  columns.push({
    name: 'AAAA',
    fieldName: 'AAAA',
    width: 70,
    ...
    editable: false,
    styleName: 'center-column',
    mergeRule: { criteria: 'values["AAAA"]' },
  })
  ...
  Object.keys(sample).forEach((key) => {
    ...
    if (['AAA', 'AAAA', '구분'].includes(key)) return
  ...
  nextTick(() => {
    ...
    if (gridView && dataProvider) {
      ...
      gridView.layoutByColumn("AAAA").spanCallback = function (grid, layout, itemIndex) {
      const aaaa = grid.getValue(itemIndex, "AAAA")
      const gubun = grid.getValue(itemIndex, "구분")
      return aaaa === gubun ? 2 : 1
    }
    }
  })
}
