...
const searchData = () => {
  ...
  return commonSearchApi({
    queryId: 'HLTCA0020_SEARCH2_03_POPUP',
    param: searchParam,
  })
}
...
const afterSearch = (res) => {
  ...
  saveParams.CLINIC_CNT = row.CLINIC_CNT ?? null
  saveParams.LOCAL_CNT = row.LOCAL_CNT ?? null
  saveParams.REHAB_CNT = row.REHAB_CNT ?? null
}
...
const saveData = () => {
  // 변환이 필요한 키 목록
  const numberKeys = ['STD_EMP_DIR', 'STD_EMP_PART', 'CLINIC_CNT', 'LOCAL_CNT', 'REHAB_CNT']
  ...
  return commonExecuteApi({ queryId: 'HLTCA0020_SEARCH2_03_SAVE', list: [params] })
}
            ...
            <v-window-item value="tab1">
              <div class="h-grow">
                <v-sheet class="pa-0 mr-3">
                  ...
                  <i-input
                    v-model="saveParams.CLINIC_CNT"
                    :label="$t('부속의원 의료시설 이용인원')"
                    label-width="180px"
                    width="260px"
                    style="margin-bottom: 10px"
                  />
                  <i-input
                    v-model="saveParams.LOCAL_CNT"
                    :label="$t('지역의무실 의료시설 이용인원')"
                    label-width="180px"
                    width="260px"
                    style="margin-bottom: 10px"
                  />
                  <i-input
                    v-model="saveParams.REHAB_CNT"
                    :label="$t('물리·재활치료실 의료시설 이용인원')"
                    label-width="180px"
                    width="260px"
                    style="margin-bottom: 10px"
                  />
                  <!-- <i-input
                    v-model="saveParams.PER_EMP_CNT"
                    :label="$t('직원 1인당 의료시설 이용 횟수')"
                    label-width="180px" 
                    width="260px"
                    style="margin-bottom: 10px"
                  /> -->
