...
const openPopup = (param) => {
  ...
  <div style="margin-left:16px;">
      ...
      <div>
          3. 요청사항: <span style="background-color:yellow;"><strong>[HiSEs - 보건 - 직업병 관리 - 업무상질병 신청 관리]</strong></span> 메뉴에서 대상자 검색 후 자료 등록 요망
      </div>
      ...
      <div style="margin-left:8px;">
          ...
          <div>
              대외송부용 인사기록부를 메일 확인 즉시 자료 등록 혹은 메일 회신
          </div>
          <div>
              &nbsp;
          </div>
  ...
  mailParams.SUBJECT = '요양급여신청 사실 통지 및 자료 등록 요청 件- ' + searchParams.EMP_NM + `(${searchParams.HLD_OFFI_GBN}) ` + searchParams.JOB_TIT_NM
  ...
  commonSearchApi({ queryId : 'OPRAD0010_SEARCH_01', param: params }).then(res => {
    for(let result of res.ORESULT_CUR) {
      if(!isEmpty(result.EMAIL)) {
        chipArr.REFERER.unshift({ EMP_NM: result.KOR_NM, EMAIL: result.EMAIL })
        mailParams.REFERER.unshift(result.EMAIL)
      }
    }
  })
  dialog.value = true
    ...
    <v-card class="pa-0 fill-height rounded-b-5">
      ...
      <v-card-text class="pa-4 pt-0 content-area">
        <div class="d-flex flex-column fill-height">
          ...
          <v-sheet class="mb-2">
            <CKEditor ref="ckEditor" id="HLTJB0010Popup02" height="100%" v-model="mailParams.CONTENT" class="mt-2" @change="onChange"/>
          </v-sheet>
        </div>        
      </v-card-text>
    </v-card>
