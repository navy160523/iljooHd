...
defineOptions({
  name:'100_health-HLT_F-HLTFC0030', 
})
...
const grd1Props = reactive({
  ...
  keys : ['GBN1','GBN2'], 
  fields : [ 
    { fieldName: 'GBN1', width: '90', editable: false, dataType: 'text', styleName: 'pk_column', header: { text: t('NO') },  mergeRule: { criteria: 'value' } },
    { fieldName: 'GBN2', width: '90', editable: false, dataType: 'text', styleName: 'pk_column', header: { text: t('차수') }, lookupDisplay: true },
    { fieldName: 'HI_0', width: '90', editable: false, dataType: 'text', styleName: 'right-column', header: { text: t('측정일자') } },
    { fieldName: 'LO_0', width: '90', editable: false, dataType: 'text', styleName: 'right-column', header: { text: t('사업부') } },
    { fieldName: 'HI_1', width: '90', editable: false, dataType: 'text', styleName: 'right-column', header: { text: t('사업부') } },
    { fieldName: 'LO_1', width: '90', editable: false, dataType: 'text', styleName: 'right-column', header: { text: t('사업부') } },
    { fieldName: 'HI_2', width: '90', editable: false, dataType: 'text', styleName: 'right-column', header: { text: t('사업부') } },
    { fieldName: 'LO_2', width: '90', editable: false, dataType: 'text', styleName: 'right-column', header: { text: t('사업부') } },
    { fieldName: 'HI_3', width: '90', editable: false, dataType: 'text', styleName: 'right-column', header: { text: t('사업부') } },
    { fieldName: 'LO_3', width: '90', editable: false, dataType: 'text', styleName: 'right-column', header: { text: t('사업부') } },
    { fieldName: 'HI_4', width: '90', editable: false, dataType: 'text', styleName: 'right-column', header: { text: t('사업부') } },
    { fieldName: 'LO_4', width: '90', editable: false, dataType: 'text', styleName: 'right-column', header: { text: t('사업부') } },
...
const onButtonsClick = btn => {
  ...
  if (btn.id === 'btnSearch') {
    ...
    if(!isEmpty(searchParams.DEPT_NM)) {
      deptNm = ' ' + searchParams.DEPT_NM
    }
    commonRequest2('hse/excel/HLTFC0020_PRINT01', { 
        data: grdMain.value.getDataProvider().getJsonRows(0,-1),
        image: [link1.href],
        param: [columnNames, {sheetName: '부서별 현황 관리'}, {fileName: '작업환경측정 통계 (부서별 현황 관리) ' + searchParams.FR_YEAR + '년_' + (searchParams.FR_YEAR + 4) + '년' + deptNm}],
    ...
    //   target: 'local',
    //   lookupDisplay: true,
    //   fileName: searchParams.FR_YEAR+ '_' + searchParams.TO_YEAR 
    //     + (searchParams.REG_DIV === 'A' ? ' 정기' : ' 수시')
    //     + searchParams.DEPT_NM == '' ? '' : searchParams.DEPT_NM
    //     + ' 작업환경측정 통계'
    //     + '.xlsx', // 저장될 파일 name
    //   documentTitle: { //제목
    //     message:  searchParams.FR_YEAR+ '_' + searchParams.TO_YEAR 
    //     + (searchParams.REG_DIV === 'A' ? ' 정기' : ' 수시')
    //     + searchParams.DEPT_NM == '' ? '' : ' ' + searchParams.DEPT_NM
    //     + ' 작업환경측정 통계',
    //     visible: true, 
...
//  조회
const searchData = () => {
  return commonSearchApi({ queryId: 'HLTFC0030_SEARCH_01', param: searchParams })
}
...
const afterSearch = (res) => {
  ...
  for(var result of res.ORESULT_CUR) {
    if (result.GBN1 == '초과건수') {
      chartData1.push({ data: getChartData(result), type: 'line', name: '초과건수', label: { show: true, position: 'bottom' }, itemStyle: { color: "#FF0000" } })
    } else if (result.GBN1 == '초과율') {
      chartData1.push({ data: getChartData(result), type: 'bar', yAxisIndex: 1, name: '초과율(%)', label: { show: true, position: 'top' }, tooltip: {valueFormatter: function (value) { return value + ' %' }}, itemStyle: { color: "#0581FD" } })
    }
  } 
  chartsOptions1.value.legend.data.push('초과건수')
  chartsOptions1.value.legend.data.push('초과율(%)')
  // 데이터 삽입
