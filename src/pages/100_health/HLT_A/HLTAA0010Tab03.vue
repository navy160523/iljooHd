<!-- 
  일주지앤에스 SI2팀 장필구
  보건 위험성 평가 > 기초 정보 관리 > 위험도 산출기준(정성)(탭) 
-->
<script setup>
...
import HLTAA0010Tab03Popup01 from '@/pages/100_health/HLT_A/HLTAA0010Tab03Popup01.vue'
import HLTAA0010Tab03Popup02 from '@/pages/100_health/HLT_A/HLTAA0010Tab03Popup02.vue'
...
const grdMainProps = reactive({
  ...
  fields : [ 
    { fieldName: 'CATEGORY_MA_NM', dataType: 'text', header: { text: t('대분류') }, width: "20",  editable: false},
    { fieldName: 'CATEGORY_ME_NM', dataType: 'text', header: { text: t('중분류') }, width: "20", editable: false},
    { fieldName: 'CATEGORY_SU_NM', dataType: 'text', header: { text: t('소분류') }, width: "20", editable: false},
    { fieldName: 'CATEGORY_1', dataType: 'text', header: { text: t('카테고리 1') }, width: "20", editable: false, styleName: "left-column"},
    { fieldName: 'CATEGORY_2', dataType: "text", header: { text: t("카테고리 2") }, width: "50", editable: false, styleName: "left-column"},
    { fieldName: 'POINT', dataType: "text", header: { text: t("배점") }, width: "10", editable: false},
...
const onButtonsClick = btn => {
  if (btn.id === 'btnSearch') {
    searchData()
    // console.log('Tab 03 조회')
    // new queryFlowHelper(vm, t)
    // .setQuery(searchData)
    // .setAfter(afterSearch)
    // .run()
  } else if (btn.id === 'btnCreate') {
    // console.log('Tab 03 추가')
    let chekedRow = ref([])
    let data = ref([])
...
const deleteData = () => { 
  ...
  return commonExecuteApi({ queryId : 'HLTAA0010_DELETE_03', list: saveParams })
}
...
<div class="d-flex flex-column fill-height">
  ...
  <!-- 팝업존 -->
  <HLTAA0010Tab03Popup01 ref="CreatePopup" @closed="onPopupClosedPopUp01"/> <!-- 추가 -->
  <HLTAA0010Tab03Popup02 ref="CopyPopup" @closed="onPopupClosedPopUp01"/> <!-- 추가 -->
