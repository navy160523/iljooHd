<!-- 
  일주지앤에스 SI2팀 장필구
  보건 위험성 평가 > 기초 정보 관리 > 위험도 산출기준(정량)(탭) 
-->
<script setup>
...
import HLTAA0010Tab02Popup01 from '@/pages/100_health/HLT_A/HLTAA0010Tab02Popup01.vue'
import HLTAA0010Tab02Popup02 from '@/pages/100_health/HLT_A/HLTAA0010Tab02Popup02.vue'
...
const grdMainProps = reactive({
  ...
  fields : [ 
    { fieldName: 'CATEGORY_NM', dataType: 'text', header: { text: t('구분') }, width: "10",  editable: false},
    { fieldName: 'METHOD_TYPE_NM', dataType: 'text', header: { text: t('구분') }, width: "10", editable: false},
    { fieldName: 'UNIT_INFORMATION_NM', dataType: 'text', header: { text: t('단위정보') }, width: "25", editable: false},
    { fieldName: 'METHOD_DETAILS', dataType: 'text', header: { text: t('산출세부') }, width: "30", editable: false},
    { fieldName: 'ONE_POINT_SYMBOL', dataType: "text", header: { text: t("1점") }, width: "35", editable: false},
    { fieldName: 'TWO_POINT_SYMBOL', dataType: "text", header: { text: t("2점") }, width: "35", editable: false},
    { fieldName: 'THREE_POINT_SYMBOL', dataType: "text", header: { text: t("3점") }, width: "35", editable: false},
    { fieldName: 'FOUR_POINT_SYMBOL', dataType: "text", header: { text: t("4점") }, width: "35", editable: false},
    { fieldName: 'FIVE_POINT_SYMBOL', dataType: "text", header: { text: t("5점") }, width: "35", editable: false},
  ...
  columnLayout: [
    ...
    {
      name: '산출방법',
      items: [ 'METHOD_TYPE_NM', 'UNIT_INFORMATION_NM', 'METHOD_DETAILS'],
      header: { text: t('산출방법') },
    },
    'ONE_POINT_SYMBOL',
    'TWO_POINT_SYMBOL',
...
const onButtonsClick = btn => {
  if (btn.id === 'btnSearch') {
    searchData()
    // console.log('Tab 02 조회')
    // new queryFlowHelper(vm, t)
    // .setQuery(searchData)
    // .setAfter(afterSearch)
    // .run()
  } else if (btn.id === 'btnCreate') {
    // console.log('Tab 02 추가')
    let chekedRow = ref([])
    let data = ref([])
...
const deleteData = () => { 
  ...
  return commonExecuteApi({ queryId : 'HLTAA0010_DELETE_02', list: saveParams })
}
...
<div class="d-flex flex-column fill-height">
  ...
  <!-- 팝업존 -->
  <HLTAA0010Tab02Popup01 ref="CreatePopup"  @closed="onPopupClosedPopUp01"/> <!-- 추가 -->
  <HLTAA0010Tab02Popup02 ref="CopyPopup" @closed="onPopupClosedPopUp01"/> <!-- 복사 -->
