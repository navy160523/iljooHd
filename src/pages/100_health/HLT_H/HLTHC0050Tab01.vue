...
const onExceldown = async () => {
  ...
  if(searchParams.REQ_CHA !== ''){
    std = codeList.REQ_CHA.find(item => item.COD === searchParams.REQ_CHA)?.TXT + '_';
  }
  ...
  grdMain.value.getGridView().exportGrid({
    ...
    target: 'local',
    lookupDisplay: true,
    fileName: '안전보호구_도수보안경 지급 현황(지급 명단)_' + dayjs(new Date()).format('YYYY') + '_' + std + '.xlsx', // 저장될 파일 name
