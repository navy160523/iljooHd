...
const initCodeList = async () =>{
  Promise.all([
    ...
    if(!codeList.BSNS_CD.some(item => item.TXT === "전체")){
    codeList.BSNS_CD.unshift({ TXT: "전체", COD: "" });
    }
...
const onButtonsClick = async (btn) => {
  if (btn.id === "btnSearch") {
    ...
    hLTHF0030Popup.value.openPopup('B', searchParam.BSNS_CD)
...
const onCellDblClicked = (grid, clickData) => {
  ...
  hLTHF0030Popup.value.openPopup(data.DIVISION, searchParam.BSNS_CD, data)
}
