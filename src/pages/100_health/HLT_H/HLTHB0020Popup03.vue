...
const openPopup = (param, status) => {
  ...
  searchParams.REQ_CHA_NM = codeList.REQ_CHA.find(item => item.COD == searchParams.REQ_CHA).TXT
...
const onButtonsClick = (btn) => {
  if (btn.id === 'btnUpdate') {
    ...
    let mailParam = {
      //EMAIL: ['BP27930@bp.hd.com'], 
      EMAIL: mailParams.EMAIL, // mailParams.EMAIL
      //REFERER: ['BP27930@bp.hd.com'], 
      REFERER: [], // mailParams.REFERER
      SUBJECT: '안전장화 '+ titleApproval + ' 안내- ' + searchParams.EMP_NM + '/' + searchParams.JOB_TIT_NM,
      ...
      if (ck.isConfirmed === true) {
        commonSendApi(mailParam).then((res) => {
          ...
          if(searchParams.STATUS !== '5') {
            ...
            commonExecuteApi({ queryId : 'HLTHB0020_SAVE_04', list: [ searchParams ] }).then(res => {
              closePopup()
              Message.success(t('메일이 전송되었습니다.'))
            })
          }
  ...
  >
    ...
    <v-card class="pa-0  rounded-b-5">
      ...
      <div ref="content" class="pa-4 ml-4 mr-4 mb-4 cont">
        <div style="font-size:28px; color:black; text-align: center; font-weight: bold;"> [안전장화 신청 {{ searchParams.STATUS == '5' ? '반려' : '승인' }} 안내]</div>
        <br>
        <div style="margin-left: 16px;">
          ...
          <!-- <div style="font-size:17px; color:rgb(23, 23, 23);"> 신청시기: {{ searchParams.REQ_CHA_NM }}</div> -->
