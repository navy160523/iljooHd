...
const openPopup = (param) => {
  ...
  for (let idx of param) {
    ...
    if(!isEmpty(idx.TO_EMAIL)){
      ...
      //   });
      //   mailParams.EMAIL.push(idx.TO_EMAIL);
  ...
  //  console.log('1111 : ', empList.value)
  const emailKey = Object.keys(empList.value);
  for(let dataA of emailKey){ // 수신자 empList.value
    // console.log('empList.value : ', empList.value[dataA])
    const innerKeys = Object.keys(empList.value[dataA]);
    // console.log('2222 : ', innerKeys)
    for(let dataB of innerKeys){ //날짜
      //  console.log('3333 : ', empList.value[dataA][dataB])
      let sendEMP = ''

      for(let dataC in empList.value[dataA][dataB]){
        // console.log('4444 : ', empList.value[dataA][dataB][dataC])
  ...
  // console.log('chipArr.TO_EMP : ', chipArr.TO_EMP)
  // console.log('chipArr.TO_SAVE : ', chipArr.TO_SAVE)
...
const singleContent = (empValue) =>{
  ...
  if(approvalYn.value === 'Y'){
    content = `<div data-v-24470f18="" class="pa-4 ml-4 mr-4 mb-4 cont"><div data-v-24470f18="" style="margin: 5px; font-size: 13px; font-weight: bold;"> 
      제목: 안전화 지급 승인 안내 </div><br data-v-24470f18=""><div data-v-24470f18="" style="margin: 5px; font-size: 13px; font-weight: bold;"> 
      ...
      안전화는 아래 시간과 장소에서 수령 가능합니다. </div><br data-v-24470f18=""><div data-v-24470f18="" style="font-size: 13px;"> 
      ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ </div><div data-v-24470f18="" style="font-size: 13px;"> 
      📍 안전화 창고 운영 안내 </div><br data-v-24470f18=""><div data-v-24470f18="" style="font-size: 13px;"> 
    ...
    content = `<div data-v-24470f18="" class="pa-4 ml-4 mr-4 mb-4 cont"><div data-v-24470f18="" style="margin: 5px; font-size: 13px; font-weight: bold;"> 
      제목: 안전화 지급 신청 반려 안내 </div><br data-v-24470f18=""><div data-v-24470f18="" style="margin: 5px; font-size: 13px; font-weight: bold;"> 
      ...
      신청하신 대상자의 안전화 신청이 아래의 사유로 반려되었음을 안내드립니다. </div><br data-v-24470f18=""><div data-v-24470f18="" style="font-size: 13px;"> 
      ...
      안전화 관리 및 효율적 운영을 위한 절차이오니, 양해 부탁드립니다. </div><br data-v-24470f18=""><div data-v-24470f18="" style="font-size: 13px;"> 
      감사합니다. </div></div></div>`
  }
  return content
}
...
const saveApprovalData = async (data) => {
  ...
  commonExecuteApi({ queryId: "HLTHA0030_SAVE_06", list: saveParam });
};

const sendAllEmails = async () =>{
  const promises = chipArr.TO_EMP.map(async (idx) => {

    let title = approvalYn.value === 'Y' ? '안전화 지급 승인 안내' : '안전화 지급 신청 반려 안내'
...
const onButtonsClick = async (btn) => {
  if (btn.id === "btnSendMail") {
    ...
    //   SUBJECT: "안전화 지급 승인 안내111",
    //   CONTENT: doc.body.innerHTML,
    // };
    // // console.log(doc.body.innerHTML);
    // console.log(mailParam.CONTENT);
    // vm.$swal({
    //   title: t("메일 전송하시겠습니까?"),
    //   showCancelButton: true,
    // }).then((ck) => {
...
<template>
  ...
  >
    ...
    >
      <span>안전화 지급/비지급 대상자 메일 작성</span>
    </v-sheet>
    <v-card class="pa-0 rounded-b-5">
      ...
      <div v-else ref="content" class="pa-4 ml-4 mr-4 mb-4 cont">
        ...
        <div style="margin-left: 16px">
          <div style="font-size: 13px">
            신청하신 대상자의 안전화 신청이 아래의 사유로 반려되었음을 안내드립니다.
          </div>
          <br />
          <div style="font-size: 13px">
            ■ 대상자명: [안전화 지급 대상 직원 이름]
          ...
          <br />
          <div style="font-size: 13px">
            안전화 관리 및 효율적 운영을 위한 절차이오니, 양해 부탁드립니다.
          </div>
          <br />
          <div style="font-size: 13px">
            감사합니다.  
      ...
      <!-- <div ref="content" class="pa-4 ml-4 mr-4 mb-4 cont">
        ...
        <div style="margin: 5px; font-size: 13px; font-weight: bold">
          제목: 월별 안전보호구 및 안전소모품 비용 실적 등록 요청 건
        </div>
        <br />
        <div style="margin-left: 16px">
          <div style="font-size: 13px">
          ...
          <br />
          <div style="font-size: 13px">
            2. 중대재해처벌법 시행에 따른 협력회사 안전/보건 부자재 운영제도가
            변경('22년 1월부)됨에 따라 안전보호구 및 안전소포품비용
          </div>
          <br />
          <div style="margin-left: 15px; font-size: 13px">
            실적 등록을 월별로 시행하고 있습니다.
          </div>
          <br />
          <div style="font-size: 13px">
            3. 각 부서에서는 담당자 지정 후 등록 주기(1회/월)에 맞게 등록 업무
            진행 부탁드립니다.
          </div>
          <br /><br />
          <div
            ...
            "
          >
            * 등록순서: HiSEs=>보건=>안전보호구 및 안전소모품 비용(직영 및
            협력사 지급 비용 입력)
          </div>
          <div
            style="
            ...
            "
          >
            ※ 부자재 업부 담당자 변경 시 변경된 담당자에게 해당 내용 전달
            바라며, 담당자 변경시 통보 바람!!!
          </div>
          <br />
          <div>
