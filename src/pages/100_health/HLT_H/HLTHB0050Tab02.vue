...
const initCodeList = () => {
  Promise.all([getCodeList('HHII808C'), getCodeList('HHII740')]).then((res) => {
...
const onExceldown = async () => {
  ...
  grdMain.value.getGridView().exportGrid({
    ...
    target: 'local',
    lookupDisplay: true,
    fileName: '안전보호구_안전장화신청 지급 현황(지급 명단)_' + dayjs(new Date()).format('YYYY') + '_' + std + '.xlsx', // 저장될 파일 name
