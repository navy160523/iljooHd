...
import HLTHA0020Popup03 from "./HLTHA0020Popup03.vue";
...
const hLTHA0020Popup03 = ref(null);
...
const btnAdd = async () => {
  if(props.isAdmin === 3){
    hLTHA0020Popup03.value.openPopup({
      readonly: false,
      CMPNY_DIV_D: true,
      FLAG_D: true,
      //BSNS_CD_D: true,
  ...
  provider.addRows([
    {
      ...
      USER_DIV: userDiv//row ? 'Z' + row.USER_DIV : 'ZZ',
    },
  ]);
};
...
const afterShoesSave = async (res) => {
  ...
  // grdMain.value
  //   .getDataProvider()
  //   .setValue(gridView.getCurrent().itemIndex, "REQ_DT", searchParam.REQ_DT);
...
const beforeDelete = () => {
  ...
  for (let itemIndex of checkedData) {
    if (
      Number(
        provider.getValue(
          grdMain.value.getGridView().getDataRow(itemIndex),
          "STATUS"
        )
      ) >= 2
    ) {
