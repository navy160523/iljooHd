...
defineOptions({
  name: '100_health-HLT_H-HLTHA0070',
})
...
const initCodeList = async () => {
  ...
  Promise.all([
    ...
    if(codeList.shoesKind.length>0)
    {
      ...
      codeList.shoesKind.push({TXT:'복지포인트',COD:null})
      if(tab.value=="tab01")
      {
...
const searchData = async () =>{
  ...
  {
  await commonSearchApi({
    // queryId: 'HLTHA0070_SEARCH_01',
    queryId: 'HLTHA0070_SEARCH_11',
    param: searchParam,
  }).then(res=>{
...
function setupTab01BandGrid(startYear, endYear){
  ...
  const columns = [
  {
    ...
    styleCallback: function (grid, dataCell) {
      var ret = {};
      if (
        grid.getValue(dataCell.index.itemIndex, "USR_DIV_TITLE") === null
      ) {
        ret = {
  ...
  for (let y = startYear; y <= endYear; y++) {
    ...
    const h1 = [
      ...
      const fieldName = `Y_${y}_CHA_1_USER_${type}`;
      return {
    ...
    const h2 = [
      ...
      const fieldName = `Y_${y}_CHA_2_USER_${type}`;
      return {
        field: { fieldName, dataType: "number"},
        column: {
          name: `col_${fieldName}`,
