...
import HLTHA0050Popup01 from "./HLTHA0050Popup01.vue"
...
const grdMainProps = reactive({
  ...
  fields: [
    ...
    { fieldName: "FACTORY", width: "100", dataType: "text", header: { text: t("공장구분"), styleName: 'header_validit' }, lookupDisplay: true, editor: { type: 'dropdown', }, requiredMessage: '[공장구분]은 필수입니다.',
      styleCallback: function(grid, dataCell) {
    ...
    { fieldName: "SHOES_GUBN", width: "200", dataType: "text", header: { text: t("안전화 종류"), styleName: 'header_validit' }, lookupDisplay: true, requiredMessage: '[안전화 종류]는 필수입니다.', editor: { type: 'dropdown', },
      styleCallback: function(grid, dataCell) {
        const ret = { editable : true, styleName : 'editable_column-left', enabled: true }
        const savedValue = grid.getValue(dataCell.index.itemIndex, 'SAVE_YN')
        if(savedValue !== 'N') {
    ...
    { fieldName: "CNT", width: "50", dataType: "number", numberFormat: '#,###', header: { text: t("수량(족)"), styleName: 'header_validit' }, requiredMessage: '[수량]은 필수입니다.', styleName: 'editable_column-right' },
    {
      fieldName: "INBOUND_DATE", width: "100", header: { text: t("일자"), styleName: 'header_validit' }, lookupDisplay: true, styleName: 'editable_column-center', requiredMessage: '[일자]는 필수입니다.',
      editor: {
        type: 'date',
        commitOnSelect: true,
    ...
    { fieldName: "REMARK", width: "100", dataType: "text", header: { text: t("비고") }, styleName: 'editable_column-left'},
  ],
  columns: []
})
...
const hLTHA0050Popup01 = ref(null)
...
const onButtonsClick = btn => {
  if(btn.id === 'btnSearch') {
    ...
    hLTHA0050Popup01.value.openPopup(props.searchParams)
  }
}
...
const deleteData = () => { 
  ...
  return commonExecuteApi({ queryId : 'HLTHA0050_DELETE_01', list: saveParams })
}
...
const searchData = () => {
  return commonSearchApi({ queryId: 'HLTHA0050_SEARCH_01', param: props.searchParams })
}
...
const beforeSave = () => {
  ...
  for (let rowIdx of checkedRows) {
    ...
    if(isEmpty(data.IO_GUBN)) {
      ...
      return Message.warn(t('안전화 종류는 필수 입력 사항입니다.'))
...
const saveData = () => {
  ...
  return commonExecuteApi({ queryId : 'HLTHA0050_SAVE_01', list: saveParams })
}
