...
import HLTHA0020_Tab01 from "./HLTHA0020_Tab01.vue";
import HLTHA0020_Tab02 from "./HLTHA0020_Tab02.vue";
import HLTHA0020_Tab03 from "./HLTHA0020_Tab03.vue";
import HLTHA0020_Tab04 from "./HLTHA0020_Tab04.vue";
import HLTHA0020Popup from "./HLTHA0020Popup.vue";
...
defineOptions({
  name: "100_health-HLT_H-HLTHA0020",
});
...
const searchParam = reactive({
  ...
  // ASGN_CD: userStore.asgnCd, // userStore.userDiv === "A" ? userStore.deptCd : userStore.asgnCd,
  ...
  //ASGN_NM: userStore.asgnNm, // userStore.userDiv === "A" ? userStore.deptNm : userStore.asgnNm,
  OUT_YN: userStore.userDiv === "A" ? "N" : "Y",
  USER_DIV: userStore.userDiv,
  USER_ID: userStore.userId,
  DUTY_YN: "N",
  //ISASMIN: await getAdmin(),
});
...
onMounted(async () => {
  // console.log('userStore1 : ', userStore)
  ...
  if(isAdmin.value === 3){
    ...
    if (userStore.userDiv === "A") {
      // console.log("직영 접속 : ", userStore.userDiv);
      ...
      // console.log("협력사 접속 : ", userStore.userDiv);
...
const buttonList = computed(() => {
  if(userStore.userDiv === "A"){
    return ["btnShoesInfo", "btnSearch", "btnApplication", "btnReqApply"];
  }else{
    return ["btnShoesInfo", "btnSearch", "btnApplication"];
  }
});
...
const onDeptSelected = async (val) => {
  // console.log("11 : ", codeList.isSearchAdmin);
  // console.log("22 : ", val.BSNS_CD); // AB00
  // console.log('val : ', val)
  if (isAdmin.value === 3) {
  ...
  // 2025-09-24 : 서무권한자에 한해서 부서별 담당자 적용으로 인해 주석
  // if (isAdmin.value == 0) {
  //   return Message.warn(
  //     t(
  //       "해당 조직으로 변경할 권한이 없습니다. 보건팀 담당자에게 권한요청하시기 바랍니다."
  ...
  // console.log('codeList : ', codeList.isSearchAdmin)
  // const isIncluded = codeList.isSearchAdmin.some(item => item.DEPT_CD === val.DEPT_CD);
  // if (isIncluded) {
  //   searchParam.ASGN_FULL_NM = val.ASGN_FULL_NM;
  //   searchParam.DEPT_CD = val.DEPT_CD;
  //   searchParam.ASGN_CD = val.ASGN_CD;
  ...
  // } else {
  //   Message.warn(
  //     t("해당 조직으로 변경할 권한이 없습니다. 보건팀 담당자에게 권한요청하시기 바랍니다.")
  //   );
  // }
};
...
const openPopupShoes = async () => {
  hLTHA0020Popup.value.openPopup();
};
</script>
...
<template>
        ...
        <v-sheet>
          <v-card class="pa-0 fill-height">
            <v-tabs v-model="tab" @update:modelValue="onTabChange">
              ...
              <v-tab v-if="userStore.userDiv === 'A' || isAdmin === 3" value="tab04">직영외 신청</v-tab>
  ...
  <HLTHA0020Popup ref="hLTHA0020Popup" />
</template>
