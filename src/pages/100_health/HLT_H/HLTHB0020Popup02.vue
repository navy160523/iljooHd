<!--
  화면명 : 안전장화 정기 지급 양식 업로드
  화면개요 : 안전장화 정기 지급 양식 업로드
-->
<script setup>
...
const grdMainProps = reactive({
  ...
  fields: [
    ...
    { fieldName: 'SHOES_KIND', width: '120', editable : false, dataType: 'text', styleName: 'left-column', header: { text: t('안전장화') }, lookupDisplay: true },
...
const onButtonsClick = (btn) => {
  if (btn.id === 'btnFormDownload') {
    let params = {
      FILE_PATH: 'HLTH',
      FILE_NAME: '안전장화 정기 지급 업로드 샘플.xlsx',
    }
...
//  조회
const searchData = () => {
  return commonSearchApi({ queryId: 'HLTHB0020_SEARCH_05', param: searchParams })
}
...
const excelUploadCallback = excelData => {  
  ...
  commonExecuteApi({ queryId: 'HLTHB0020_DELETE_01', list: [searchParams] }).then(res => {
    commonExecuteApi({ queryId: 'HLTHB0020_SAVE_02', list: saveParam }).then(re => {
      getData()
...
const saveData = () => {
  ...
  return commonExecuteApi({ queryId : 'HLTHB0020_SAVE_03', list: saveParams })
}
