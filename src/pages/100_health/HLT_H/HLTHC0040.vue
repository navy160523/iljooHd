...
defineOptions({
  name: "100_health-HLT_H-HLTHC0040",
});
...
const popupSet = async(row) => {
  ...
  if(res[0].ORESULT_CUR.length > 0){
    ...
    searchParam.GLASSES_YN = res[0].ORESULT_CUR[0].GLASSES_YN_NM;
  }else{
        ...
        <v-sheet class="searchArea d-flex flex-column">
          ...
          <div class="d-flex flex-gap mt-2">
            ...
            <i-input
              :label="$t('지급대상')"
              labelWidth="70px"
              v-model="searchParam.GLASSES_YN"
              :disabled="true"
              class="custom-flex-1"
            />
            <span  :class="props.isPopup ? '' : 'custom-flex-2'" />
