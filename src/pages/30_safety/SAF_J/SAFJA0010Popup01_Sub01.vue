...
const setData = (data) => {
  ...
  page1.CMPNY_DIV = data.CMPNY_DIV
  page1.BIZ_REG_NO = data.BIZ_REG_NO
  ...
  //공사정보
  page1.WORK_AREA_LCD = data.WORK_AREA_LCD
  page1.WORK_AREA_MCD = data.WORK_AREA_MCD
  page1.WORK_AREA_SCD = data.WORK_AREA_SCD
  page1.WORK_AREA_DESC = data.WORK_AREA_DESC
  page1.DTL_WORK_DESC = data.DTL_WORK_DESC
  page1.EQUIP_DRIVER = data.EQUIP_DRIVER
  page1.USE_EQUIP = data.USE_EQUIP
  page1.RELE_LICENSE = data.RELE_LICENSE
  page1.HARM_DPOINT = data.HARM_DPOINT
  page1.ENTRANCE = data.ENTRANCE

  //기타 신청 서류 저장
  page1.FILE_CONTRACT = data.FILE_CONTRACT
  page1.FILE_ETC = data.FILE_ETC
  page1.FILE_INSURANCE = data.FILE_INSURANCE
  page1.FILE_SAFETY = data.FILE_SAFETY
  page1.FILE_SECURITY = data.FILE_SECURITY
  page1.FILE_SIGN = data.FILE_SIGN
  page1.FILE_VEHICLES = data.FILE_VEHICLES
}
...
const chkBtn = async (nm, tel, gb) => {
  ...
  if (nm.length > 0) {
    if (tel.length === 4) {
      ...
      await commonSearchApi({
        ...
        console.log('2.res ', res.ORESULT_CUR.length)

        if (res.ORESULT_CUR.length > 0) {

          checkVal.asgnCd = res.ORESULT_CUR[0].ASGN_CD
          checkVal.bsnsCd = res.ORESULT_CUR[0].BSNS_CD
          checkVal.deptCd = res.ORESULT_CUR[0].DEPT_CD
          checkVal.cmpnyDiv = res.ORESULT_CUR[0].CMPNY_DIV
  ...
  if (gb === 0) {
    ...
    if (checkVal.cmpnyDiv != page1.CMPNY_DIV && page1.WORK_NO != '') {
      Message.warn('기존 신청한 출입회사 정보가 다릅니다.</br>공사를 삭제 후 새로 신청하시기 바랍니다.')
      btnCheck.reqEmp = 'error'
      page1.REQ_CHECK = false
      return
    }
...
const dateChange = async (e) => {
  ...
  day.value = getDateDiff(page1.WORK_FROM_DT, page1.WORK_TO_DT)+1+'일'
...
const issueYnChk = (e) => {
  if (e === 'N') {
    Message.warn(t('출입증발급 체크 해제 시 출입증이 발급되지 않습니다.') + '<br/>' + t('출입증발급이 필요한 경우 체크를 하시기 바랍니다.'))
  }
}
      ...
      <v-col class="mR-5 pa-5 bg-base">
        <v-row no-gutters class="align-center pB-10">
          ...
          <v-radio-group inline v-model="radio" class="radioGroup" readonly>
            ...
            <v-radio color="primary" label="진행중공사 인원추가" :value="'PIC'" class="mr-4"></v-radio>
            <v-radio color="primary" label="과거공사 복사/사용" :value="'COPY'"></v-radio>
          </v-radio-group>          
        </v-row>
        <v-row no-gutters class="pB-10" v-if="false">
          <i-select
              class="mr-1"
