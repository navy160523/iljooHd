...
const strTm = ref('08:00')
const enTm = ref('17:00')
const parmNm = ref('')
...
onMounted(() => {
  vm.$nextTick(() => {
    initCodeList()
    grdMain2.value.getDataProvider().setOptions({softDeleting: false})
  })
})
...
const grdMainProps2 = reactive({
    ...
    },
  {
    ...
    },
    {
      fieldName: "GATE_CD",
      lookupDisplay: true,
      editor: { type: 'list', dropDownWhenClick: true, textReadOnly: true }, 
      dataType: "text",
      header: { text: "출입문" },
      visible: true,
      editable: true,
    ...
    {
      fieldName: "CAR_SAVE_DESC",
      dataType: "text",
      header: { text: "CAR_SAVE_DESC" },
      visible: false,
      editable: false,
    },
    
    {
      fieldName: "CAR_SAVE_FLAG",
      dataType: "text",
      header: { text: "CAR_SAVE_FLAG" },
      visible: false,
      editable: false,
    },
...
const initCodeList = () => {
  ...
  getCodeList('HHIE240'),
    ...
    grdMain2.value.setBindingColumn("GATE_CD", res[0].ORESULT_CUR, "COD", "TXT") // 회사구분
...
const addPerson = () => {
  ...
  let subGrdDataRow = grdMain2.value.getDataProvider().getJsonRows(0, -1, true);
  console.log('subGrdDataRow222  ', subGrdDataRow)
  //삭제 데이터 제거 후 저장
  subGrdDataRow = subGrdDataRow.filter((x) => x.__rowState !== "deleted")
  ...
  for (let rowNum of chekedRow) {
    ...
    if (data.SAVE_FLAG === 'N') {
      ...
      console.log('data.RETURN_YN ', data.RETURN_YN)

      return Message.warn(t(data.SAVE_DESC))
    ...
    if (chkVal) {
      ...
      return Message.warn(t('이미 등록된 인원입니다.'))
    }
  }
...
const removePerson = () => {
  ...
  grdMain2.value.getDataProvider().removeRows(chekedRow,true)
  console.log('grdMain2.value.getDataProvider().getJsonRow(0,-1) ', grdMain2.value.getDataProvider().getJsonRows (0,-1,true))
...
const getListData = () => {
  ...
  const chkRow = grdMain2.value.getDataProvider().getJsonRows(0, -1) 
  ...
  for (let i = 0; i < chkRow.length; i++){
    ...
    console.log('data ', chkRow[i])

    if (chkRow[i].INOUT_FROM !== null) {
      chkRow[i].INOUT_FROM = chkRow[i].INOUT_FROM.replaceAll(':', '')
    }
    
    if (chkRow[i].INOUT_TO !== null) {
      chkRow[i].INOUT_TO = chkRow[i].INOUT_TO.replaceAll(':','')
    }

    if (isEmpty(chkRow[i].INOUT_TO) || isEmpty(chkRow[i].INOUT_FROM) || isEmpty(chkRow[i].GATE_CD)) {
      Message.warn('출입 시간 또는 출입문이 입력되지 않았습니다.')
      return resultData = []
    }

    resultData.push(chkRow[i])
  ...
  //   if (isEmpty(data.INOUT_TO) || isEmpty(data.INOUT_FROM) || isEmpty(data.GATE_CD)) {
  //     Message.warn('출입 시간 또는 출입문이 입력되지 않았습니다.')
  //     return resultData = []
  //   }
...
const onSubGrdCellClicked = (grid, clickData) => { 
  ...
  console.log('clickData.itemIndex ', clickData.itemIndex)
  ...
  if ((clickData.column === 'PIC_YN' && data2['PIC_YN2'] === 'N') || (clickData.column === 'PIC_YN2' && data2['PIC_YN'] === 'N')) {
    ...
    const subGridData = grdMain2.value.getDataProvider().getJsonRows(0, -1)
    
    //현재 리스트에 있는 선택된 컬럼에 해당하는 부분 전부 false 로 초기화 함,
    for (let i = 0; i < subGridData.legnth; i++){
      grdMain2.value.getDataProvider().setValue(i, clickData.column, 'N')
    }
...
if (grdMain2.value.getRowData(index.dataRow).CAR_SAVE_FLAG === 'N') {
  
  Message.err(t(grdMain2.value.getRowData(index.dataRow).CAR_SAVE_DESC))
  grdMain2.value.getGridView().cancel()
}
}
...
<template>
  <div class="fill-height">
    ...
    <i-card class="d-flex w-100 align-center pa-0 card-arrow" style="justify-content: center; background: #ffffff !important">
      <div class="d-flex">
        <div>
            <v-btn @click="addPerson" color="primary" style="width: 70px;">
              <svg-icon name="arrowRight" style="transform: rotate(90deg); width:23px; height: 23px">
              </svg-icon>
              
              <!-- {{ t('OUTBA0020Sub03.출입인원추가') }} -->
            </v-btn>
          </div>
          <div>
            <v-btn @click="removePerson" color="primary" style="width: 70px;">
              <svg-icon name="arrowRight" style="transform: rotate(-90deg); width:23px; height: 11px"></svg-icon>
              
              <!-- {{ t('OUTBA0020Sub03.출입인원제외') }} -->
            </v-btn>
          </div>
        </div>
    </i-card>
