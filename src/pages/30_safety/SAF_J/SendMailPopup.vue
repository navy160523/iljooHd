...
import AddressMailPopup from '@/pages/30_safety/SAF_J/AddressMailPopup.vue'
...
const addressMailPopup = ref(null)
...
const openPopupReferer = () => {
  empPopupReferer.value.openPopup({ MULTI: true, readonly: true })
}

// 주소록에서(받는사람)
const onEmpSelectedToAddress = (row) => {
  console.log('row :: ', row)
  for (let i of row) {
    console.log(' i :: ', i)
    if (i.EMAIL && i.EMAIL !== '') {
      // 사람
      chipArr.TO_EMP.push({ EMP_NM: i.KOR_NM, EMAIL: i.EMAIL })
      ...
      Message.warn(t(`${i.KOR_NM} 님이 이메일주소가 없습니다.`))
    }
  }
}
...
const onButtonsClick = (btn) => {
  if (btn.id === 'btnAddressMail') {
    //주소록 버튼
    addressMailPopup.value.openPopup()
...
<template>
      ...
      <VRow class="mt-3">
        <VCol>
          <IGridTitle
            ...
            :button-list="['btnAddressMail','btnSendMail', 'btnClose']"
  ...
  <AddressMailPopup ref="addressMailPopup" @selected="onEmpSelectedToAddress"/>
