<!--
작성자 :SI3팀 김현재
화면명 : 안전수칙위반 Dashboard (일렉용) 2024.11.08
...
const onButtonsClick = (btn) => {
  if (btn.id === 'btnSearch') {
    ...
    Promise.all([
      //수칙위반 합계
      commonSearchApi({
        queryId: 'SAFDA010_SEARCH01_ELEC',
        param: searchParam,
      }),
      //소속별 수칙위반 합계
      commonSearchApi({
        queryId: 'SAFDA010_SEARCH02_ELEC',
        param: searchParam,
      }),
      //수칙위반 시간대별 현황 조회
      commonSearchApi({
        queryId: 'SAFDA010_SEARCH03_ELEC',
        param: searchParam,
      }),
      //안전수칙위반 건수 조회
      commonSearchApi({
        queryId: 'SAFDA010_SEARCH04_ELEC',
        param: searchParam,
      }),
    ]).then((res) => {
      ioDivLength = 0
...
const selectedVioMdiv = (e) => {
  ...
  if (ioDivLength > e.length) {
    ...
    if (e.length > 5) {
      ...
      commonSearchApi({
        queryId: 'SAFDA010_SEARCH05_ELEC',
        param: param,
      }).then((res) => {
  ...
  if (vioColorD.style.background === 'rgb(26, 64, 199)') {
    ...
    if (searchParam.VIO_GDIV === '') {
      ...
      Promise.all([
        ...
        //수칙위반건수 조회
        commonSearchApi({
          queryId: 'SAFDA010_SEARCH06_ELEC',
          param: param,
        }),
      ]).then((res) => {
        //소속별 위반현황 초기화
