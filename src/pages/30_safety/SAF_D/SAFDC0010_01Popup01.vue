...
const safetyImageUpload = ref(null) //위반사항 사진첨부
const safetyFileUpload = ref(null) //위반사항 파일첨부
const actionImageUpload = ref(null) //조치사항 사진첨부
const actionFileUpload = ref(null) //조치사항 파일첨부
const safetyDansokEmpPopup = ref(null) //단속자성명
const safetyActEmpPopup = ref(null) //조치자성명
const safetyAppEmpPopup = ref(null) //승인자성명
const isAppSameAsDansok = ref(false) //승인자동일 체크박스
...
const safetyField = reactive({
  ...
  DANSOK_JOB_TIT_NM: userStore.jobTitNm || '', // 단속자직위
  ACT_EMP_NM: '',                           // 조치자성명
  ACT_EMP_NO: '',                           // 조치자사번
  ACT_JOB_TIT_NM: '',                       // 조치자직위
  ACT_ASGN_NM: '',                          // 조치자소속
  ACT_TEL_NO: '',                           // 조치자전화번호
  ACT_RSLT: '',                             // 조치결과
  ACT_DIV: '조치필요',                       // 조치구분
  ...
  vm.$nextTick(() => {
    if (safetyImageUpload.value) {
      if (!safetyField.FILE_ID1) {
        ...
        safetyField.FILE_ID1 = safetyImageUpload.value.guid
      } else {
        safetyImageUpload.value.setGuid(safetyField.FILE_ID1)
    ...
    if (actionImageUpload.value) {
      if (!safetyField.FILE_ID2) {
        ...
        safetyField.FILE_ID2 = actionImageUpload.value.guid
      } else {
        actionImageUpload.value.setGuid(safetyField.FILE_ID2)
    ...
    if (actionFileUpload.value) {
      if (!safetyField.FILE_ID2) {
        ...
        safetyField.FILE_ID2 = actionFileUpload.value.guid
      } else {
        actionFileUpload.value.setGuid(safetyField.FILE_ID2)
...
const openPopup = () => {
  if(isEmpty(safetyField.VIO_NO)){
    ...
    safetyField.DANSOK_JOB_TIT_NM = userStore.jobTitNm || ''
    ...
    safetyField.ACT_EMP_NM = ''
    safetyField.ACT_EMP_NO = ''
    safetyField.ACT_JOB_TIT_NM = ''
    safetyField.ACT_ASGN_NM = ''
    safetyField.ACT_TEL_NO = ''
    safetyField.ACT_RSLT = ''
    safetyField.ACT_DIV = '조치필요'
    safetyField.APP_EMP_NM = ''
    safetyField.APP_EMP_NO = ''
    isAppSameAsDansok.value = false
...
const openPopup2 = (rowData) => {
  ...
  Promise.all([
    ...
    const appEmp = codeList.APP_EMP_NO.find(x => x.APP_EMP_NO === safetyField.APP_EMP_NO)
    if (appEmp) {
      safetyField.APP_EMP_NM = appEmp.APP_EMP_NM
    }
    
    isAppSameAsDansok.value = (safetyField.APP_EMP_NO === safetyField.DANSOK_EMP_NO && safetyField.APP_EMP_NO !== '')
  })
...
const saveSafetyData = () => {
  ...
  let saveData = {
    ...
    ACT_EMP_NM: safetyField.ACT_EMP_NM, // 조치자 성명
    ACT_EMP_NO: safetyField.ACT_EMP_NO, // 조치자 사번
    ACT_JOB_TIT_NM: safetyField.ACT_JOB_TIT_NM, // 조치자 직위
    ACT_ASGN_NM: safetyField.ACT_ASGN_NM, // 조치자 소속
    ACT_TEL_NO: safetyField.ACT_TEL_NO, // 조치자 전화번호
    ACT_RSLT: safetyField.ACT_RSLT, // 조치 결과
    ACT_DIV: safetyField.ACT_DIV, // 조치 구분
...
const afterSave = (res) => {
  ...
  if (!safetyField.VIO_NO) {
    ...
    approvalReadOnly.value = false //승인신청 활성화
    safetyField.VIO_NO = res.list[0].OUT_RES_VIO_NO
  }
  // closePopup()
}
//안전수칙위반 저장 관련 로직 끝
...
const selectedSafetyActEmp = (val) => {
  safetyField.ACT_EMP_NM = val.EMP_NM
  safetyField.ACT_EMP_NO = val.EMP_NO
  safetyField.ACT_JOB_TIT_NM = val.JOB_TIT_NM
  safetyField.ACT_ASGN_NM = val.ASGN_NM
  safetyField.ACT_TEL_NO = val.TEL_NO || val.HND_PHN
}

const openSafetyAppEmpPopup = () => {
  if (isAppSameAsDansok.value) return
  safetyAppEmpPopup.value.openPopup({
    CMPNY_DIV: userStore.cmpnyDiv,
    HSE_ONLY: 'Y',
    EMP_NM: safetyField.APP_EMP_NM,
    readonly: true,
  })
}

const selectedAppEmp = (val) => {
  safetyField.APP_EMP_NM = val.EMP_NM
  safetyField.APP_EMP_NO = val.EMP_NO
}

watch(isAppSameAsDansok, (newValue) => {
  if (newValue) {
    safetyField.APP_EMP_NM = safetyField.DANSOK_EMP_NM
    safetyField.APP_EMP_NO = safetyField.DANSOK_EMP_NO
  } else {
    safetyField.APP_EMP_NM = ''
    safetyField.APP_EMP_NO = ''
  }
          ...
          <v-sheet class="searchArea">
            ...
            <div class="d-flex mt-2 mb-4">
              ...
              <i-input
                ...
                readonly
                @click:appendInner="openSafetyLocationPopup"
                required
              ></i-input>
              <i-input
            ...
            <div class="d-flex mt-2 mb-2">
              <i-input
                ...
                @keydown.enter="openSafetyEmpPopup('인원조회')"
              ></i-input>
              <i-input
            ...
            <div class="d-flex mb-4">
              ...
              <i-input
                ...
                width="250px"
              ></i-input>
              <span class="mt-6 ml-2 text-grey-darken-1" style="font-size: 13px;"
                >위반조직으로 검색되지 않는 경우에 수기입력하세요.</span
              >
            </div>
            ...
            <div class="d-flex mt-2">
              <i-input
                ...
                readonly
                append-inner-icon="mdi-magnify"
                @click:appendInner="openVioDivPopup"
            ...
            <!-- 위반종류가 '교통수칙'이 아닐 때만 아래 콘텐츠 노출 -->
            <div v-if="safetyField.VIO_GDIV_NM !== '교통수칙'">
              ...
              <div class="d-flex mt-2 mb-2">
                <i-input
                  v-model="safetyField.ACT_EMP_NM"
                  :label="$t('조치자성명')"
                  width="200px"
                  top-label
                  append-inner-icon="mdi-magnify"
                ...
                ></i-input>
                <i-input
                  v-model="safetyField.ACT_TEL_NO"
                  :label="$t('전화번호')"
                  width="200px"
                  top-label
                ></i-input>
              ...
              <div class="d-flex mt-2 mb-4 align-center flex-wrap">
                <i-input
                  ...
                  @keydown.enter="openDansokEmpPopup('단속자인원조회')"
                  required
                ></i-input>
                <i-input
                ...
                <div class="d-flex align-center mt-5 ml-4">
                  ...
                  <i-input
                    ...
                    width="200px"
                    append-inner-icon="mdi-magnify"
                    :readonly="isAppSameAsDansok"
                    @click:appendInner="openSafetyAppEmpPopup"
                    hide-details
                  ></i-input>
                </div>

                <v-btn class="mt-5 ml-4" @click="approval" :disabled="approvalReadOnly"
                  >승인신청</v-btn
                >
                <v-btn
                  class="mt-5 ml-2"
            ...
            <!-- 교통수칙일 때 나중에 추가할 화면 (플레이스홀더) -->
            <div v-else class="pa-4 text-center">
              <v-alert type="info" variant="tonal" class="my-5">
                교통 수칙일 때는 나중에 추가할 화면(도면/추가 이미지 등)이 노출될 예정입니다.
              </v-alert>
            </div>
