    ...
    { 
      ...
      renderer: {
        ...
        callback: function (grid, cell, w, h) {
          ...
          return `<div style="display: flex; justify-content: space-between;">
            <p style="width: 100px; overflow-x: auto;">${data}</p>
            </div>`
        },
      },
    },
...
// props 감지
watch(
  () => props.business02, newValue => {
    console.log('[watch02] : ', newValue)
    searchParams.BSNS_CD = props.business02
  },
)
...
const searchData = () =>{
  ...
  return commonSearchApi({ queryId : 'SAFIB0010_SEARCH_MAIN02', param: searchParams })
}
...
//컬럼 가로 병합(소계)
const setStyleGrid = () => {
  grdMain.value.getGridView().layoutByColumn("BSNS_NM").spanCallback = function (grid, layout, itemIndex) {
...
const setCellStyleGrid = () => {
  grdMain.value.getGridView().setRowStyleCallback(function (grid, item, fixed) {
    let ret = {}
    let GUBUN = grid.getValue(item.index, "GUBUN")

    if (!isEmpty(GUBUN) && GUBUN.includes("담당소계")) {
      ret.style = { background: "#FAED7D" }
    }
    
    if (!isEmpty(GUBUN) && GUBUN.includes("부문소계")) {
      ret.style = { background: "#FFC19E" }
    }
