...
const openPopup = (popupParam, page, EMAILDTE) => {
  ...
  searchParams.START_DATE = popupParam.START_DATE
  searchParams.END_DATE = popupParam.END_DATE
  // ------------------------------------------------------------------------
  /* 작성자 : 이희원
   * 작성일 : 2024-01-25
...
watch(
  ...
  (newValue) => {
    if (!newValue) {
      ...
      commonSearchApi({
        queryId: "searchDept3",
        param: { CMPNY_DIV: 'HHI', BSNS_CD: newValue },
      }).then((res) => {
        searchParams.DEPT_CD = ""
...
const searchReceive = () => {
  ...
  if(isEmpty(searchParams.BSNS_CD)) return
  commonSearchApi({
    queryId: "SAFIE0010_TAB01_SEARCH_03",
    param: searchParams,
  }).then((res) => {
    let nullEmail = ''
    for(var result of res.ORESULT_CUR) {
      if(isEmpty(result.EMAIL)) {
        nullEmail += result.EMP_NM + ','
      } else {
        chipArr.TO_EMP.push(result)
        mailParams.EMAIL.push(result.EMAIL)
      }
    }

    if(!isEmpty(nullEmail)) {
      Message.warn(t(nullEmail + '의 메일 주소가 존재하지 않습니다.'))
    }
  })
}
      ...
      .finally(() => {
        if (empList[0].DEPT_CD.length == 4) {
          return commonSearchApi({
            ...
            console.log('이거는?', res.ORESULT_CUR)
...
const SendMailSAFAG0010 = (popupParam, EMAILDTE) => {
  ...
  if (EMAILDTE == 'EMAILDTE') {
    if (popupParam.SANGBN == 'N') {
      console.log('산재구분 난청일때')
      dialog.value = true
      mailParams.SUBJECT = `안녕하세요. ${popupParam.BSNS_NM} ${popupParam.DEPT_NM} ${popupParam.KOR_NM} 난청 결과 통지서(${mail.HLD_OFFI_GBN}) 입니다.`
      mailParams.CONTENT = `${popupParam.BSNS_NM} ${popupParam.DEPT_NM} ${popupParam.KOR_NM}(${popupParam.EMPLNO}) (${popupParam.JOB_TIT_NM}) 난청 ${mail.STATUS} 되었습니다.
                          <br>
                          <h4>재해일자 : ${popupParam.OCCURDATE}<br></h4>
      ...
      editorOptions.data = mailParams.CONTENT
      console.log('메일파람', mailParams)
    } else {
      console.log('난청이 아닐때')
      dialog.value = true
      mailParams.SUBJECT = `(산재-${mail.YOGBN}) ${popupParam.BSNS_NM} ${popupParam.DEPT_NM} ${popupParam.KOR_NM} 산재결과 통지서(${mail.HLD_OFFI_GBN})`
      mailParams.CONTENT = `${popupParam.BSNS_NM} ${popupParam.DEPT_NM} ${
        popupParam.KOR_NM
      } (${popupParam.EMPLNO}) (${popupParam.JOB_TIT_NM})  ${mail.YOGBN} ${
        mail.STATUS
      } 되었습니다.<br>
                           
                           <h4>재해일자 : ${popupParam.OCCURDATE}<h4>
                            <h4>${
                              popupParam.STATUS === '승인'
                                ? '승인결과 안내 드립니다. <br> 인사 담당자께서는 휴직일자 확인 후 회신 부탁드립니다.<br> 좋은 하루 되세요.'
                                : '좋은 하루 되세요.'
                            }</h4>
                          <h4>최 수 진</h4>
                          <h4>경영지원본부 법무지원팀 / 사원</h4>
                          `
      editorOptions.data = mailParams.CONTENT
      console.log('메일파람', mailParams)
    }
    ...
    mailParams.CONTENT = `안녕하세요
                        <br> ${popupParam.BSNS_NM} ${popupParam.DEPT_NM} ${popupParam.KOR_NM}(${popupParam.EMPLNO}) (${popupParam.JOB_TIT_NM})님 산재요양 종결 안내드립니다.
                        <br>
                        <h4> - 재해일자 : ${popupParam.OCCURDATE}</h4>
                        <h4> - 종결일자 : ${popupParam.ENDDTE}</h4>
                        <br>
                        <h4>산재요양 종결 안내 드립니다.</h4>
                        <h4>좋은 하루 되세요.</h4>
                        <br>
                        <h4>최 수 진</h4>
                        <h4>경영지원본부 법무지원팀 / 사원</h4>
