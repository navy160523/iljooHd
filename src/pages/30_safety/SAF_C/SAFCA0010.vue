...
const searchData = () => {
  ...
  return Promise.all([
    //일상점검 좌측 전체,양호,미흡 조회
    commonSearchApi({
      queryId: 'SAFCA0010_SEARCH01',
      param: searchParam,
    }),
    //일상점검 우측 라인차트 조회
    commonSearchApi({
      queryId: 'SAFCA0010_SEARCH02',
      param: searchParam,
    }),
    //부서관리작업점검 좌측 바차트 계획건수 조회
    commonSearchApi({
      queryId: 'SAFCA0010_SEARCH03',
      param: searchParam,
    }),
    //부서관리작업 실적,미흡 건수 조회
    commonSearchApi({
      queryId: 'SAFCA0010_SEARCH04',
      param: searchParam,
    }),
    // 부서관리작업 월별 실적,미흡(건)
    commonSearchApi({
      queryId: 'SAFCA0010_SEARCH05',
      param: searchParam,
    }),
    //PTW 계획건수 조회
    commonSearchApi({
      queryId: 'SAFCA0010_SEARCH06',
      param: searchParam,
    }),
    //PTW 실적,미흡 건수 조회
    commonSearchApi({
      queryId: 'SAFCA0010_SEARCH07',
      param: searchParam,
    }),
    //PTW 안전현장,안전대면,전산 계획건수 조회
    commonSearchApi({
      queryId: 'SAFCA0010_SEARCH08',
      param: searchParam,
    }),
    //PTW 안전현장,안전대면,전산 실적,미흡건수 조회
    commonSearchApi({
      queryId: 'SAFCA0010_SEARCH09',
      param: searchParam,
    }),
    //PTW 월별 실적,미흡건수 조회
    commonSearchApi({
      queryId: 'SAFCA0010_SEARCH10',
      param: searchParam,
    }),
  ])
}
...
const defaultDate = () => {
  ...
  searchParam.CHECK_DATE_FROM =
    dayjs().$y + '-' + dayjs().$M.toString().padStart(2, '0')
                    ...
                    <v-sheet width="70%" height="100%">
                      <!-- <div> -->
                        <table style="height: 100%;">
                          ...
                          <tr style="height: 33%;">
                            <td class="searchArea">3중 점검대상</td>
                          ...
                          <tr style="height: 34%;">
                            <td class="searchArea">3중점검 미대상</td>
