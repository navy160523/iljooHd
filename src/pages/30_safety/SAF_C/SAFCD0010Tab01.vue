...
import SAFCD0010Tab01Popup01 from './SAFCD0010Tab01Popup01.vue'
...
import OZReportBig from '@/components/OZReportBig.vue'
...
const grdMainProps = reactive({
  ...
  fields: [
    ...
    },
    {
      ...
      width: '70',
      dataType: 'text',
      visible: userStore.authGrpCd.includes('GRP00381') || userStore.authGrpCd.includes('GRP00369') ? true : false,
      editable: false,
    ...
    },
    {
      ...
      dataType: 'text',
      visible: false,
      header: { text: t('잠재사고유형(대)CD') },
    },
    {
      fieldName: 'SAGO_DIV_M',
      dataType: 'text',
      visible: false,
      header: { text: t('잠재사고유형(중)CD') },
    },
    {
      fieldName: 'SAGO_DIV_S',
      dataType: 'text',
      visible: false,
      header: { text: t('잠재사고유형(소)CD') },
    },
    {
  ...
  'SEND_MAIL_YN',
  {
    ...
    items: ['SAGO_DIV_L_NM', 'SAGO_DIV_M_NM'],
    header: {
      text: t("잠재사고유형"),
    },
  },

  {
    name: "점검대상물",
    direction: "horizontal",
    items: [ 'CHECK_TARGET_OBJ1_NM',  'CHECK_TARGET_OBJ2_NM', 'CHECK_TARGET_OBJ3_NM'],
    header: {
      text: t("점검대상물"),
    },
  },
...
const setSliParams = () => {
  if(sliSAFCD0010Tab01 !== undefined) {
    ...
    if(sliSAFCD0010Tab01.SAFE_YN === 'Y') {
      searchParam.DEPT_CD = ''
      searchParam.DEPT_NM = ''
      searchParam.RECEIVER_DEPT_CD = sliSAFCD0010Tab01.DEPT_CD
      searchParam.RECEIVER_DEPT_NM = sliSAFCD0010Tab01.DEPT_NM
    } else {
      searchParam.DEPT_CD = sliSAFCD0010Tab01.DEPT_CD
      searchParam.DEPT_NM = sliSAFCD0010Tab01.DEPT_NM
...
const onButtonsClick = btn => {
  if (btn.id === 'btnSearch') {
    ...
    checkedRows.forEach((item) => {
      ...
      console.log('삭제 점검상세 : ', data)
      console.log('권한그룹 : ', userStore.authGrpCd)

      if (userStore.authGrpCd.includes('GRP00381') || userStore.authGrpCd.includes('GRP00369') 
      || data.INSERT_USER_ID === userStore.userId || data.CHECK_EMP_NO === userStore.userId){
        result = true
      } else {
        Message.warn(t('작성자 또는 점검자만 삭제 가능합니다.'))
        result = false
      }
    })
...
const ozPrint = () => {
  ...
  for (let i = 0; i < chkNum.length; i++) {
    ...
    if (data.CHECK_GUBUN == 0) {
      ...
      reportName.value = '/manage/hse/SAFCD0010_1.ozr'
    } else {
      //우수사례
      param = [
        'IN_KEY=' +
      ...
      reportName.value = '/manage/hse/SAFCD0010_2.ozr'
    }
    reportParam.value.push({ reportName: reportName.value, args: param })
    isMulti.value = true
...
<template>
      ...
      <div class="d-flex flex-column fill-height">
        <v-sheet class="searchArea">
          <div class="d-flex">
            ...
            <i-input
              v-model="searchParam.CHECK_EMP_NO"
              width="100px"
              readonly
            />
            <i-input
          ...
          <div class="d-flex mt-1">
            ...
            <p class="ml-4" style="color: #222;margin-top:6px">{{ t('등록구분') }}</p>
            <VRadioGroup class="ml-2" style="margin-top:2px;" v-model="searchParam.CHANNEL" inline>
              <VRadio
  ...
  />
  <SAFCD0010Tab01Image ref="sAFCD0010Tab01Image" />
  <OZReport
    :showPop="showOz"
