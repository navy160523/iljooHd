...
const vioApprovalEmpPopup = ref(null)     // 수칙위반등록 승인자 인원팝업
const safetyDansokEmpPopup = ref(null)    // 수칙위반등록 단속자 인원팝업
...
const disableEdit = ref(false)            // 조치자 수정불가 처리
...
const divVio = reactive({
  ...
  VIO_EMP_NO: '',     //위반자성명
  DANSOK_EMP_NM: '',  // 단속자성명
  DANSOK_ASGN_CD: '', // 단속자 조직코드
  DANSOK_ASGN_NM: '', // 단속자조직이름
  DANSOK_DEPT_CD: '', // 단속자 부서코드
  DANSOK_EMP_NO: '',  // 단속자사번
  ASGN_CD: '',        //위반자조직
  BSNS_CD: '',
  APP_EMP_NO: '',     //승인자사번
  ...
  Promise.all([
    ...
    commonSearchApi({
      ...
      param: {
        ...
        DANSOK_EMP_NO: userStore.userId,
      },
    }),
  ]).then((res) => {
    ...
    codeList.NOTI_APP_EMP_NO = res[4].ORESULT_CUR
...
const dailyModify = (rowData) => {
  ...
  if (rowData.MODIFY) {
    vm.$nextTick(() => {
      ...
      // 저장버튼 비활성화 (작성자, 점검자 또는 안전직원이 아닌 경우 조치내용 입력 외 비활성화)
      if (userStore.authGrpCd.includes('GRP00381') || userStore.authGrpCd.includes('GRP00369') 
      || rowData.INSERT_USER_ID === userStore.userId || rowData.CHECK_EMP_NO === userStore.userId){
        disableEdit.value = false
      } else {
        disableEdit.value = true
      }
      console.log('rowData : ', rowData)
      console.log('Edit 상태 : ', disableEdit.value)
      ...
      if (dailyCheckField.JSTOP_NO) {
        ...
        commonSearchApi({
          ...
          workStopField.WORKER_CNT = res.ORESULT_CUR[0].WORKER_CNT      // 작업인원
        })
      }
      //수칙위반 번호가 있으면 수칙위반 데이터 조회
        ...
        commonSearchApi({
          ...
          divVio.DANSOK_EMP_NM = res.ORESULT_CUR[0].DANSOK_EMP_NM // 위반자이름
          divVio.DANSOK_EMP_NO = res.ORESULT_CUR[0].DANSOK_EMP_NO // 위반자사번
...
const beforeSave = () => {
  ...
  if (today.diff(dayjs(dailyCheckField.CHECK_DATE), 'day') >  14 && dailyCheckField.MODIFY === 'N'){
      Message.warn(t('2주일 이전의 점검은 등록이 불가능합니다.'))
      return false
  }
  
  // 미래 일자 저장 불가 처리(키보드로 입력한 경우) 
  if (dayjs(getToday()).diff(dayjs(dailyCheckField.CHECK_DATE), 'day') < 0 && dailyCheckField.MODIFY === 'N'){
    Message.warn(t('점검일자가 미래 일자인 경우 등록이 불가능합니다.'))
    return false
  }
  ...
  if (radioBtnValue.CHECKED === 'VIO_DIV') {
    ...
    if (!divVio.VIO_DIV) {
      ...
      return false
    } else if (!divVio.DANSOK_EMP_NO) {
      Message.warn(t('단속자는 필수입력값 입니다.'))
      return false
    }
...
const saveData = () => {
  ...
  let saveData = {
    ...
    SHIP_NO: dailyCheckField.SHIP_NO,   //호선
    ...
    RECEIVER_BSNS_CD: dailyCheckField.RECEIVER_BSNS_CD,       // 대상조직 사업부 코드
    RECEIVER_DEPT_CD: dailyCheckField.RECEIVER_DEPT_CD,       // 대상조직 부서 코드
    RECEIVER_ASGN_CD: dailyCheckField.RECEIVER_ASGN_CD,       // 대상조직 소속 코드
    ...
    WORKER_CNT: workStopField.WORKER_CNT, //작업인원
    ...
    VIO_ASGN_CD : divVio.ASGN_CD, //위반자조직
    DANSOK_EMP_NO: divVio.DANSOK_EMP_NO, //단속자사번
    DANSOK_ASGN_CD: divVio.DANSOK_ASGN_CD, //단속자조직
    DANSOK_DEPT_CD: divVio.DANSOK_DEPT_CD, //단속자부서
    IO_DIV: divVio.IO_DIV, //위반자 대상구분
    USER_ID: userStore.userId, //로그인유저 아이디
    CHANNEL: 'W', //접속경로
  }
  saveParam.push(saveData)
  return commonExecuteApi({
    queryId: 'SAFCD0010_SAVE01',
    list: saveParam,
  })
}
...
const selectedDept = (val) => {
  ...
  dailyCheckField.RECEIVER_DEPT_NM = val.ASGN_FULL_NM.replace(val.BSNS_NM + '-','')
...
//수칙위반 등록 위반자 선택 이벤트
const selectedVioEmp = async (val) => {
  divVio.VIO_EMP_NM = val.EMP_NM
  divVio.VIO_EMP_NO = val.EMP_NO
  divVio.IO_DIV = val.USER_DIV //대상구분(직영,협력사)
  divVio.ASGN_CD = val.ASGN_CD //위반자 조직
  divVio.BSNS_CD = val.BSNS_CD

  
  divVio.VIO_DIV = ''
  divVio.VIO_MDIV = ''

  const res = await getCodeList('HHIF110')
  codeList.VIO_GDIV = res.ORESULT_CUR.filter((x) => !x.COD.includes('S') && !x.COD.includes('C'))
}

//수칙위반 등록 단속자 인원팝업 오픈 이벤트
...
const selectedDansokEmp = (val) => {
  divVio.DANSOK_EMP_NM = val.EMP_NM
  divVio.DANSOK_EMP_NO = val.EMP_NO
  divVio.DANSOK_ASGN_CD = val.ASGN_CD
  divVio.DANSOK_DEPT_CD = val.DEPT_CD
}
...
const initField = () => {
  ...
  //점검사진 개수 초기화
  fileUpload1.value.resetCntVal()
  fileUpload2.value.resetCntVal()
...
const setHDPAY = (res1) => {
  ...
  Promise.all([
    commonSearchApi({
      queryId: 'SAFCD0010_SEARCH82',
      param: inCurrentMonthHdPayCountParam,
    }),
    commonSearchApi({
      queryId: 'SAFCD0010_SEARCH83',
...
const checkVioEmp = (event) => {
  if (!divVio.VIO_EMP_NM) {
    event.preventDefault(); // 드롭다운 열리는 것 방지
    divVio.VIO_GDIV = ''
    Message.warn(t('위반자를 먼저 선택해 주세요.'))
    return false
  }
}
...
watch(
  ...
  (newValue, oldValue) => {
    ...
    commonSearchApi({
      ...
      param: {
        CMPNY_DIV: userStore.cmpnyDiv,
        BSNS_CD: divVio.BSNS_CD,
      },
    })
  ]).then((res) => {
      //codeList.VIO_GDIV = res[0].ORESULT_CUR.filter((x) => !x.COD.includes('S') && !x.COD.includes('C'))
      codeList.VIO_GDIV_NM = res[1].ORESULT_CUR.filter((x) => x.UP_COD.includes(newValue))
    })
...
//승인자 추가시 해당 기능 오픈
// // 단속자 변경시
// watch(() => divVio.DANSOK_EMP_NO, (newValue, oldValue) => {
//   commonSearchApi({
//     queryId: 'SAFDC0010_SEARCH_04',
//     param: {
//       CMPNY_DIV: 'HHI',
  ...
  <v-card class="pa-0 fill-height rounded-b-5">
    ...
    <v-card-text class="pa-3 pb-0">
      <v-sheet class="searchArea">
        <div>
          <div class="sheetTitle" style="display: inline-block;">
            점검정보
          </div>
          <div style="display: inline-block; margin-top: 12px; margin-left: 16px; color: blue; width: 560px;">
            ※ 일상점검 등록 후에는 점검일시 수정이 불가하니, 저장 전 정확히 확인해 주시기 바랍니다.
          </div>
          <div style="display: inline-block; margin-top: 12px; margin-left: 16px; color: blue">
            ※ 불안전사례 조치 시, 조치구분을 '조치완료'로 변경해 저장해 주세요. 
          </div>
        </div>
        <div class="d-flex mt-2">
          ...
          ></i-select>
          <i-select 
            v-model="dailyCheckField.SHIP_NO"
            :label="$t('호선/프로젝트 No.')"
            top-label
            width="200px"
            ...
            item-title="TXT"
            />
            <div style="margin-top: 12px; margin-left: 16px; color: blue"
            > ※ 호선/공사별 통계를 위해, <br>
               관련된 경우 No.를 기입해 주세요
            </div>
        </div>
        <div class="d-flex mt-2">
          ...
          <div class="">
            <v-radio-group 
              :label="$t('계획점검 연계')"              
              inline 
              v-model="radioBtnValue.CHECKED_DAILY_PLAN"
              :disabled = "disableEdit">
              <v-radio label="미연계" value="DAILY"></v-radio>
              <v-radio label="연계" value="PLAN"></v-radio>
            </v-radio-group>
          </div>
        ...
        <div class="mt-5">
          <div v-if="readOnlyValue.RADIO_BTN">
            <div class="sheetTitle" style="display: inline-block;">안전수칙위반</div>
            <div v-if="dailyCheckField.DAILY_CHECK_SEQ" style="display: inline-block; margin-top: 12px; margin-left: 16px; color: blue; width: 560px;">
              ※ 최초 저장시에만 수칙위반연계 가능합니다.
            </div>
            <v-radio-group v-model="radioBtnValue.CHECKED" inline class="mt-5" >
              ...
              ></v-radio>
              <v-radio
                label="수칙위반(The Safe Care/절대수칙/중대성수칙/일반수칙)"
                value="VIO_DIV"
        ...
        <div v-if="radioBtnValue.CHECKED === 'NOTI_CORR'">
          <div class="d-flex mt-4">
            ...
            <i-select
                ...
                width="200px"
                top-label
                :items="codeList.NOTI_APP_EMP_NO"
        ...
        <div v-if="radioBtnValue.CHECKED === 'VIO_DIV'">
          <div class="d-flex mt-4">
            ...
            <i-input
              ...
              @keydown.enter="openDansokEmpPopup"
              v-model="divVio.DANSOK_EMP_NM"
            ></i-input>
            <!-- <i-select
                v-model="divVio.APP_EMP_NO"
                :label="$t('승인자')"
                width="200px"
                top-label
