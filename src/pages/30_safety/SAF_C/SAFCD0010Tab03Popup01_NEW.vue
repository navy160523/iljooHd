...
const title = ref('고위험작업 점검결과 등록')
const emit = defineEmits(['closePopup'])
const readonlyAttach = ref(false)
...
const saveData = async() => {
  ...
  for (let i = 0; i < grdSubData.length; i++) {
    let saveDataSub = {
      ...
      RCV_NO: ptwField.RCV_NO, //허가서No
      CHECK_EMP_NO: grdMainData.CHECK_EMP_NO, //점검자 사번
      P_WRK_ID: (!ptwField.P_WRK_ID || (ptwField.P_WRK_ID.substring(0,1) === 'S')) ? 'P000000' : ptwField.P_WRK_ID, //PTW 대상 ID
...
const modifyGrdMain = () => {
  let param = {
    ...
    RCV_NO: ptwField.RCV_NO, //허가서No
    P_WRK_ID: (!ptwField.P_WRK_ID || (ptwField.P_WRK_ID.substring(0,1) === 'S')) // wrkid가 없거나, 단기공사일때 
          ? 'P000000'
          : ptwField.P_WRK_ID,
  }
  console.log('파람', param)
  commonSearchApi({
    queryId: 'SAFCD0010_SEARCH94',
    param: param,
  }).then((res) => {
...
const modifyGrdSub = () => {
  ...
  let param = {
    ...
    WORK_ID: ptwField.WORK_ID, //작업지시 ID
    RCV_NO: ptwField.RCV_NO, //허가서No
    P_WRK_ID: ((!ptwField.P_WRK_ID || (ptwField.P_WRK_ID.substring(0,1) === 'S'))
      ? 'P000000'
      : ptwField.P_WRK_ID), //PTW 대상 ID
    CHECK_EMP_NO: row.CHECK_EMP_NO,
    HISTORY_SEQ: row.HISTORY_SEQ
  }
  console.log('수정시 조회 점검결과 조회파람', param)
  commonSearchApi({
    queryId: 'SAFCD0010_SEARCH93',
    param: param,
  }).then((res) => {
    ...
    for (let i = 0; i < res.ORESULT_CUR.length; i++) {
      ...
      } else if (res.ORESULT_CUR[i].CHECK_RESULT === '0') {
        grdSub.value
          .getDataProvider()
...
const onGrdMainCurrRowChanged = (grid, oldIdx, newIdx) => {
  if (!title.value.includes('등록')) {
    ...
    let param = {
      ...
      P_WRK_ID:
          (!ptwField.P_WRK_ID || (ptwField.P_WRK_ID.substring(0,1) === 'S')) // wrkid가 없거나, 단기공사일때 
          ? 'P000000'
          : ptwField.P_WRK_ID,
      CHECK_EMP_NO: row.CHECK_EMP_NO,
      HISTORY_SEQ: row.HISTORY_SEQ
    ...
    commonSearchApi({
      ...
      if (res.ORESULT_CUR.length > 0) {
        ptwField.CHECK_DATE = res.ORESULT_CUR[0].CHECK_DATE
...
const checkMaxTime = () => {
  ...
  if (ptwField.CHECK_DATE === today) {
    ...
    //console.log('max ::::::', max)
    //console.log('ptwField.CHECK_TIME ::::::', ptwField.CHECK_TIME)
...
<template>
  ...
  >
    ...
    <IUploadPopup ref="fileUploadPopup" :read-only="readonlyAttach"></IUploadPopup>
  </v-dialog>
</template>
