...
const initCodeList = () => {
  Promise.all([
    ...
    commonSearchApi({
      ...
      param: { CMPNY_DIV: userStore.cmpnyDiv, BSNS_CD: userStore.bsnsCd, USE_DIV: 'Y' },
    }), // 부서
  ]).then((res) => {
...
const grdMainProps = reactive({
  ...
  field: [
    ...
    { fieldName: 'REGISTERED', dataType: 'text', visible: false },
  ],
  columns: [],
})
...
const openPopup = (popupParam) => {
  ...
  if (options.values.EMP_NM || options.values.BSNS_CD || options.values.DEPT_CD || options.values.EMP_NO) {
    ...
    //   }
    // })
    console.log('option.value',options.values)
    options.values.DEPT_CD = popupParam.DEPT_CD
    console.log('popupParam.DEPT_CD',popupParam.DEPT_CD)
    console.log('option.value.DEPT_CD',options.values.DEPT_CD)
    ...
    Promise.all([
        ...
        registered = res[1].ORESULT_CUR

        const allData = grdMain.value.getDataProvider().getJsonRows()
        allData.forEach((a,idx) => {
          registered.forEach(r => {
            if(a.EMP_NO === r.EMP_NO){
              grdMain.value.getDataProvider().setValue(idx,'REGISTERED','Y')
            }
        })
        })
...
const onCellDblClicked = (grid, clickData) => {
  ...
  if(selectData.REGISTERED === 'Y'){
    return Message.warn('이미 추가된 인원입니다.')
  }else{
    onSelect(clickData)
  }
  ...
  >
    ...
    <v-card class="pa-0 fill-height rounded-b-5">
      <v-card-title class="pa-4 pb-2">
        <IGridTitle :use-permission="false" :button-list="['btnSearch', 'btnSelect', 'btnClose']" @click-button="onButtonsClick" />
      </v-card-title>
          ...
          <v-sheet class="searchArea flex-column">
            <div class="d-flex">
              ...
              <i-label class="pl-5" label-loc="none" required>
                <template #editor>
                  <v-radio-group v-model="options.values.USER_DIV" inline :disabled="options.onlyDisabled.USER_DIV_D">
                    <v-radio v-for="item in codeList.USER_DIV" :key="item" :value="item.COD" :label="item.TXT" :disabled="item.DISABLE" />
                  </v-radio-group>
                </template>
              </i-label>
              <i-label>
