...
const emit = defineEmits(['close', 'upData', 'useBtn', 'img1', 'img2', 'img3'])
...
const CompleteSend = () => {  
  ...
  if (!Popdata.SAFE_GAMGR_EMPEMAIL) {
   ...
   if(isLATE.value){
    if(!LATECheck()){
      Message.warn(t('지연보고시 지연 / 미이행 사유는 필수입니다.'))
    }
   }
  vm.$swal({
    title: t(
  ...
  (newValue) => {
    if (Popdata.APPROVAL_STATUS === '종결') {
      ...
      vm.$nextTick(() => {
        ...
        // menuTitle.value.disableBtn('btnComplete', true)
        if(Popdata.SAGO_DIV_L ==='B' || userStore.empNo === sagoEmpNo || userStore.empNo === Popdata.SAFE_GAMGR_EMP){
          menuTitle.value.disableBtn('btnCancelComplete', false)
        }
        else{
          menuTitle.value.disableBtn('btnCancelComplete',true)
...
<v-card class="mb-3 mr-1 popUp-input-Form">
      ...
      v-if="userStore.empNo === sagoEmpNo"
      >
        <v-card-title class="pa-0">
          <IGridTitle
            ref="GridTitle8"
