...
const formatTimeFromInput = (inputTime) => {
  const inputTime2 = inputTime.replace(/:/gi, '')
  const timePattern = /^([01]?[0-9]|2[0-3])([0-5][0-9])$/
  if (!timePattern.test(inputTime2)) {
    return ''
  }

  const hours = inputTime2.substring(0, 2)
  const minutes = inputTime2.substring(2)
...
const mainCheck = () => {
  ...
  if (
    chkEmpty(popupParam.SaveData.SAGO_LOC_DTL)
  ) {
    Message.warn(t('상세장소를 입력해주세요.'))
    return false
  }
  if (
    chkEmpty(popupParam.SaveData.SAGO_GAEYO)
  ) {
    Message.warn(t('사고 내용을 입력해주세요.'))
    return false
  }
                ...
                <div class="d-flex my-2 box">
                  <div>
                    <div class="mb-1" style="position: relative;">
                      ...
                      <p style="font-size: 12px; position: absolute; top: 0; right: 0;">
                        * 별도로 책임부서/협력사가 지정되지 않은 경우, 재해자가<br/>&nbsp;&nbsp;&nbsp;소속된 부서/협력사를 책임부서로 하여 관리합니다.
                      </p>
                    </div>
                    <!-- <i-input
                      top-label
