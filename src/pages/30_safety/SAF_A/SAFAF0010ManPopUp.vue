...
const menuTitle = ref(null) //팝업 타이틀
const grdTitle = ref(null) //산업재해 조사표
const grdTitle2 = ref(null) //산재불인정 부서의견서
...
const disabledGridBtn = async () => {
  ...
  //산업재해 조사표
  grdTitle.value.disableBtn("btnPrintHandwriting", true)
  //파일첨부
  fileUpload.value.diableFileUploadButton(["btnFileUpload", "btnDownLoad", "btnDelete"])
  //산재불인정 부서 의견서
  grdTitle2.value.disableBtn("btnSearch", true)
  grdTitle2.value.disableBtn("btnPrint", true)
  grdTitle2.value.disableBtn("btnUpdate2", true)
  grdTitle2.value.disableBtn("btnDelete2", true)
}
      ...
      await commonSearchApi({
        ...
        if (res.ORESULT_CUR.length != 0) {
          ...
          if (res.ORESULT_CUR[0].OCCUR_DT) {
            res.ORESULT_CUR[0].OCCUR_DT = dayjs(res.ORESULT_CUR[0].OCCUR_DT).format("YYYY-MM-DD")
          }
...
//  파일관련 메소드
const checkFileId = () => {
  if (popupParam.SaveData.RPT_FILE_ID === null || popupParam.SaveData.RPT_FILE_ID === undefined) {
        ...
        <v-row style="overflow-x: hidden; height: 590px">
          ...
          <VRow class="popUp-input-Form">
            ...
            <v-col cols="12">
              <v-row>
                <VCol cols="12" md="4">
                  <ILabel :label="$t('사고구분')" labelLoc="top">
                    <template #editor="editorProps">
                      <v-row>
                        <v-col>
                          <v-select
                ...
                <VCol cols="12" md="4">
                  <ILabel :label="$t('산재구분')" labelLoc="top" :required="true">
                    <template #editor="editorProps">
                      <v-row>
                        <v-select
                          readonly
                ...
                <VCol cols="12" md="4" class="d-flex align-center">
                  <v-checkbox disabled label="개인진정" v-model="popupParam.SaveData.CONTACTING" />
                </VCol>
              </v-row>
            </v-col>
          </VRow>
          ...
          <VRow class="popUp-input-Form">
            <IUpload ref="fileUpload" :is-visibled="isVisibled" @uploaded="uploaded($event)" style="height: 300px" />
          </VRow>
          ...
          <v-col cols="12" class="my-3">
            <IGridTitle
              ref="grdTitle2"
              :title="$t('산재불인정 부서의견서')"
              :button-list="['btnSearch', 'btnPrint', 'btnUpdate2', 'btnDelete2']"
          ...
          <VRow class="popUp-input-Form">
            <v-col cols="12">
                    ...
                    <template #editor="editorProps">
                      <v-row>
                        <v-col cols="12">
                          <v-text-field
                            :placeholder="t('성명')"
                            v-model="popupParam.NRoIAData.NAME"
                            variant="solo"
                            readonly
                            flat
                          />
                ...
                <VCol cols="12" md="8">
                  <ILabel :label="$t('소속/조직')" labelLoc="top" :required="true">
                    <template #editor="editorProps">
                      <v-row>
                        <v-col cols="12">
                          <v-text-field readonly v-model="popupParam.NRoIAData.SOSOG_DESC" variant="solo" flat />
                        </v-col>
                      </v-row>
                    </template>
                  </ILabel>
              ...
              <v-row>
                <VCol cols="12" md="6">
                  <ILabel :label="$t('발생일시')" labelLoc="top" :required="true">
                    <template #editor="editorProps">
                      <v-row>
                        <v-col cols="7">
                          <v-text-field
                            v-model="popupParam.NRoIAData.OCCUR_DT"
                            type="date"
                            variant="solo"
                            flat
                            readonly
                        ...
                        <v-col cols="5">
                          <v-text-field
                            ...
                            v-model="popupParam.NRoIAData.OCCUR_TIME"
                            variant="solo"
                            flat
                          />
                        </v-col>
                ...
                <VCol cols="12" md="6">
                  <ILabel :label="$t('발생장소')" labelLoc="top" :required="true">
                    <template #editor="editorProps">
                      <v-row>
                        <v-col cols="4">
                          <v-text-field
                            :placeholder="t('대구분')"
                            v-model="popupParam.NRoIAData.SAGO_LOC_LM"
                            variant="solo"
                            flat
                            readonly
                            class="mr-2"
                        ...
                        <v-col cols="4">
                          <v-text-field
                            :placeholder="t('중구분')"
                            readonly
                            v-model="popupParam.NRoIAData.SAGO_LOC_MM"
                            variant="solo"
                            flat
                          />
                        </v-col>
                        <v-col cols="4">
                          <v-text-field
                            :placeholder="t('소구분')"
                            readonly
                            v-model="popupParam.NRoIAData.SAGO_LOC_SM"
                            variant="solo"
                            flat
                          />
                        </v-col>
              ...
              <v-row>
                <VCol cols="12" md="12">
                  <ILabel :label="$t('사고형태')" labelLoc="top" :required="true">
                    <template #editor="editorProps">
                      <v-row>
                        <v-col cols="3">
                          <v-text-field
                            v-model="popupParam.NRoIAData.SAGO_DIV_L_NM"
                            readonly
                            variant="solo"
                            flat
                            class="mr-2"
                        ...
                        <v-col cols="3">
                          <v-text-field
                            v-model="popupParam.NRoIAData.SAGO_DIV_M_NM"
                            readonly
                            variant="solo"
                            flat
                            class="mr-2"
                        ...
                        <v-col cols="6">
                          <v-text-field v-model="popupParam.NRoIAData.SAGO_DIV_S_NM" readonly variant="solo" flat />
                        </v-col>
                      </v-row>
                    </template>
                  </ILabel>
                ...
                <VCol cols="12" md="6">
                  <ILabel :label="$t('상해부위')" labelLoc="top" :required="true">
                    <template #editor="editorProps">
                      <v-row>
                        <v-col>
                          <v-text-field readonly v-model="popupParam.NRoIAData.INJURY_PART_LM" variant="solo" flat />
                        </v-col>
                        <v-col>
                          <v-text-field
                            disabled
                            ...
                            menu-icon=""
                            readonly
                            v-model="popupParam.NRoIAData.INJURY_PART_SM"
                            variant="solo"
                            flat
                          />
                        </v-col>
                      ...
                      <v-row>
                        <v-col>
                          <v-select
                            ...
                            bg-color="#EEEEEE"
                            menu-icon=""
                            v-model="popupParam.NRoIAData.INJURY_TYPEM"
                            :items="codeList.INJURY_TYPE"
                            item-value="COD"
                            item-title="TXT"
                            variant="solo"
              ...
              <v-row>
                <VCol cols="12" md="12">
                  <ILabel :label="$t('사고 개요')" labelLoc="top" :required="true">
                    <template #editor="editorProps">
                      <v-row>
                        <v-textarea
                          v-model="popupParam.NRoIAData.SAGO_GAEYO"
                          variant="solo"
                          flat
                          readonly
                          class="mt-2"
              ...
              <v-row>
                <VCol cols="12" md="12">
                  <ILabel :label="$t('산재요청 경위')" labelLoc="top" :required="true">
                    <template #editor="editorProps">
                      <v-row>
                        <v-textarea
                          readonly
              ...
              <v-row>
                <VCol cols="12" md="12">
                  <ILabel :label="$t('경과 및 현재상태')" labelLoc="top" :required="true">
                    <template #editor="editorProps">
                      <v-row>
                        <v-textarea readonly v-model="popupParam.NRoIAData.STATUS" variant="solo" flat class="mt-2" />
                      </v-row>
                    </template>
                  </ILabel>
                </VCol>
              ...
              <v-row>
                <VCol cols="12" md="12">
                  <ILabel :label="$t('부서의견')" labelLoc="top" :required="true">
                    <template #editor="editorProps">
                      <v-row>
                        <v-textarea readonly v-model="popupParam.NRoIAData.COMMENTS" variant="solo" flat class="mt-2" />
                      </v-row>
                    </template>
                  </ILabel>
                </VCol>
              ...
              <v-row>
                <VCol cols="12" md="12">
                  <ILabel :label="$t('기타')" labelLoc="top" :required="true">
                    <template #editor="editorProps">
                      <v-row>
                        <v-textarea readonly v-model="popupParam.NRoIAData.ETC" variant="solo" flat class="mt-2" />
                      </v-row>
                    </template>
                  </ILabel>
                </VCol>
