...
const MUL1_CNT = reactive([])
const MUL2_CNT = reactive([])
const JUNGDAE_CNT = reactive([])
const ALL_CNT = reactive([])
const chartDraw = ref(0)

// 사고현황_누적
const TopAllCnt2 = ref(0)
const MONTH2 = reactive([])
const SAN_CNT2 = reactive([])
const MUL1_CNT2 = reactive([])
const MUL2_CNT2 = reactive([])
const JUNGDAE_CNT2 = reactive([])
const ALL_CNT2 = reactive([])
const chartDraw2 = ref(0)
...
const searchdashBoard = () => {
  ...
  // 사고현황 누적
  for (let i in props.SagoData.ORESULT_CUR5) {
    console.log("i=", i)
    SagoH2[i] = props.SagoData.ORESULT_CUR5[i]
  ...
  for (let i in SagoH) {
    ...
    MUL1_CNT.push(SagoH[i].MUL1_CNT)
    //MUL1_CNT.push(3)
    MUL2_CNT.push(SagoH[i].MUL2_CNT)
    //MUL2_CNT.push(5)
    JUNGDAE_CNT.push(SagoH[i].JUNGDAE_CNT)
  ...
  // 사고현황누적
  for (let i in SagoH2) {
    MONTH2.push(SagoH2[i].MONTH)
    SAN_CNT2.push(SagoH2[i].SAN_CNT)    
    MUL1_CNT2.push(SagoH2[i].MUL1_CNT)
    //MUL1_CNT.push(3)
    MUL2_CNT2.push(SagoH2[i].MUL2_CNT)
    //MUL2_CNT.push(5)
    JUNGDAE_CNT2.push(SagoH2[i].JUNGDAE_CNT)
    ALL_CNT2.push(SagoH2[i].ALL_CNT)
    console.log(SagoH2[i], i)
  }
  ...
  // REDUCE : 배열을 상대로 각 요소인자로 넘어온 콜백 함수를 실행하여 누적된 하나의 결과값을 반환합니다.
  let topCnt2 = SagoH2.reduce((max, obj) =>
    max.ALL_CNT > obj.ALL_CNT ? max : obj
  ).ALL_CNT
  TopAllCnt2.value = 0
  TopAllCnt2.value = topCnt2 + 10
  let MONTHChang2 = MONTH2.map((x) => {
    return `${x}월`
  })

  console.log(TopAllCnt.value,'TopAllCnt.value')
  ...
  lineChartsOptions2.value.series.map((x) => {
    switch (x.name) {
      case "산재":
        x.data = [...SAN_CNT2]
        break
  ...
  lineChartsOptions2.value.yAxis.map((x) => {
    x.max = TopAllCnt2.value
  })
  ...
  console.log(lineChartsOptions2.value.series, "result")
}
...
const lineChartsOptions = ref({
  ...
  legend: {
    // data: ["산재", "경미상1", "경미상2", "중대성재해", "합계"],
    data: ["산재", "경미상1", "경미상2",  "합계"],    
  },
  xAxis: [
    {
      type: "category",
      ...
      <v-sheet class="pa-0 h-auto">
        ...
        <v-card class="pa-0 ">
          ...
          <v-card-text class="pa-0">
            <e-chart :option="lineChartsOptions" :key="chartDraw" class="echarts_style" style="height: 320px;"/>
          </v-card-text>
        </v-card>
        <v-card class="pa-0 mt-5">
          <v-card-title class="pa-0 pb-2">
            {{
              SearchParam.SAGO_GBN == "A" || SearchParam.SAGO_GBN == "ALL"
                ? `${dayjs().format("YYYY")}년도 사고 현황(누적)`
                : `${dayjs().format("YYYY")}년도 사고 현황(누적)`
            }}
          </v-card-title>
          <v-card-text class="pa-0">
            <e-chart :option="lineChartsOptions2" :key="chartDraw2" class="echarts_style " style="height: 320px;"/>
          </v-card-text>
        </v-card>
      </v-sheet>
    </div>
