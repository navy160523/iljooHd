...
import { commonSampleDownFilesApi, commonDownloadFilesApi } from '@hiway/api/commonFileApi'
...
const isUpload4 = ref(false)
// 팝업관련
const deptPopup = ref(null)
const MainPopOpen = ref(null)
const emit = defineEmits(['close', 'upData', 'upDngTab2'])
...
// 진단서 작성 여부
const GINDANS_CANT = ref(false)
const GINDANS_CANTBTN = ref(false)
const GINDANS_isNoDown = ref(false)
// 진술서 작성여부 확인
const JINSUL_CANT = ref(false)
const JINSUL_CANTBTN = ref(false)
const JINSUL_isNoDown = ref(false)
const JINSUL_CANT2 = ref(false)
const JINSUL_CANTBTN2 = ref(false)
const JINSUL2_isNoDown = ref(false)
const JINSUL_width = ref("400px")
const JINSUL_width2 = ref("400px")
...
const FILE_BTN = reactive({
  RPT_BTN: ['btnFileUpload', 'btnDownLoad', 'btnDelete'],
  GINDANS_BTN: ['btnFileUpload', 'btnDownLoad', 'btnDelete'],
})
...
const CancelApprovebtn = ref(true)

const sAFAB0020_Reason = ref(null)
const m_dialog = ref(false)
const dialog_text = ref("<br>근태지원 : 재해발생일 포함 3일(주말/공휴일 제외)<br>" +
...
onMounted(() =>  {
  ...
  console.log('사고 보고서 Popdata ▶', Popdata)

  // 산업재해 조사표 버튼 및 다운로드 가능 여부
  if (isNullCheck(Popdata.RPT_FILE_ID)){
    FILE_BTN.RPT_BTN = ['btnFileUpload', 'btnDownLoad', 'btnDelete']
  ...
  console.log('산업재해 조사표 버튼 Mount' ,FILE_BTN.RPT_BTN)

  // 진단서 버튼 및 다운로드 가능 여부
  if (isNullCheck(Popdata.GINDANS_FILE_ID)){
    FILE_BTN.GINDANS_BTN = ['btnFileUpload', 'btnDownLoad', 'btnDelete']
    GINDANS_isNoDown.value = true
  } else {
    // 파일키가 있지만 업로드 된 파일이 없는경우 버튼 활성화
    if (Popdata.GINDANS_CNT === 0){
      FILE_BTN.GINDANS_BTN = ['btnFileUpload', 'btnDownLoad', 'btnDelete']
      GINDANS_isNoDown.value = true
    } else {
      if (userStore.userId === sagoEmpNo || userStore.empNo === sagoEmpNo2 
      || userStore.userId === Popdata.GINDANS_USER_ID || userStore.userId === Popdata.SAFE_GAMGR_EMP  || userStore.authGrpCd.includes('GRP00268')
      // || userStore.userId === 'BP21926'
      ){
        FILE_BTN.GINDANS_BTN = ['btnFileUpload', 'btnDownLoad', 'btnDelete']
        GINDANS_isNoDown.value = true
      } else {
        FILE_BTN.GINDANS_BTN = []
        GINDANS_isNoDown.value = false
      }
    }
  }

  console.log('산업재해 조사표 첨부파일 활성화', isadmin)
  console.log('진단서 첨부파일 활성화', isadmin2)
  

  if (Popdata.APPROVAL_STATUS === "결재 완료"){
    CancelApprovebtn.value = false
  }
...
const chkSaveDNG = () => {
  if((Popdata.SAGO_DIV_L === 'A'||Popdata.SAGO_DIV_L === 'D'||Popdata.SAGO_DIV_L === 'E')
    ...
    return false
  } else if((Popdata.SAGO_DIV_L === 'A'||Popdata.SAGO_DIV_L === 'D'||Popdata.SAGO_DIV_L === 'E')
     && Popdata?.RESP_DEPT_CD?.substr(0, 1) === 'E' && chkEmpty(Popdata.WRK_STD_NM_E)){
    Message.warn(t('사고분류가 작업성, 물적, 아차이면 작업표준은 필수입력입니다.'))
    return false
  } else if(chkEmpty(Popdata.WIHEOM_YN)){
    Message.warn(t('Hi-Standard 개정 필요 여부를 선택해주세요.'))
    return false
  } else if(Popdata.WIHEOM_YN ==='N'){
...
const grdDng2Props = reactive({
  ...
  fields: [
    ...
    { fieldName: 'UNIT_DESC_ORG', visible: false, },
    { fieldName: 'WORK_DESC_ORG', visible: false, },
    { fieldName: 'FACTOR_DESC_ORG', visible: false, },
    { fieldName: 'DNG_LVL_ORG', visible: false, },
    { fieldName: 'MEASURE_ORG', visible: false, },
  ],
  columns: [],
})
...
// 재해 원인 및 재발방지대책 입력 체크 (사고형태가 물적, 아차인 경우만)
const preCheck = () => {
  ...
  let param = [
    {
      ...
      JINSUL_CANT_CHK2: Popdata.JINSUL_CANT_CHK2,
      JINSUL_CANT_DESC2: Popdata.JINSUL_CANT_DESC2,
...
// 산업재해 조사표 작성 여부 체크
watch(
  () => Popdata.RPT_CANT_CHK,
  (newValue) => {
    
    if (newValue == null || newValue == '' || newValue == undefined) {
      Popdata.RPT_CANT_CHK = 'N'
    }
    if (newValue == 'Y') {
      FILE_BTN.RPT_BTN = []
      
      RPT_CANT.value = false
      RPT_CANTBTN.value = true
    }
    if (newValue == 'N') {
      if (userStore.userId === sagoEmpNo || userStore.empNo === sagoEmpNo2
      || userStore.userId === Popdata.RPT_USER_ID || userStore.userId === Popdata.SAFE_GAMGR_EMP || userStore.authGrpCd.includes('GRP00268')){
      ...
      RPT_CANT.value = true
      RPT_CANTBTN.value = false
      Popdata.RPT_CANT_DESC = ''
    }

    console.log('산업재해 조사표 버튼 watch' ,FILE_BTN.RPT_BTN)
  }
)

// 진단서 작성 여부 체크
watch(
  () => Popdata.GINDANS_CANT_CHK,
  (newValue) => {
    if (newValue == null || newValue == '' || newValue == undefined) {
      Popdata.GINDANS_CANT_CHK = 'N'
    }
    if (newValue == 'Y') {
      FILE_BTN.GINDANS_BTN = []
      
      GINDANS_CANT.value = false
      GINDANS_CANTBTN.value = true
    }
    if (newValue == 'N') {
      if (userStore.userId === sagoEmpNo || userStore.empNo === sagoEmpNo2
      || userStore.userId === Popdata.RPT_USER_ID || userStore.userId === Popdata.SAFE_GAMGR_EMP || userStore.authGrpCd.includes('GRP00268')){
      ...
      GINDANS_CANT.value = true
      GINDANS_CANTBTN.value = false
      Popdata.GINDANS_CANT_DESC = ''
    }
  }
)
...
watch(
  ...
  (newValue) => {
    ...
    if (newValue == 'Y') {
      ...
      JINSUL_isNoDown.value = true
      JINSUL_width.value = '350px'
    }
    if (newValue == 'N') {
      ...
      JINSUL_isNoDown.value = false
      JINSUL_width.value = '400px'
    }
  }
)

watch(
  () => Popdata.JINSUL_CANT_CHK2,
  (newValue) => {
    if (newValue == null || newValue == '' || newValue == undefined) {
      Popdata.JINSUL_CANT_CHK2 = 'N'
    }
    if (newValue == 'Y') { 
      ...
      JINSUL_CANT2.value = false
      JINSUL_CANTBTN2.value = true
      JINSUL2_isNoDown.value = true
      JINSUL_width2.value = '350px'
    }
    if (newValue == 'N') {
      ...
      JINSUL_CANT2.value = true
      Popdata.JINSUL_CANT_DESC2 = ''
      JINSUL_CANTBTN2.value = false
      JINSUL2_isNoDown.value = false
      JINSUL_width2.value = '400px'
    }
  }
)
...
const joUploaded = () => {
  ...
  commonExecuteApi({ queryId: 'SAFAB0020_save35', list: param }).then((res) => {
...
const checkFilejinUp = async () => {
  if (
    Popdata.GINDANS_FILE_ID === null ||
    Popdata.GINDANS_FILE_ID === undefined
  ) {
    jinUpload.value.setGuid()
    Popdata.GINDANS_FILE_ID = jinUpload.value.guid
    await jinUpload.value.onButtonsClick({ id: 'btnSearch' })
  } else {
    //파일아이디 있을때

    jinUpload.value.setGuid(Popdata.GINDANS_FILE_ID)
    Popdata.GINDANS_FILE_ID = jinUpload.value.guid
    await jinUpload.value.onButtonsClick({ id: 'btnSearch' })
  }
  ...
  let param = [
    {
      ...
      SAGO_SN: injuSahang.value.SAGO_SN,
      SEQ: injuSahang.value.SEQ,
      GINDANS_FILE_ID: Popdata.GINDANS_FILE_ID,
      GINDANS_CANT_CHK: Popdata.GINDANS_CANT_CHK,
      GINDANS_CANT_DESC: Popdata.GINDANS_CANT_DESC,
      JINDAN_CD: injuSahang.value.JINDAN_CD,
    },
  ]
  commonExecuteApi({ queryId: 'SAFAB0020_save24', list: param }).then((res) => {
    Message.success(t('저장 되었습니다.'))
  })
}
...
const CompleteSend = () => {
  ...
  // 사고형태가 물적 또는 아차인 경우 재해원인 및 재발방지대책 필수 입력
  if (Popdata.SAGO_DIV_L === 'E' || Popdata.SAGO_DIV_L === 'D'){
    if (preCheck()){
      Message.warn(t('재해원인 및 재발방지대책은 필수 입력 입니다.'))
      return ''
    }
  }
  ...
  if(lengthgrdTwo === 0){
    Message.warn(t('재해원인 및 재발방지대책이 입력되지않았습니다.'))
    return
  }

  // 2025.06.25 산업재해 조사표 체크 로직이 새로 적용 필요 X
  // if(injuSahang.value){
  ...
  // 산업재해 조사표 작성불가 사유 
  if (Popdata.RPT_CANT_CHK === 'N'){
    // 사고 대분류 : 작업성 사고 - 사고처리 구분 : 산재, 경미공상, 휴업 3일이상 사고
    if (Popdata.SAGO_DIV_L === 'A' && 
      ...
      if(!isUpload.value){
        Message.warn(t('산업재해 조사표 작성은 필수입니다. 산업재해 조사표에 첨부된 파일이없습니다.'))
        return
      }
    }
  } else {
    if (!Popdata.RPT_CANT_DESC){
      Message.warn(t('산업재해 조사표 작성불가 사유를 입력해주십시오.'))
      return
    }
  }

  // 진단서 조사표 작성불가 사유 
  if (Popdata.GINDANS_CANT_CHK === 'N'){
    // 사고 대분류 : 작업성 사고 - 사고처리 구분 : 산재
    if (Popdata.SAGO_DIV_L === 'A' && Popdata.SAGO_TYPE_CD == "SJE"){
      if(!isJinUpload.value){
        Message.warn(t('진단서 작성은 필수입니다. 진단서에 첨부된 파일이없습니다.'))
        return
      }
    }
  } else {
    if (!Popdata.GINDANS_CANT_DESC){
      Message.warn(t('진단서 작성불가 사유를 입력해주십시오.'))
      return
    }
  }
  ...
  if(Popdata.JINSUL_CANT_CHK !=='Y'){
    ...
    if(!Popdata.JINSUL_CANT_DESC){
      Message.warn(t('재해자 진술서 작성불가 사유를 입력해주십시오.'))
      return
    }
  }

  // 관리감독자 진술서
  if(Popdata.JINSUL_CANT_CHK2 !=='Y'){
    if(!isUpload3.value){
    ...
    if(!Popdata.JINSUL_CANT_DESC2){
      Message.warn(t('관리감독자 진술서 작성불가 사유를 입력해주십시오.'))
      return
    }
  }
    ...
    if (swalRes.isConfirmed === true) {
      ...
      commonExecuteApi({ queryId: 'SAFAB0020_save26', list: saveParams })
        .then((res) => {
          ...
          //SMS 발송
          let sendSMSParams2 = {
            EMPNO_FROM: sagoEmpNo2, // 강현웅 선임 매니저
            EMPNO_TO: Popdata.SAFE_GAMGR_EMP,
            SYSCODE: '156',
            FROM_P_NO: Popdata.SAGODHND.replace(/-/g, ''),
            TO_P_NO: Popdata.SAFE_GAMGR_HNO,
...
Popdata.CANCEL_REASON2 = val
Popdata.GBN = '1'

let saveParams = []
saveParams.push(Popdata)
let name = injuSahang.value.NAME ? injuSahang.value.NAME : ''
let SOSOG_DESC = injuSahang.value.SOSOG_DESC ? injuSahang.value.SOSOG_DESC : ''
let sago = `${Popdata.SAGO_DIV_M_NM}`
let Date = `${dayjs(Popdata.OCCUR_DT).format('YYYY')}년 ${dayjs(
  Popdata.OCCUR_DT
).format('MM')}월 ${dayjs(Popdata.OCCUR_DT).format('DD')}일`
...
commonExecuteApi({ queryId: 'SAFAB0020_save26', list: saveParams }).then(
  (res) => {
    ...
    mailParams.SUBJECT = '사고 보고서 작성완료 취소 건'
    mailParams.CONTENT = `${Date} ${SOSOG_DESC} ${name} ${sago} 사고의 안전사고 보고서 작성이 취소되었으니, 내용 확인 후 재작성 요청드립니다.` +
                         `<br> 취소사유 : ${val}`
...
// 안전확인
const SafetyCheck = () => {
  // if(chkItems.PEOPLE === 'N' || chkItems.DESC === 'N' || chkItems.INFO === 'N' ||
  //    chkItems.DNG === 'N' || chkItems.MEASURE === 'N' || chkItems.PICTURE === 'N' ||
  //    chkItems.JOSA === 'N' || chkItems.JINDAN === 'N' || chkItems.JINSUL === 'N' ||
  //    (isBull.value && chkItems.SANJE === 'N')) {
  ...
  //       <확인 필요 항목>
  //       인적사항 현황, 안전 사고 개요, 사고 정보, 수시 위험성평가,
  //       재해원인 및 예방대책, 사고 상황 사진, 산업재해 조사표,
  //       진단서, 진술서, 산재불인정 부서의견서(해당할 경우)`
  //     ),
  //   })
      ...
      commonExecuteApi({ queryId: 'SAFAB0020_save27', list: saveParams }).then(
        (res) => {
          commonSearchApi({
            ...
            //   }
            // }).then(resWiheom => {
            //   const wiheomYn = resWiheom?.ORESULT_CUR[0]?.WIHEOM_YN
            //   const wrkStdNo = resWiheom?.ORESULT_CUR[0]?.WRK_STD_NO
            //   // console.log('wiheomYn', wiheomYn)
            ...
            //             CMPNY_DIV: Popdata.CMPNY_DIV,
            //             SAGO_SN: Popdata.SAGO_SN,
            //             WRK_STD_NO: res.returnValue,//WORK_STANDARD_MASTER.WORK_STANDARD_ID,
            //           },
            //         ]
            //         commonExecuteApi({ queryId: 'SAFAB0020_save23', list: param }).then(
            //           (res) => {
...
const CancelApprove = () => {
  vm.$swal({
    ...
    if (swalRes.isConfirmed === true) {
      if (Popdata.APPROVAL_STATUS !== '결재 완료') {
        return Message.warn(t('결재완료 사고 건만 취소 가능합니다.'))
      }

      let saveParams = []
      saveParams.push(Popdata)
      commonExecuteApi({ queryId: 'SAFAB0020_save34', list: saveParams }).then(
        (res) => {
          Message.warn(t('결재 취소 되었습니다.'))
          emit('close')
        }
      )
    }
...
const allDownload = async () => {
  let fileIdList = []
  let query = 'searchFile'
  
  await Promise.all([
    commonSearchApi({queryId: query, param: { CMPNY_DIV: Popdata.CMPNY_DIV, FILE_ID: Popdata.RPT_FILE_ID },}),      // 산업재해 조사표
    commonSearchApi({queryId: query, param: { CMPNY_DIV: Popdata.CMPNY_DIV, FILE_ID: Popdata.GINDANS_FILE_ID },}),  // 진단서
    commonSearchApi({queryId: query, param: { CMPNY_DIV: Popdata.CMPNY_DIV, FILE_ID: Popdata.JINSUL_FILE_ID },}),   // 재해자 진술서
    commonSearchApi({queryId: query, param: { CMPNY_DIV: Popdata.CMPNY_DIV, FILE_ID: Popdata.JINSUL_FILE_ID2 },}),  // 관리감독자 진술서
    commonSearchApi({queryId: query, param: { CMPNY_DIV: Popdata.CMPNY_DIV, FILE_ID: Popdata.JINSUL_FILE_ID3 },}),  // 목격자/기타 진술서
  ]).then((res) => {
    for(var i = 0; i < res.length; i++) {
      for (var id of res[i].ORESULT_CUR) {
        fileIdList.push(id)
      }
    }
  })
...
const chkRequireDNG = () => {
  const data1 = grdDng1.value.getDataProvider().getJsonRows()
  const data2 = grdDng2.value.getDataProvider().getJsonRows()
  const fData1 = _.filter(data1, row => row.STATUS !== 'I')
  const fData2 = _.filter(data2, row => !chkEmpty(row.SEQ))
  const reqFData2 = _.filter(
    fData2,
    row => chkEmpty(row.UNIT_DESC_ORG) || chkEmpty(row.WORK_DESC_ORG) || chkEmpty(row.FACTOR_DESC_ORG) ||
           chkEmpty(row.DNG_LVL_ORG) || chkEmpty(row.MEASURE_ORG)
  )
  ...
  if(data1.length < 1 || data2.length < 1) {
    Message.warn(t('평가자, 평가내용은 필수입력입니다.'))
    return false
  } else if(fData1.length < 1) {
    Message.warn(t('저장된 평가자가 없습니다.'))
    return false
  } else if(fData2.length < 1) {
    Message.warn(t('저장된 평가내용이 없습니다.'))
    return false
  } else if(reqFData2.length > 0) {
    Message.warn(t('평가내용의 단위작업, 작업행동, 위험요인, 위험성, 위험감소대책을 모두 입력 후 저장해주세요.'))
    return false
  }
    ...
    <div>
      ...
      <v-btn @click="CancelSafetyCheck" v-if="userStore.authGrpCd.includes('GRP00381')"
       :disabled="endCheck || CancelSafety"
        >안전확인 취소</v-btn
      >
      <!-- <v-btn @click="CancelSafetyCheck"
       :disabled="endCheck || CancelSafety"
        >안전확인 취소</v-btn
      > -->
      <!-- 사고 담당자만 종결 및 종결 취소, 결재취소 가능 -->
      <v-btn @click="Complete" :disabled="endCheck || Completebtn" v-if="userStore.empNo === sagoEmpNo ||  userStore.empNo === sagoEmpNo2 ">종결</v-btn>
      <v-btn @click="CancelComplete" :disabled="CancelCompletebtn" v-if="userStore.empNo === sagoEmpNo || userStore.empNo === sagoEmpNo2 ">종결취소</v-btn>

      <v-btn @click="CancelApprove" :disabled="CancelApprovebtn" v-if="userStore.empNo === sagoEmpNo || userStore.empNo === sagoEmpNo2 ">결재취소</v-btn>
      <v-btn @click="allDownload" v-if="userStore.authGrpCd.includes('GRP00268')">일괄 다운로드</v-btn>
    </div>
  </div>
    ...
    <v-card-text class="wrap">
        ...
        <v-card class="pa-3 rounded-lg">
          ...
          <div class="d-flex align-center ma-0">
            <div
              ...
              style="width: 120px; padding: 8px 0; font-size: 14px; color: #1a40c7; font-weight: bold; background-color: #eeeeee; position: relative; margin-right: 10px !important;"
            >
              수시 위험성평가
              <v-icon
                icon="mdi-circle-small"
                size="30"
                style="color: red; position: absolute; top: -6px; right: -2px;"
              />
            </div>
            <!-- <v-checkbox v-if="isSafeUser" v-model="chkItems.DNG" label="작성내용 확인" class="chk-confirm" /> -->
          </div>
          <div
            class="pa-3"
          ...
          <div class="d-flex ma-0 px-3 pb-3">
            <div class="mr-4">
              <p style="color: #222; position: relative;">
                {{ $t('Hi-Standard 개정 필요 여부') }}
                <v-icon
                  icon="mdi-circle-small"
                  size="30"
                  style="color: red; position: absolute; top: -13px; right: -18px;"
                />
              </p>
              <v-radio-group
                inline
      ...
      <v-card class="mb-3 mr-1 popUp-input-Form">
        <v-card-title class="pa-0">
          ...
          >
            <template #editors>
              <!-- <v-checkbox v-if="isSafeUser" v-model="chkItems.MEASURE" label="작성내용 확인" class="chk-confirm" /> -->
            </template>
          </IGridTitle>
        </v-card-title>
        <v-card-text class="pa-0">
      ...
      <div class="d-flex mb-3">
        <v-sheet width="50%" class="pa-0 h-auto mr-3">
          <div style="height: 42px; padding-top: 6px; margin-bottom: 18px">
            ...
            >
              <template #editors>
                <!-- <v-checkbox v-if="isSafeUser" v-model="chkItems.JOSA" label="작성내용 확인" class="chk-confirm" /> -->
              </template>
            </IGridTitle>
          </div>
          ...
          <div class="popUp-input-Form mt-3">
            ...
            >
              ...
              <div class="d-flex align-center">
                <v-checkbox
                  class="mx-2"
                  v-model="Popdata.RPT_CANT_CHK"
                  label="산업재해 조사표 작성 불가"
                />
                <i-input
                  :label="$t('사유 입력')"
                  v-model="Popdata.RPT_CANT_DESC"
                  label-width="110px"
                  type="Text"
                  width="460px"
                  :disabled="RPT_CANT"
                />
                <v-btn @click="joUploaded()" v-if="RPT_CANTBTN">
                  저장
                </v-btn>
              </div>
            </div>
        ...
        <v-sheet width="50%" class="pa-0 h-auto mr-3">
          <div class="d-flex align-center">
            <IGridTitle :title="$t('진단서')">
              <template #editors>
                <!-- <v-checkbox v-if="isSafeUser" v-model="chkItems.JINDAN" label="작성내용 확인" class="chk-confirm" /> -->
              </template>
            </IGridTitle>
            <div class="mr-3">
          ...
          <div class="popUp-input-Form mt-3">
            ...
            >
              ...
              <IUpload
                ...
                :download-off="GINDANS_isNoDown"
                :button-list="FILE_BTN.GINDANS_BTN"
                :isVisibled="GINDANS_CANT"
                style="height: 400px"
              />
              <div class="d-flex align-center">
                <v-checkbox
                  class="mx-2"
                  v-model="Popdata.GINDANS_CANT_CHK"
                  label="진단서/소견서 작성 불가"
                />
                <i-input
                  :label="$t('사유 입력')"
                  v-model="Popdata.GINDANS_CANT_DESC"
                  label-width="110px"
                  type="Text"
                  width="460px"
                  :disabled="GINDANS_CANT"
                />
                <v-btn @click="jinUploaded()" v-if="GINDANS_CANTBTN">
                  저장
                </v-btn>
              </div>
            </div>
      ...
      <v-card class="mb-3 mr-1 popUp-input-Form">
        ...
        >
          <IGridTitle
            ...
            :button-list="['btnPrintHandwriting']"
            @click-button="onButtonsClick($event.id, '진술서')"
          >
            <template #editors>
              <!-- <v-checkbox v-if="isSafeUser" v-model="chkItems.JINSUL" label="작성내용 확인" class="chk-confirm" /> -->
            </template>
          </IGridTitle>
        </v-card-title>
        <v-card-text class="pa-0 d-flex" style="justify-content: space-around">
          ...
          >
            <IUpload
              ...
              :gridOnly="NOEdit"
              class="fileClass"
              :isVisibled="JINSUL_CANT2"
              :download-off="JINSUL2_isNoDown"
              @uploaded="JINSULUploaded($event)"
            ...
            <div class="d-flex align-center">
              <v-checkbox
                class="mx-2"
                v-model="Popdata.JINSUL_CANT_CHK2"
                label="관리감독자 작성 불가"
              />
              <i-input
                :label="$t('사유 입력')"
                v-model="Popdata.JINSUL_CANT_DESC2"
                label-width="80px"
                type="Text"
                :width="JINSUL_width2"
                :disabled="JINSUL_CANT2"
              />
              <v-btn @click="JINSULUploaded()" v-if="JINSUL_CANTBTN2">
                저장
              </v-btn>
            </div>
          </div>
 ...
 <!-- <v-card class="mb-3 mr-1 popUp-input-Form">
      ...
      <v-card class="mb-3 mr-1 popUp-input-Form">
        <v-card-title class="pa-0">
          ...
          >
            <template #editors>
              <!-- <v-checkbox v-if="isBull && isSafeUser" v-model="chkItems.SANJE" label="작성내용 확인" class="chk-confirm" /> -->
            </template>
          </IGridTitle>
          <!-- 'btnDelete2' -->
        </v-card-title>
