...
const DateInt = ref([{ date: "", dt: "", time: "", DN_DIV: "" , CANCELED_YN: "",}])
const CCP = ref(null)
...
const datainer = ref({
  ...
  URGENT_YN : 'N',
  REQ_EMP_NO : userStore.empNo,
  REQ_EMP_NM : userStore.empNm,
  WORK_NO_EXIST : 'N'  // 호선없음 체크박스
})
...
const ReSetData = () => {
  datainer.value = {}
  DateInt.value = [{ date: "", dt: "", time: "", DN_DIV: "" , CANCELED_YN: "",}]
  LocationType.value = ""
  ...
  datainer.value.WORK_NO_EXIST = 'N'
  ...
  if (!isDan.value) {
    ...
    commonSearchApi({
      ...
      let fres = res.ORESULT_CUR.filter(x => (x.WRK_PLC != '99') || (x.BSNS_CD == datainer.value.JOB_BSNS_CD))
      console.log("fres2", fres)
      if(datainer.value.JOB_BSNS_CD === 'AQ00'){
      ...
      console.log("ReSetData fres", fres)

      grdSub2.value.getDataProvider().setRows(fres)
    })
  } else {
    ...
    commonSearchApi({
      ...
      if(datainer.value.JOB_BSNS_CD === 'AE00'){
        res.ORESULT_CUR = res.ORESULT_CUR.filter(item => item.P_WRK_ID !== 'S999999')
      }
...
const openPopup = async (popupParam, Safety) => {
  ...
  vm.$nextTick(() => {
    ...
    grdSub2.value.getGridView().setFocus(0) //행 0으로
...
const onEmpSelected = (row) => {
  ...
  if (gbnEmp.value === "req") {
    ...
    grdMain.value
      .getDataProvider()
      .setValue(rowChValue.value, "REQ_EMP_NM", row.EMP_NM)
    grdMain.value
      .getDataProvider()
      .setValue(rowChValue.value, "REQ_EMP_NO", row.EMP_NO)
...
const RtnWorkPaln = (res) => {
  console.log('RtnWorkPaln res ', res)
  console.log('rowChValue.value :: ' , rowChValue.value)

  if(res.list[0].RES_KEY){
    grdMain.value.getDataProvider().setValue(rowChValue.value, "WRK_PLAN_YN", "Y")
  } else {
    grdMain.value.getDataProvider().setValue(rowChValue.value, "WRK_PLAN_YN", "N")
  }
}
...
const searchData = async (popupParam) => {
  ...
  await commonSearchApi({
    ...
    /*if (!res.ORESULT_CUR[0].WRK_DIV) {
      if(res.ORESULT_CUR[0].WRK_DIV === "B"){ //건설공사일 경우
        var header = grdMain.value.getGridView().getColumnProperty("REQ_EMP_NM", "header");
        header.text = "공사감독자";
        grdMain.value.getGridView().setColumnProperty("REQ_EMP_NM", "header", header);
      }
    }*/
    ...
    if (!res.ORESULT_CUR[0].SHORT_WORK_NO) {
      isDan.value = false
      grdMain.value.getGridView().columnByName('WRK_PLAN_YN').visible = false;
    } else {
      isDan.value = true
      grdMain.value.getGridView().columnByName('WRK_PLAN_YN').visible = true;
    }
  ...
  .then((Hres)=>{
    ...
    if (res.ORESULT_CUR[0].ALL_START_TIME) {
      startarr = res.ORESULT_CUR[0].ALL_START_TIME.split(",") // "202412121659,202412131659,202412141659,202412151659,202412161659"
      cancelarr = res.ORESULT_CUR[0].ALL_CANCELED_YN.split(",") // "N ,N, Y, N, N"
      console.log("startarr.length",startarr , cancelarr,  startarr.length)
      ...
      for (let i = 0; i < startarr.length ; i++) {
        ...
        DateInt.value[cnt].dt = dayjs(startarr[i]).format("MM/DD")
        DateInt.value[cnt].CANCELED_YN = cancelarr[i]
...
const EmailSend = async (gubun) => {
  if(gubun === 'APP') {
    ...
    await commonSearchApi({
      ...
      param: {
        CMPNY_DIV: datainer.value.CMPNY_DIV,
        EMP_NO: datainer.value.MNGR_EMP_NO,        
      },
    }).then(async res => { 
      console.log('REQ ', res)
...
const onButtonsClick = async (e, gubn) => {
  ...
  if (gubn == "메인") {
    ...
    if (e == "btnApplication") {
      ...
      console.log('datainer.value.RCV_NO 0', datainer.value.RCV_NO)
      let temp_datainer = datainer.value
      ...
      datainer.value = temp_datainer 
      console.log('datainer.value.RCV_NO 1', datainer.value.RCV_NO)
      // 현재 날짜를 dayjs로 가져오기      
      ...
      console.log('datainer.value.RCV_NO 2', datainer.value.RCV_NO)
      // 날짜 끝
      // 허가서 신청 할 행을 선택
      if (
        rowChValue.value === null ||
     ...
     // console.log('datainer.value.URGENT_YN ', datainer.value.URGENT_YN)
      ...
      console.log('datainer.value.RCV_NO 3', datainer.value.RCV_NO)
      ...
      vm.$swal({
        ...
        console.log('datainer.value.RCV_NO 4', datainer.value.RCV_NO)
  ...
  if (gubn == "위험작업허가서상세") {
    if (e == "btnCreate") {
      ...
      if (grdSub1_1.value) {
        grdSub1_1.value.getDataProvider().setRows(null)
      }
      if (grdSub1_2.value) {
    ...
    if (e === "btnRowCopy") {
      ...
      if (ALLDATA.value[dataRow.value] && ALLDATA.value[dataRow.value].Main) {
        ...
        let SaveData = {
          Main: { ...datainer.value },
          //Date: { ...DateInt.value }, //2025.09.30 일시는 복사안함
          LocationType: LocationType.value,
          Location: Location,
        }
        ALLDATA.value.push(SaveData)
      ...
      grdMain.value.addRow(
        {
          ...
          WRK_DATE: '', //row.WRK_DATE,2025.09.30 일시는 복사안함
          WRKCHK: row.WRKCHK,
          ...
          REQ_EMP_NO: row.REQ_EMP_NO,
          REQ_EMP_NM: row.REQ_EMP_NM,
          SHORT_WORK_NO: row.SHORT_WORK_NO,
          WRK_PLAN_YN: row.WRK_PLAN_YN,
        },
        true
      )
...
const grdProps1 = reactive({
  ...
  fields: [
    ...
    { fieldName: "REQ_EMP_NO", visible: false, dataType: "text" },    
  ],
  columns: [],
})
...
const onCellClicked = (grid, clickData) => {
  ...
  console.log('isDan.value ', isDan.value)
  ...
  if (!disabledSTATUS.value) {
    ...
    if (clickData.cellType === "data" && clickData.column == "RESP_EMP_NM") {
      gbnEmp.value = "grd"
      empPopup.value.openPopup({USER_DIV: isDan.value ? 'D' : ''})      
      isGrdint.value = true
    }
    if (clickData.cellType === "data" && clickData.column == "REQ_EMP_NM") {
      gbnEmp.value = "req"
  ...
  // 2025.04.22 단기공사 작업표준은 팝업을 띄워서 확인이 필요하여 따로 뺌
  ...
  // 작업계획서 팝업 호출
  if (clickData.cellType === "data" && clickData.column == "WRK_PLAN_YN") {
    if (isDan.value) {
      if (isEmpty(datainer.value.SHORT_WORK_NO)) {
        console.log('isEmpty(datainer.value.SHORT_WORK_NO) ', isEmpty(datainer.value.SHORT_WORK_NO), ' ::: ', datainer.value.SHORT_WORK_NO)
      ...
      if (isEmpty(datainer.value.RCV_NO)) {       

        Message.warn(t("허가서 저장 후에 입력해 주시기 바랍니다."))
        return
      }

      WorkPlanPopup.value.openPopup(datainer.value)
    }
  }
}
...
const onCellDblClicked = (grid, clickData) => {
  if (clickData.cellType === "data") {
    ...
    if (
      ...
      // 단기공사 긴급 신청작업 선택 시
      if(row.P_WRK_ID === 'S000000'){
        return Message.warn(t("단기공사 긴급 신청작업은 단기공사업체에서 신청 시만 사용가능합니다.\n위험작업 분류 추가 후 사용바랍니다."))
      }
      ...
      else if  (row.CF_DIV === "F" || row.CF_DIV === "S") {
        grdMain.value
          .getDataProvider()
...
const DateInner = (e) => {
  ...
  datainer.value.ALL_CANCELED_YN = canceledyn.join(",")
  ...
  console.log("datainer.value.ALL_CANCELED_YN1", datainer.value.ALL_CANCELED_YN)
...
const beforeSave = () => {
  ...
  if (ALLDATA.value.length === 0) {
    ...
    console.log(" datainer",  datainer.value )
    console.log(" datainer.value.WORK_NO_EXIST",  datainer.value.WORK_NO_EXIST )

    // 작업장소
    if (LocationType.value == "D" || LocationType.value == "E") {
      if ( datainer.value.WORK_NO_EXIST === 'N' && !datainer.value.WORK_NO ) {
        return Message.warn(t("호선/공사를 선택해주세요."))
      }

    } 

    if (LocationType.value == "B" || LocationType.value == "D" || LocationType.value == "E") {
      let rowCnt = 0
      ...
      if(rowCnt == 0){
        if  (datainer.value.WORK_NO_EXIST == 'N'){
          return Message.warn(t("작업 장소의 위치 추가를 해주세요"))
        }
      }
    }

    // 2025.05.13 윤찬양 책임 요청으로 작업사업부가 조선,해양,특수선이고 장소가 외업인 경우 3일까지만 저장 가능하게 수정
    let DateCnt = DateInt.value.length
    let JobBsnsCd = datainer.value.JOB_BSNS_CD

    if ( (JobBsnsCd=== "AC00" || JobBsnsCd === "AE00" || JobBsnsCd === "AQ00") 
        && datainer.value.JOB_LPLC === "B" && DateCnt > 3) 
    {  
      return Message.warn(t("작업 기간은 최대 3일까지만 설정 가능합니다."))   
    }
    ...
    console.log('datainer.value.RCV_NO 01', datainer.value.RCV_NO)
    let cnt = -1
    for (let i of ALLDATA.value) {
      ...
      if (!i.isDel) {
        ...
        if (isDan.value && !rowData.SHORT_WORK_NO){
          Message.warn(
            t(
              `${
                cnt + 1
              } 번째로 생성된 위험작업허가서의 기본정보 부분의 단기공사 번호가 입력되어있지 않습니다.`
            )
          )
          return false
        }
        ...
        if (!rowData.DANG_DIV || !rowData.WORKER_CNT || !rowData.DNGR_RSN || !rowData.WRK_DATE) {
          Message.warn(
            t(
              `${
                cnt + 1
              } 번째로 생성된 위험작업허가서 상세 부분의 필수입력값이 입력되있지않습니다.<br> 필수입력 : 위험작업구분,인원, 유해위험요소,날짜 `
            )
          )
          return false
        }
        ...
        if (
          ...
          Message.warn(
            t(`${cnt + 1} 번째 의 기본정보 필수 입력값이 입력되지 않습니다.`)
          )
          return false
        }
        ...
        if (
          ...
          Message.warn(t("추가정보 부분의 필수 입력값이 입력되있지않습니다."))
          return false
        }
        ...
        if (i.Main.PRESSURE_YN === "Y" && !i.Main.PRESSURE) {
          Message.warn(t("압력테스트을 입력해주세요"))
          return false
        }

        if (i.Main.PIPE_DIA_YN === "Y" && !i.Main.PIPE_DIA) {
          Message.warn(t("압력테스트 PIPE직경을 입력해주세요"))
          return false
        }

        if (!i.Main.JOB_PLC) {
          Message.warn(t(`${cnt + 1} 번째 작업장소가 입력되있지않습니다.`))
          return false
        }
      }
    }
    console.log('datainer.value.RCV_NO 02', datainer.value.RCV_NO)
    return true
  }
}
...
const saveData = async(e) => {
  ...
  if (ALLDATA.value.length <= 1 || e === "신청") {
    ...
    if (isDateint.value) {
      ...
      datainer.value.ALL_OVERTIME_YN = OT.join(",")
      datainer.value.ALL_CANCELED_YN = canceledyn.join(",")
      console.log("datainer.value.ALL_CANCELED_YN2", datainer.value.ALL_CANCELED_YN)
      datainer.value.WORK_DT = workDt
      datainer.value.DN_DIV = DateInt.value[0].DN_DIV
    ...
    for (let i of ALLDATA.value) {
      ...
      if (!i.isDel) {
        ...
        console.log(cnt, "번째")
        console.log(cnt, "번째 rowrr", row)
        console.log(cnt, "번째 Main", i.Main)
        console.log(cnt, "번째 LOCATION", i.Location)
  ...
  (newValue) => {
    ...
    if (!isDan.value) {
      ...
      commonSearchApi({
        ...
        console.log("DANG_DIV fres", fres)

        grdSub2.value.getDataProvider().setRows(fres)  //여기서 오류
      })
    } else {
      DangSearch.JOB_LPLC = datainer.value.JOB_LPLC
      DangSearch.JOB_MPLC = datainer.value.JOB_MPLC
...
watch(
  ...
  ([BSNS_CD, JOB_BSNS_CD, RCV_TYPE, PLC]) => {
    ...
    if (JOB_BSNS_CD) {
      ...
      //console.log("row.RCV_NO", row.RCV_NO)

      // 주석 풀 경우 허가서 복사 시 문제 됨
      /* if(row.RCV_NO === "" || row.RCV_NO === undefined || row.RCV_NO === null){
        grdMain.value.getDataProvider().setValue(rowChValue.value, "WORK_COMPLEX_CD", "")
        grdMain.value.getDataProvider().setValue(rowChValue.value, "CF_DIV", "")
        grdMain.value.getDataProvider().setValue(rowChValue.value, "DANG_DIV", "")
        grdMain.value.getDataProvider().setValue(rowChValue.value, "P_WRK_ID", "")
        grdMain.value.getDataProvider().setValue(rowChValue.value, "WRK_DIST", "")
        grdMain.value.getDataProvider().setValue(rowChValue.value, "THREECHECKYN", "")
      } */
    ...
    if (newValue) {
      if (!isDan.value) {
        ...
        commonSearchApi({
          ...
          console.log("JOB_BSNS_CD fres", fres)
    ...
    if (oldValue != newValue) {
      if (!isDan.value) {
        ...
        commonSearchApi({
          ...
          console.log("RCV_TYPECheck fres", fres)
...
const setPrint = () => {
  ...
  let param = {
    ...
    /* commonRequest(
      '/hse/slm/',
      {
        orgLinkUrl: getQDomain() + `/#/ptw_qr?RCV_NO=${datainer.value.RCV_NO}&SHORT_WORK_NO=`,
      ...
      console.log('datainer.value.DANG_DIV ', datainer.value.DANG_DIV)
      console.log('datainer.value.P_WRK_ID ', datainer.value.P_WRK_ID)
      console.log('datainer.value.WRK_DIV : ', datainer.value.WRK_DIV)
      console.log('datainer.value.CF_DIV : ', datainer.value.CF_DIV)
      console.log('datainer.value : ', datainer.value)
      // urlArr = res.ORESULT_CUR.url
      // urlArr =  res.body
      urlArr = getQDomain() + `/#/ptw_qr?RCV_NO=${datainer.value.RCV_NO}&SHORT_WORK_NO=`
      console.log(urlArr,'dmdkdddk')

      if(datainer.value.DANG_DIV === "Z00"){
        // PSM 출력
        let sP_WRK_ID = datainer.value.P_WRK_ID

        datainer.value.CF_DIV === "F" ? (visible = "Y") : (visible = "N")
        let sCF_DIV_NM = datainer.value.CF_DIV === "C" ? `[전산허가]` : datainer.value.CF_DIV === "F" ? `[안전대면허가]` : `[안전현장허가]`
        ...
        if (
          ...
          // 대면허가
          if (datainer.value.WRK_DIV === "B" && datainer.value.CF_DIV === "F") {
            datainer.value.CF_DIV === "F" ? (visible = "Y") : (visible = "N")
            let sCF_DIV_NM = datainer.value.CF_DIV === "F" ? `[안전대면허가]` : `[안전현장허가]`
          ...
          // 현장허가  
          } else if (datainer.value.WRK_DIV === "B" && datainer.value.CF_DIV === "S" ) {
          ...
          // 전산허가
          } else if (datainer.value.WRK_DIV === "B" && datainer.value.CF_DIV === "C") {
            datainer.value.CF_DIV === "F" ? (visible = "Y") : (visible = "N")
          ...
          else if (datainer.value.JOB_LPLC === "B") {
            //외업작업장
          ...
          else if (datainer.value.DANG_DIV === "E03") {
...
async function applyDan(e){
  ...
  grdMain.value.getGridView().columnByName('WRK_PLAN_YN').visible = isDan.value;  
}
  ...
  (newValue) => {
    ...
    if (!newValue) {
      ...
      commonSearchApi({
        ...
        let fres = res.ORESULT_CUR.filter(x => (x.WRK_PLC != '99') || (x.BSNS_CD == datainer.value.JOB_BSNS_CD))
        console.log("fres7", fres)

        if(datainer.value.JOB_BSNS_CD === 'AQ00'){
            if(datainer.RCV_TYPECheck) {
              // 해양공사
              fres = fres.filter(x => (x.WRK_PLC === '99') || (x.BSNS_CD === 'AQ00'))
            } else {
              // 호선
  ...
  >
    ...
    <v-card class="pa-0 fill-height rounded-b-5">
      <v-card-title class="pa-4 pb-2 d-flex align-center">
        ...
        <span style="color: #ef5350; font-size: 13px;" v-show="isDan">
          ※ 작업표준(위험성평가서) 및 작업계획서는 아래 허가서 목록 상세에서 조회 및 출력 가능합니다.
        </span>
        <IMenuTitle
          ref="menuTitle"
          :button-list="[
        ...
        <v-card class="mb-3 mx-3 popUp-input-Form">
          ...
          <v-card-text class="pa-0">
            <v-sheet class="d-flex flex-row mt-3">
              ...
              <div v-if="isDan" class="pt-7">{{ datainer.WRK_DIV_NM }}</div>
            </v-sheet>
        ...
        <div class="h-grow">
          <v-sheet class="pa-3 mr-3 h-auto">
            ...
            <div class="d-flex">              
              ...
              <br/>
              ※위험작업 분류 추가는 [안전]-[단기공사 관리]-[단기공사 위험작업허가 작업 등록/관리] 메뉴에서 가능
              </p>
            </div>
            <div class="d-flex">
              <i-select
                ...
                <div>
                  <div class="d-flex mb-1" style="width: 826px !important">
                    <i-input
                      ...
                      :class ="(i.CANCELED_YN === 'Y' ? 'custom-input1' : 'custom-input2') "
                      v-model="i.dt"
                      readonly
                    />
                  </div>
