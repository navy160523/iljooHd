...
const grdMainProps = reactive({
  ...
  fields: [
    { fieldName: 'AREA_L_NM', dataType: 'text', header: { text: t('장소1'), }, mergeRule: { criteria: "values['AREA_L_NM']" }, width: '85', },
    { fieldName: 'AREA_M_NM', dataType: 'text', header: { text: t('장소2'), }, mergeRule: { criteria: "values['AREA_L_NM'] + values['AREA_M_NM']" }, width: '100', },
    { fieldName: 'AREA_S_NM', dataType: 'text', header: { text: t('장소3'), }, width: '120', },
...
const searchData = () => {
  ...
  return commonSearchApi({ queryId: 'SAFBC0020_SEARCH_01', param: searchParams })
}
...
const afterSearch = res => {
  ...
  for(let i = 0; i < dtSearch.value.length; i++) {
    ...
    if(!_.some(dtCnt, { JOB_LPLC: row.JOB_LPLC, JOB_MPLC: row.JOB_MPLC, JOB_SPLC: row.JOB_SPLC, })) {
      dtCnt.push({
        JOB_LPLC: row.JOB_LPLC,
        JOB_MPLC: row.JOB_MPLC,
        JOB_SPLC: row.JOB_SPLC,
        AREA_L_NM: row.AREA_L_NM,
        AREA_M_NM: row.AREA_M_NM,
        AREA_S_NM: row.AREA_S_NM,
  ...
  const sorted = _.sortBy(dtCnt, ['JOB_LPLC', 'JOB_MPLC', 'JOB_SPLC'])
...
const searchSub = async () => {
  ...
  const filterData = _.filter(dtSearch.value, row => {
    if(curr.fieldName === 'AREA_L_NM') {
      return row.JOB_LPLC === currData.JOB_LPLC
    } else if(curr.fieldName === 'AREA_M_NM') {
      return row.JOB_LPLC === currData.JOB_LPLC && row.JOB_MPLC === currData.JOB_MPLC
    } else {
      return row.JOB_LPLC === currData.JOB_LPLC && row.JOB_MPLC === currData.JOB_MPLC && row.JOB_SPLC === currData.JOB_SPLC
    }
  })
