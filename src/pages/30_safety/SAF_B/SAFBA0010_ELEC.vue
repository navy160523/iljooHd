<!--
  화면명 : SAFBA0010 / 위험작업허가 작성/승인 (일렉용)
...
import SAFBA0010PopUp1 from "@/pages/30_safety/SAF_B/SAFBA0010PopUp1_ELEC.vue"
...
const searchData = () => {
  ...
  return commonSearchApi({
    queryId: "SAFBA0010_ELEC_search01",
    param: searchPam,
  })
}
...
const setPrintElec = () => {
  ...
  for (let rowIdx of checkRows) {
    ...
    let param = {
      originalURL:
        getQDomain() +
        `#/ptw_qr?RCV_NO=${data.RCV_NO}&SHORT_WORK_NO=`,
    }
    ...
    commonShortenUrl(param).then((res) => {
      ...
      console.log("data.PSM_YN=" + data.PSM_YN)      

      let psmYn = data.PSM_YN
      if (psmYn == 'N') {
        // 일반 허가서
        reportName.value = "/manage/hse/RETA3011_ELEC1.ozr"
      } else {
        // PSM 허가서
        reportName.value = "/manage/hse/RETA3011_ELEC2.ozr"
      }
      
      params.value.push({ reportName: reportName.value, args: dataRort })

      console.log("data.RCV_NO=" + data.RCV_NO)
    })

    console.log("reportName=" + reportName.value)
    console.log("dataRort=" + dataRort)
...
const setPrintHHI = () => {
  ...
  for (let rowIdx of checkRows) {
    ...
    if (
      ...
      if (data.DANG_DIV_CD === "C01" && data.CF_DIV === "S") {
      ...
      if (data.DANG_DIV_CD === "C01" && data.CF_DIV === "C") {
        data.CF_DIV === "F" ? (visible = "Y") : (visible = "N")
...
const IndividualPermit = () => {
  ...
  vm.$swal({
    ...
    if (swalRes.isConfirmed === true) {
      ...
      if (!beforeCheck(rowData)) {
        return
      } else {
        saveParams.push(rowData)
      }

      console.log("urgetn_yn", urgetn_yn)
      console.log("cross_yn", cross_yn)
