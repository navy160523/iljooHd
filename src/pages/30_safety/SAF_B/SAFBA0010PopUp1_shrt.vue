...
const datainer =  ref<PTWdata>({
    ...
    DEPT_NM: userStore.asgnFullNm,        // 소속명
...
const setDataIner = async (cmpny_div: string, rcv_no: string | null)  =>{
  ...
  datainer.value.THREECHECKYN= "N"   // 3중점검대상
  ...
  datainer.value.WRK_PLAN_YN = res.WRK_PLAN_YN  // 작업계획서 여부
  ...
  datainer.value.VND_NAME = res.VND_NAME || ""            // 협력사코드 : 일반공사는 ''
  datainer.value.DAN_WORK_ID = res.DAN_WORK_ID || ""       // 단기공사 작업표준
  datainer.value.WORK_COMPLEX_CD = res.WORK_COMPLEX_CD || ""  // 단기공사 복합공정코드
  datainer.value.USER_ID = userStore.empNo           // 작성자
  datainer.value.WRK_DIV = res.WRK_DIV                // 단기공사 or 건설공사   
...
const openSelectPopup = (val:string): void => {
  ...
  if (val === '소속'){
    ...
    if(datainer.value.JOB_LPLC &&datainer.value.JOB_MPLC  && datainer.value.JOB_SPLC  ){
      // 소속부서 및 장소의 단기공사 PTW리스트에서 선택하는 팝업
      ptwPopup.value?.openPopup("Y") // 단기공사여부 Y
    }
    else{
      Message.warn("장소를 먼저 선택해주세요")
      return 
    }
    ...
    if (datainer.value.SHORT_WORK_NO){
      DanWrkPopup.value?.openPopup(datainer.value) // 단기공사 작업표준 팝업
    }
    else{
      Message.warn("단기공사번호를 입력해주세요")
      return 
    }

  }
  else if (val ==="허가서 불러오기"){
    rcvPopup.value?.openPopup('Y')
  }
  else if (val === "단기공사팝업"){
...
const onDeptSelected = (row:any):void => {
  ...
  datainer.value.DEPT_NM = row.DEPT_NM
...
// 단기공사 선택 로직
const dangiSelect = (row:any) => {
  console.log('row ', row)
  datainer.value.SHORT_WORK_NO = row.SHORT_WORK_NO
  datainer.value.VND_NAME = row.VND_NAME

}

const RtnWorkPaln = (res:any) => {
  console.log('res ', res)

  if(res.list[0].RES_KEY){
    datainer.value.WRK_PLAN_YN = 'Y'
  } else {
    datainer.value.WRK_PLAN_YN = 'N'
  }
}

const WrkSelected3 = (row:any) => {

  console.log('row ', row)

  if (!disabledSTATUS.value) {
    datainer.value.WRK_STD_NM = row.WRK_STD_NM
    datainer.value.WRK_STD_NO = row.WRK_STD_NO
...
const check_ess = (Pdatainner: PTWdata): boolean => {
  console.log("Pdatainner", Pdatainner)
    ...
    if (!Pdatainner.RESP_EMP_NM){ // 단기공사는 사번이 없을 수 있으므로 NM으로 체크 
...
<template>
  ...
  <div class="title-bar"> 단기공사 위험작업허가서(PTW)작성</div>
              ...
              <v-row>
                ...
                <v-col cols="12" xl="7" class="d-flex flex-row ">
                  ...
                  <i-input
                    ...
                    @keydown.enter="
                      (e) => {
                        openSelectPopup('단기공사팝업')
                      }
                    "
                    @click:appendInner="openSelectPopup('단기공사팝업')"
                    :readonly="disabledSTATUS"
                  />
                  <div class="d-flex align-center" style="margin-top: 20px; margin-left: 10px;">
                    업체명: {{ datainer.VND_NAME ? datainer.VND_NAME: "-"  }}
                  </div>
               </v-col>
              </v-row>
            </v-sheet>
          ...
          <v-col cols="12" lg="6" >
            <v-card class="popUp-input-Form">
              <v-card-title class="pa-0">
                ...
                <v-row class="py-2 ptwinfocol">
                  <v-col cols="12" md="4" class="d-flex align-center pa-0 pr-md-2">
                    <div class="ptwinfolabel">
                      작업책임자
                      <v-icon
                        class="required-icon"
                  ...
                  <v-col cols="12" md="4" class="d-flex align-center pa-0 px-md-2">
                    <div class="ptwinfolabel2">
                      공사담당자
                    </div>
                    <i-input
                      width=""
                  ...
                  <v-col cols="12" md="4" class="d-flex align-center pa-0 pl-md-2">
                    <div class="ptwinfolabel2">
                      작업인원
                      <v-icon
                        class="required-icon"
                ...
                <v-row class="py-2 ptwinfocol">
                  ...
                  <v-col cols="12" md="6" class="d-flex align-center pa-0 pl-md-2">
                    ...
                    <i-input
                      width=""
                      style="flex: 1; max-width: 100%;"
                      v-model="datainer.WRK_PLAN_YN"
                      readonly
                      @click="openSelectPopup('작업계획서')"
                      margin="0"
                    >
                    <template v-slot:append-inner>
                      <v-icon icon="mdi-paperclip" @click="openSelectPopup('작업계획서')"></v-icon>
                    </template>  
                    </i-input>
                  </v-col>
                </v-row>
