...
const reportName = ref('/manage/hse/SAFBC0010_v2.ozr')
const reportParam = ref([])
const showOz = ref(false)
const safba00101 = ref(null)
const CCP = ref(null)
const safeGroup = ref(false)
...
onMounted(() => {
  Promise.all([
    ...
    codeList.CMPNY_DIV = res[5].ORESULT_CUR
  })
  vm.$nextTick(()=>{
    menuTitle.value.setBtnProperty("btnExcel", "visible", false)
    
    // 안전 직원 그룹만 회사구분 콤보 활성화
    if (userStore.authGrpCd.includes('GRP00381')){
      safeGroup.value = false
    } else {
      safeGroup.value = true
    }
  ...
  (newValue) => {
    if (!newValue){
      ...
      Promise.all([
        ...
        commonSearchApi({
          queryId: 'searchDept3',
          param: { CMPNY_DIV: searchParams.CMPNY_DIV, BSNS_CD: '' },
        }), //1
      ]).then((res) => {  
...
const afterSearch = async (res) => {
  ...
  grdMain.value.getGridView().setCellStyleCallback(function (grid, dataCell) {
      ...
      if( clnmnm === 'WORK_DT' || clnmnm === 'WORK_TIME' ){
...
const selected = (row) => {
  ...
  searchParams.JOB_PLC = row[2].ALL_SYS_CD
  searchParams.JOB_PLC_NM = row[2].TXT
...
const clearCCP = () => { 
  searchParams.JOB_PLC = ''
  searchParams.JOB_PLC_NM = ''
}
...
//출력 관련 로직 시작
const print = () => {
  let sWORK_NO = searchParams.WORK_NO === '전체' ? '' : searchParams.WORK_NO
  let sSAF = searchParams.SAF === '전체' ? '' : searchParams.SAF
  let sPRINT_DIV = searchParams.PRINT_DIV === '전체' ? '' : searchParams.PRINT_DIV  
  ...
  reportParam.value = [
    ...
    'IN_WORK_NO=' + sWORK_NO,
    'IN_GBN=' + searchParams.GBN,
    'IN_SAF=' + sSAF,
    'IN_ALLDAY=' + searchParams.ALLDAY,
    'IN_PRINT_DIV=' + sPRINT_DIV,
    'IN_JOB_PLC=' + searchParams.JOB_PLC,
    'IN_THREECHECKYN=' + searchParams.THREECHECKYN,
  ]
  ...
  <v-card class="pa-0 fill-height">
    <div class="d-flex align-self-end justify-space-between pt-3 pl-3 pr-3 pb-0">    
      ...
      <div class="align-center d-flex mb-3">
        <v-btn class="mr-3" @click="onButtonsClick({id: 'btnSearch'})">조회</v-btn>
        <i-select
                :label="$t('출력조건')"
                width="180px"              
                placeholder="출력조건"
                v-model="searchParams.PRINT_DIV"
                :items="codeList.PRINT_DIV"
                item-value="COD"
                item-title="TXT"
                class="mr-0"
              />
        <v-btn class="ml-1" @click="onButtonsClick({id: 'btnPrint'})">출력</v-btn>
      </div>
    </div>    
        ...
        <v-sheet class="searchArea">
          ...
          <div class="d-flex">
            ...
            <i-input
              :label="$t('장소')"
              width="250px"
              v-model="searchParams.JOB_PLC_NM"
              readonly
            >
              <template v-slot:append-inner>
                <v-icon @click="openCCP" icon="mdi-magnify" />
                <v-icon
                  color="error"
                  @click="clearCCP"
            ...
            >
              <v-checkbox
                ...
                v-model="searchParams.THREECHECKYN"
              >
              </v-checkbox>
              <span style="font-size: 15px; font-weight: bold; color: red" class="mt-1"
