<!-- 
  화면명  : SLI 전체보기 (일렉용)
  설명    : 조회기간에 해당되는 부문 - 부서 -과별로 SLI 지수를 나타내기
-->
...
const searchParams = reactive({
  CMPNY_DIV: '200',  // userStore.cmpnyDiv
  BSNS_CD: '',
  DEPT_CD: '',
  PART_CD: '',
...
const cardData = reactive({ // 왼쪽 카드 데이터(부문 미선택시)
  P010: { L: [], R: [], OPEN : false, POINT: 0 }, // 조선
  P020: { L: [], R: [], OPEN : false, POINT: 0 }, // 해양에너지
  P030: { L: [], R: [], OPEN : false, POINT: 0 }, // 특수선
  P040: { L: [], R: [], OPEN : false, POINT: 0 }, // 엔진기계
  P050: { L: [], R: [], OPEN : false, POINT: 0 }, // 엔진기계
})
...
const rightResult = reactive({ // 오른쪽 카드 데이터
  ...
  DNG_RESULT_POINT: 0,
  CHK_RESULT_POINT: 0,
  SAF_RESULT_POINT: 0, // 자율안전
  TBM_RESULT_POINT: 0, // TBM
  ADD_RESULT_POINT: 0, // 칭찬쿠폰
  // ADD_RESULT_POINT4: 0, // 환경정화
  // ADD_RESULT_POINT5: 0, // 안전설계
  WEAK_LIST: [],
})
...
// 조선 : 안전수칙, 안전사고, 안전교육, 위험성평가, 안전점검
// 일렉 : 안전수칙, 안전사고, 안전개선활동, 자율안전, TBM, 안전교육, 안전업무
const safetyList = ref([
  ...
  { id: "CHK_RESULT_POINT", name: "안전점검" },
  { id: "SAF_RESULT_POINT", name: "자율안전" },
  { id: "TBM_RESULT_POINT", name: "TBM" },
  { id: "ADD_RESULT_POINT", name: "가점(칭찬쿠폰/환경정화활동/안전설계)" },
  // { id: "ADD_RESULT_POINT4", name: "환경정화&교통봉사" },
  // { id: "ADD_RESULT_POINT5", name: "안전설계" },
])
...
const searchData = () =>{
  ...
  return commonSearchApi({ queryId : 'SAFFA0010_ELEC_SEARCH_03', param: searchParams })
}
...
const afterSearch = res =>{
  ...
  for(let dt of res.ORESULT_CUR) {
    console.log("dt1=" + dt.DEPT_CD)
    console.log("dt2=" + dt.DEPT_NM)
    
    let bsnsPartCd = dt.PART_CD + '_' + dt.DEPT_CD
    
    // 처음엔 사업부 > 부문까지만 선택하고 부서를 선택하면 상세 부서보기로 이동하여 이렇게짰다....
    if(dt.PART_CD in data) {
      if(dt.DEPT_CD in data[dt.PART_CD]){
        data[dt.PART_CD][dt.DEPT_CD].SUM_TOT_POINT = Number(data[dt.PART_CD][dt.DEPT_CD].SUM_TOT_POINT) + Number(dt.TOT_POINT)
        data[dt.PART_CD][dt.DEPT_CD].ACC_POINT = Number(data[dt.PART_CD][dt.DEPT_CD].ACC_POINT) + Number(dt.ACC_POINT)
        data[dt.PART_CD][dt.DEPT_CD].EDU_POINT = Number(data[dt.PART_CD][dt.DEPT_CD].EDU_POINT) + Number(dt.EDU_POINT)
        data[dt.PART_CD][dt.DEPT_CD].RUL_POINT = Number(data[dt.PART_CD][dt.DEPT_CD].RUL_POINT) + Number(dt.RUL_POINT)
        data[dt.PART_CD][dt.DEPT_CD].DNG_POINT = Number(data[dt.PART_CD][dt.DEPT_CD].DNG_POINT) + Number(dt.DNG_POINT)
        data[dt.PART_CD][dt.DEPT_CD].CHK_POINT = Number(data[dt.PART_CD][dt.DEPT_CD].CHK_POINT) + Number(dt.CHK_POINT)
        data[dt.PART_CD][dt.DEPT_CD].SAF_POINT = Number(data[dt.PART_CD][dt.DEPT_CD].SAF_POINT) + Number(dt.SAF_POINT)
        data[dt.PART_CD][dt.DEPT_CD].TBM_POINT = Number(data[dt.PART_CD][dt.DEPT_CD].TBM_POINT) + Number(dt.TBM_POINT)
        data[dt.PART_CD][dt.DEPT_CD].CNT = Number(data[dt.PART_CD][dt.DEPT_CD].CNT) + 1
        data[dt.PART_CD][dt.DEPT_CD].TOT_POINT = Math.round(Number(data[dt.PART_CD][dt.DEPT_CD].SUM_TOT_POINT) / Number(data[dt.PART_CD][dt.DEPT_CD].CNT))
        data[dt.PART_CD][dt.DEPT_CD].TOT_GRADE = getGrade(data[dt.PART_CD][dt.DEPT_CD].TOT_POINT)
        if(!isEmpty(dt.WEAK_NM)) {
          for(var weak of dt.WEAK_NM.split('||')) {
            data[dt.PART_CD][dt.DEPT_CD].WEAK_LIST.push(weak)
          }
        }
      } else {
        data[dt.PART_CD][dt.DEPT_CD] = {
          ...
          SAF_POINT: Number(dt.SAF_POINT),
          TBM_POINT: Number(dt.TBM_POINT),
          CNT : 1,
          WEAK_LIST: [],
        }
        if(!isEmpty(dt.WEAK_NM)) {
          for(var weak of dt.WEAK_NM.split('||')) {
            data[dt.BSNS_CD][dt.DEPT_CD].WEAK_LIST.push(weak)
          }
        }
      }
    }
  ...
  Object.entries(data).forEach(([key, value]) => {
    ...
    Object.entries(value).forEach(([k, v]) => {
      if(cnt % 2 === 0) {
        cardData[v.PART_CD].L.push(v)
      } else {
        cardData[v.PART_CD].R.push(v)
      }
      cnt += 1
    })
  })
  // 조선, 해양에너지, 특수선, 엔진기계 카드데이터 
  // 실질적인 카드 그리기
  cardDataProcess()
}
...
const cardDataProcess = () => {
  ...
  if(stateSLI.value === 'total') {
    cardResult.value.push({NAME: '전력', CD: 'P010', DATA:cardData.P010, DATA2:data.P010})
    cardResult.value.push({NAME: '배전', CD: 'P020', DATA:cardData.P020, DATA2:data.P020})
    cardResult.value.push({NAME: '품질', CD: 'P030', DATA:cardData.P030, DATA2:data.P030})
    cardResult.value.push({NAME: '회전기', CD: 'P040', DATA:cardData.P040, DATA2:data.P040})    
    cardResult.value.push({NAME: '경영', CD: 'P050', DATA:cardData.P050, DATA2:data.P050})    
  } else {
  ...
  if(Object.keys(cardResult.value[0].DATA).length != 0) {
    ...
    if(cardResult.value[0].DATA[Object.keys(cardResult.value[0].DATA)[0]].DEPT_NM == null) 
      selectPartNm.value = cardName[stateSLI.value]
    else 
      selectPartNm.value = cardResult.value[0].DATA[Object.keys(cardResult.value[0].DATA)[0]].DEPT_NM
...
// 부문 클릭
// 일렉용 전환 : 기존에는 부문 클릭이었지만 일렉은 부서를 클릭했을때 오른족에
const clickPart = (part) => {
  selectPart.value = part.PART_CD + '_' + part.DEPT_CD
  //partResult.DATA = partData.DATA[part.PART_CD]
  console.log("partData.DATA=" + partData.DATA)
  partResult.DATA = partData.DATA[selectPart.value]
  selectDept.value = ''
  if(part.PART_NM == null) { 
    ...
    selectPartNm.value = part.DEPT_NM
    //rightResult.TITLE = part.BSNS_NM + ' ' + part.PART_NM
    // 부문 + 부서명으로 표현
    rightResult.TITLE = part.PART_NM + ' ' + part.DEPT_NM
  }
...
const clickDept = (dept) => {
  ...
  selectDept.value = dept.DEPT_CD
  rightResult.TITLE = dept.ASGN_NM // 과명
...
const makeRightData = () => {
  ...
  rightResult.SAF_POINT = 0
  rightResult.TBM_POINT = 0
  ...
  if(selectDept.value != '') { // 부서 선택
    for(var card of dbData.value) {
      if(selectDept.value == card.DEPT_CD) {
        ...
        rightResult.SAF_POINT = Number(rightResult.SAF_POINT) + Number(card.SAF_POINT)
        rightResult.TBM_POINT = Number(rightResult.TBM_POINT) + Number(card.TBM_POINT)
      }
    }
    ...
    for(var card of cardResult.value) {
      ...
      for(var dt of card.DATA.L) {
        ...
        rightResult.SAF_POINT = Number(rightResult.SAF_POINT) + Number(dt.SAF_POINT/dataCnt)
        rightResult.TBM_POINT = Number(rightResult.TBM_POINT) + Number(dt.TBM_POINT/dataCnt)
        bsnsCnt += 1
  ...
  if(scoreCnt === 0) {
    ...
    rightResult.SAF_RESULT_POINT = 0
    rightResult.TBM_RESULT_POINT = 0
  }
  else {
    ...
    rightResult.SAF_RESULT_POINT = oneRound(rightResult.SAF_POINT / scoreCnt)
    rightResult.TBM_RESULT_POINT = oneRound(rightResult.TBM_POINT / scoreCnt)
  }
  radarState.value.series[0].data[0].value = [rightResult.RUL_RESULT_POINT, rightResult.ACC_RESULT_POINT, rightResult.EDU_RESULT_POINT, rightResult.DNG_RESULT_POINT, rightResult.CHK_RESULT_POINT, rightResult.SAF_RESULT_POINT, rightResult.TBM_RESULT_POINT]
                ...
                <div class="btn-contents d-flex ma-4">
                  <v-hover v-for="(item, i) in cards" :key="i">
                    <template v-slot:default="{ isHovering, props }">
                      ...
                      >
                        <!-- 상단 카드명 나타내기 (전사, 전력, 배전, 품질, 회전기, 경영)-->
                        <p>{{ item.name  }} </p>
                      </VCard>
                    </template>
                  </v-hover>
                  ...
                  <div class="w-50 pa-4" style="overflow-y: auto; height:calc(100vh - (378px));">
                    ...
                    <div class="safe-card w-100 pa-4" >
                      ...
                      >
                        ...
                        <span class="text-card ml-4">{{ cardContent.ASGN_NM }}</span>
                        <span class="text-card ml-auto mr-4">{{ cardContent.TOT_POINT }}</span>
                      </div>
                    </div>   
                  </div>
