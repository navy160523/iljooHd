...
const searchData = () => {
  ...
  return commonSearchApi({ queryId : 'SAFFA0020_SEARCH_03', param: searchParams })
}
...
const afterSearch = res =>{
  ...
  for(var dt of res.ORESULT_CUR) {
    ...
    if(searchParams.DEPT_CD == dt.DEPT_CD) {
      resultData.DEPT_CD_POINT = dt.TOT_POINT
      resultData.PERSNAL_WEIGHT = dt.PERSONAL_WEIGHT
      resultData.GRADE_WEIGHT = dt.GRADE_WEIGHT
      resultData.GRADE = dt.GRADE
      ...
      // }
      resultData.WEAK_NM = dt.WEAK_NM
      resultData.WEAK_CNT = dt.WEAK_CNT
    }
  }
  ...
  resultData.DEPT_NM = matchNM(searchParams.DEPT_CD, codeList.DEPT_CD, 'DEPT_CD', 'DEPT_NM')
  ...
  Promise.all([
      ...
      for(var i = 0; i < resultBottom.EDU.length; i++) {
        if(resultBottom.EDU[i].EDU_TYPE == eduCal.EDU_TYPE) {
          if('CALC_COL' in resultBottom.EDU[i]) {
            ...
            if(resultBottom.EDU[i].EDU_TYPE === 'A-1' || resultBottom.EDU[i].EDU_TYPE === 'A-2') {
              resultBottom.EDU[i].CALC_COL = [eduCal.CALC_COL] 
            } else {
              resultBottom.EDU[i].CALC_COL = [eduCal.CALC_COL2] 
            }
            ...
            resultBottom.EDU[i].EDU_COUNT2 = eduCal.EDU_COUNT2
    ...
    console.log('resultBottom.DNG', resultBottom.DNG)

    let weakPoint = 0
    // 위험성평가 계산식
    // console.log('res[0].ORESULT_CHK_CAL', res[0].ORESULT_CHK_CAL)
    // console.log('resultBottom.CHK', resultBottom.CHK)
    for(var chkCal of res[0].ORESULT_CHK_CAL) {
      if(chkCal.CHK_TYPE === 'D') {
        chkBdGdCnt.BD_COUNT = chkCal.BD_COUNT
        chkBdGdCnt.GD_COUNT = chkCal.GD_COUNT
      }
...
const setResultPointCalc = () => {
  ...
  console.log('resultBottom.DNG_POINT', resultBottom.DNG_POINT)
  console.log('resultBottom.DNG', resultBottom.DNG)
  for(var i = 0; i < resultBottom.DNG.length; i++) {
  ...
  for(var i = 0; i < resultBottom.CHK.length; i++) {
    ...
    if(resultBottom.CHK_CALC.length > 0) {
...
const toggleChk = (i) => {
  // console.log(`resultBottom.CHK[${i}]`, resultBottom.CHK, resultBottom.CHK[i])
...
const movePageAcc = (acc) => {
  ...
  Promise.all([
    ...
    .then((res) => { 
      // console.log('paraMap', paraMap)
      // console.log('movePageAcc res', res)
      // console.log('res[0]', res[0])
      // console.log('res[0].ORESULT_CUR[0]', res[0].ORESULT_CUR[0])
...
const movePageRul = (rul) => {
  let paraMap = {
    ...
    DEPT_CD: searchParams.DEPT_CD,
    GUBUN: '',
    FROM_DT: dayjs(searchParams.TMP_TO_DT).subtract(30, 'day').format('YYYY-MM-DD'),//searchParams.TMP_FROM_DT,
    TO_DT: searchParams.TMP_TO_DT,//searchParams.TMP_TO_DT,
  }
  if (rul.RUL_TYPE == 'A' || rul.RUL_TYPE == 'B' || rul.RUL_TYPE == 'E' || rul.RUL_TYPE == 'F' || rul.RUL_TYPE == 'G') {
    if (rul.RUL_TYPE == 'A') paraMap.GUBUN = 'A'
    else if (rul.RUL_TYPE == 'B') paraMap.GUBUN = ''
    else if (rul.RUL_TYPE == 'E') paraMap.GUBUN = 'B'
    else if (rul.RUL_TYPE == 'F') paraMap.GUBUN = 'E'
    else if (rul.RUL_TYPE == 'G') paraMap.GUBUN = 'F'
...
const movePageEdu = (edu) => {
  if(edu.EDU_TYPE == 'A-1') {
    ...
    closePage('60_edu-EDU_A-EDUAA0010')
...
const movePageDng = (dng) => {
  ...
  if(dng.DNG_TYPE == 'A') {
    let dateObj = new Date(searchParams.TMP_TO_DT) 
    dateObj.setDate(dateObj.getDate() - 0) // -15일
    paraMap.TO_DT = dateObj.toISOString().slice(0, 10) 
    dateObj.setDate(dateObj.getDate() - 60) // -15 -30 = -45일
...
const movePageChk = (chk) => {
  ...
  .then((res) => { 
    // console.log('res', res)
    paraMap.FROM_DT = res[0].ORESULT_CUR[0].FROM_DT
    paraMap.FROM_DT2 = res[0].ORESULT_CUR[0].FROM_DT2
    paraMap.FROM_DT3 = res[0].ORESULT_CUR[0].FROM_DT3
    paraMap.TO_DT = res[0].ORESULT_CUR[0].TO_DT
    if(chk.CHK_TYPE == 'A') {
      ...
      paraMap.FROM_DT = paraMap.FROM_DT3
      paraMap.SAFE_YN = 'Y'
      closePage('30_safety-SAF_C-SAFCD0010')
      router.push({ path: '/30_safety/SAF_C/SAFCD0010', state: { sliSAFCD0010: { tab: 'DAILY' }, sliSAFCD0010Tab01: paraMap }})
    } else if(chk.CHK_TYPE == 'E') {
      paraMap.FROM_DT = dayjs(searchParams.TMP_TO_DT).subtract(7, 'day').format('YYYY-MM-DD')
      paraMap.TO_DT = dayjs(searchParams.TMP_TO_DT).format('YYYY-MM-DD')
      paraMap.YARD_APP_YN = 'Y'
      closePage('30_safety-SAF_C-SAFCD0010')
...
const getState = () => {
  ...
  if(!window.history.state.myDetail && window.history.state.data){ 
    ...
    // console.log('44', codeList.ALL_DEPT_CD)
    // if(codeList.ALL_DEPT_CD.indexOf(userStore.deptCd) >= 0) {
    if(codeList.ALL_DEPT_CD.findIndex(iDept => iDept.DEPT_CD === userStore.deptCd) >= 0) {
      // console.log('555')
      ...
      searchParams.PART_CD = codeList.ALL_DEPT_CD[codeList.ALL_DEPT_CD.findIndex(iDept => iDept.DEPT_CD === userStore.deptCd)].PART_CD
...
onMounted(() => {
  ...
  .then((res) => { 
    codeList.BSNS_CD = [
      ...
      {BSNS_NM:'함정·중형선사업부',BSNS_CD:'AE00'},
    ]
...
<template>
  <v-card class="pa-3">
    <v-card-title class="pa-2 py-0">
      ...
      >
        <div class="flex-grow-1 ml-2" style="font-size: 16px;">
          ※ 필요시 좌측 도움말(물음표) 버튼 클릭하여 SLI 세부 설명자료를 확인해 주시기 바랍니다.
        </div>
      </IMenuTitle>
    </v-card-title>
      ...
      <div class="d-flex flex-column fill-height">
            ...
            <div class="d-flex justify-space-between align-center">
              <div class="d-flex">
                <div class="d-flex align-center">
                  <span class="legend-title">SLI 등급 기준</span>
                ...
                <div class="ml-5">
                  <div class="d-flex" style="align-items: center;">
                    <div class="legend-title">감점/가점 항목 구분</div>
                    <div class="legend-additon-big legend-addition-sub ma-2"></div>
                    <span class="bottom-chip mr-2 my-2">감점항목</span>
                    <div class="legend-additon-big legend-addition-add ma-2"></div>
                    <span class="bottom-chip mr-2 my-2">가점항목</span>
              ...
              <div style="color: #000000; font-size: 16px;">
                ※ SLI 점수는 점심시간/자정 이후 업데이트(일 2회)
                <br/>
                ※ 점수 클릭시 세부지표별 점수 계산식 확인 가능
              </div>
            </div>
          </VCard>
        </v-sheet>
        <v-sheet no-gutters class="dashboard-contents mt-43">
          <div class="d-flex">
            <VCard
              color="inputDisabled"
              variant="outlined"
            ...
            >
              ...
              <div class="d-flex">
                ...
                <div class="safe-card pa-4 d-flex mr-2 h-grow align-center justify-space-between">
                  <div class="text-card2">현장 위험도</div>
                  <div class="text-card ml-auto"> {{ `${resultData.GRADE}등급(${resultData.GRADE_WEIGHT}%)` }}</div>
                </div>
                <div class="safe-card pa-4 d-flex mr-2 h-grow align-center justify-space-between">
                  ...
                  <div>
                    <v-chip v-if="resultData.WEAK_NM != null" color="error" label density class="mr-2">
                      {{ resultData.WEAK_NM }}
                    </v-chip>
                    <v-chip v-if="resultData.WEAK_CNT != null" color="error" label density>
                      {{ `반복 ${resultData.WEAK_CNT}회` }}
                    </v-chip>
                  </div>
                </div>
              </div>
        ...
        <v-sheet no-gutters class="mr-4 h-auto" style="height: 66% !important;">
          <VCard
            ...
            class="ma-2"
          >
            <div class="d-flex" style="height: 99%;">
              <div class="ma-5 bottom-card" style="width: 17%;">
                ...
                <div class="count-card mt-2 pa-2">
                  <div class="ma-2 mt-4 cursor-pointer" v-for="(acc,i) in resultBottom.ACC" :key="i">
                    <div class="mt-n2 mb-2" style="text-align:center; color: #515151;;" v-if="acc.ACC_TYPE == 'F'">--------------------------------</div>
                    <div class="d-flex" :class="[`type_${acc.ACC_TYPE}`]">
                      <div class="legend-additon mr-1" :class="setLegendColor(acc.ADDITION)" v-if="acc.ADDITION != ''"></div>
                      <span class="text-card3" @click="toggleAcc(i)">{{ acc.ACC_TYPE_NM }}</span>
                      <div class="d-flex ml-auto"> 
                        <span class="text-card4 w-50" @click="movePageAcc(acc)">{{ acc.ACC_COUNT }} 건</span>
                        <span class="text-card5 ml-auto" @click="toggleAcc(i)">{{ nullZero(acc.SUM_POINT) }} 점</span>
                      </div>
                    </div>
                  ...
                  <div class="ma-2">
                    <div class="text-label">※ 산재사고 : 산재(휴업일 3일 이상)</div>
                    <div class="text-label">※ 휴업사고 : 경미공상(직영), 휴업 3일 이상 사고(협력사)</div>
                    <div class="text-label">※ 일반사고 : 휴업 3일 미만 일반사고(직영/협력사), 일반 물적사고</div>
              ...
              <div class="ma-5 bottom-card" style="width: 17%;">
                ...
                <div class="count-card mt-2 pa-2">
                  <div class="ma-2 mt-4 cursor-pointer" v-for="(rul,i) in resultBottom.RUL" :key="i">
                    <div class="d-flex">
                      <div class="legend-additon mr-1" :class="setLegendColor(rul.ADDITION)" v-if="rul.ADDITION != ''"></div>
                      <template v-if="rul.RUL_TYPE_NM.indexOf('The Safe Care') > -1">
                        <span class="text-card3" @click="toggleRul(i)">The Safe Care<br/>핵심안전수칙</span>
                      </template>
                      <template v-else-if="rul.RUL_TYPE_NM.indexOf('크레인') > -1">
                        <span class="text-card3" @click="toggleRul(i)">크레인<br/>특별안전수칙</span>
                      </template>
                      <template v-else>
                        <span class="text-card3" @click="toggleRul(i)">{{ rul.RUL_TYPE_NM }}</span>
                      </template>
                      <div class="d-flex ml-auto">
                        <span class="text-card4 w-50" @click="movePageRul(rul)">{{ rul.RUL_COUNT }} 건</span>
                        <span class="text-card5 ml-auto" @click="toggleRul(i)">{{ nullZero(rul.SUM_POINT) }} 점</span>
                      </div>
                    </div>
                  ...
                  <div class="ma-2">
                    <div class="text-label">※ 수칙위반 조치결과 등록 시 반점 복구 (교통수칙은 감점 전체 복구)</div>
                    <div class="text-label">※ The Safe Care 핵심안전수칙은 감점 복구 없음</div>
                  </div>
                </div>
              </div>
              <v-divider
              ...
              <div class="ma-5 bottom-card" style="width: 26%;">
                <div class="d-flex">
                ...
                <div class="count-card mt-2 pa-2">
                  <div class="ma-2 mt-4 cursor-pointer" :class="subEduContent(edu)" v-for="(edu,i) in resultBottom.EDU" :key="i">
                    <div class="d-flex">
                      <div class="legend-additon mr-1" :class="setLegendColor(edu.ADDITION)" v-if="edu.ADDITION != ''"></div>
                      <span class="text-card3" @click="toggleEdu(i)">{{ edu.EDU_TYPE_NM }}</span>
                      <div class="d-flex ml-auto">
                        <template v-if="edu.EDU_TYPE === 'A-1' || edu.EDU_TYPE === 'A-2'">
                          <div @click="movePageEdu(edu)">
                            <span class="text-card4">{{ edu.EDU_COUNT }}</span>
                            <span class="text-card4" style="color: #ff0000;">{{ `(${edu.EDU_COUNT2})` }}</span>
                            <span class="text-card4">{{ ' 명' }}</span>
                          </div>
                          <span class="text-card5" @click="toggleEdu(i)">{{ nullZero(edu.SUM_POINT) }} 점</span>
                        </template>
                        <template v-else>
                          <span class="text-card4" @click="movePageEdu(edu)">{{ edu.EDU_COUNT }}</span>
                          <span class="text-card5" @click="toggleEdu(i)">{{ nullZero(edu.SUM_POINT) }} 점</span>
                        </template>
                      </div>
                    </div>
                  ...
                  <div class="ma-2">
                    <div class="text-label">※ 교육 미 이수의 경우 전월 1일 재직자 기준으로 매월 9일부터 감점 발생</div>
                    <div class="text-label">※ 교육계획 미 수립의 경우 금월 1일 재직자 기준으로 매월 16일부터 감점 발생</div>
                    <div class="text-label" style="color: #ff0000;">※ 괄호 정보는 감점 대상자 사전 정보 제공 (감점 1주 전)</div>
                  </div>
                </div>
              </div>
              <v-divider
                ...
                style="color: #d8d8d8; opacity: 1 !important; height:90%;"
              /> 
              <div class="ma-5 bottom-card" style="width: 22%;">
                <div class="d-flex">
                  <div class="bottom-title">위험성평가</div>
                  <p class="ml-auto">
                ...
                <div class="count-card mt-2 pa-2">
                  <div class="ma-2 mt-4 cursor-pointer" v-for="(dng,i) in resultBottom.DNG" :key="i">
                    <div class="d-flex">
                      <div class="legend-additon mr-1" :class="setLegendColor(dng.ADDITION)" v-if="dng.ADDITION != ''"></div>
                      <span class="text-card3" @click="toggleDng(i)">{{ dng.DNG_TYPE_NM }}</span>
                      <div class="d-flex ml-auto">
                        <template v-if="dng.DNG_TYPE === 'A'">
                          <div @click="movePageDng(dng)">
                            <span class="text-card4">{{ dng.DNG_COUNT }}</span>
                            <span class="text-card4" style="color: #ff0000;">{{ `(${dng.DNG_COUNT2})` }}</span>
                            <span class="text-card4">{{ ' 건' }}</span>
                          </div>
                          <span class="text-card5" @click="toggleDng(i)">{{ nullZero(dng.SUM_POINT) }} 점</span>
                        </template>
                        <template v-else>
                          <span class="text-card4 w-50" @click="movePageDng(dng)">{{ dng.DNG_COUNT }} 건</span>
                          <span class="text-card5 ml-auto" @click="toggleDng(i)">{{ nullZero(dng.SUM_POINT) }} 점</span>
                        </template>
                      </div>
                    </div>
                  ...
                  <div class="ma-2">
                    <div class="text-label">※ 안전사고 발생에 따른 수시 위험성평가 미실시의 경우 사고일 기준 +30일부터 감점 적용 (30일 간)</div>
                    <div class="text-label">※ 현장 위험성평가는 최근 1주일 평균 기준 적용(직영 근태 정보 연동)</div>
                    <div class="text-label" style="color: #ff0000;">※ 괄호 정보는 감점대상 사전 정보 제공 (감점 1주 전)</div>
                  </div>
                </div>
              </div>
              <v-divider
              ...
              <div class="ma-5 bottom-card">
                <div class="d-flex">
                  <div class="bottom-title">안전점검/개입</div>
                  <p class="ml-auto">
                ...
                <div class="count-card mt-2 pa-2">
                  <div class="ma-2 mt-4 cursor-pointer" v-for="(chk,i) in resultBottom.CHK.filter(row => { return row.CHK_TYPE != 'D' })" :key="i">
                    <div class="d-flex" v-if="chk.CHK_TYPE != 'C'">
                      <div class="legend-additon mr-1" :class="setLegendColor(chk.ADDITION)" v-if="chk.ADDITION != ''"></div>
                  ...
                  <div class="ma-2">
                    <div class="text-label">※ 고위험점검 항목은 최근 1주일(주말, 공휴일 포함) 누적 평균 적용되며, 안전요원 점검 실적 제외 기준</div>
                    <div class="text-label">※ 부서관리작업 실적의 경우 1일 단위로 적용</div>
                  </div>
                  <div v-if="resultBottom.CHK.length > 0" class="mt-4">
                    <div class="pannel__daily-check">
                      <div class="daily-check__title cursor-pointer" @click="movePageChk({ CHK_TYPE: 'D' })">
                        {{ `ⓘ 안전요원 일상점검 ${ (chkBdGdCnt.BD_COUNT * 1) + (chkBdGdCnt.GD_COUNT * 1) }건` }}
                      </div>
                      <div class="d-flex">
                        <div class="text-center daily-check__bad">
                          <span>불안전사례</span>
                          <br />
                          <span>{{ `${chkBdGdCnt.BD_COUNT}건` }}</span>
                        </div>
                        <div class="text-center daily-check__good">
                          <span>우수사례</span>
                          <br />
                          <span>{{ `${chkBdGdCnt.GD_COUNT}건` }}</span>
                        </div>
                      </div>
                      <div class="mt-1" style="font-size: 14px; color: #838383;">
                        ※ 최근 1개월 기준
                      </div>
                    </div>
                  </div>
