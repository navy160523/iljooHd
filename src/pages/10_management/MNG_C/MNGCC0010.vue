...
import MNGCC0010Popup from './MNGCC0010Popup.vue'
import Message from "@hiway/utils/notify"

defineOptions({
  name:'10_management-MNG_C-MNGCC0010',
})
...
const grdMainProps = reactive({
  ...
  fields : [ 
    ...
    { fieldName: 'HAEIM_DATE', dataType: 'text','displayCallback': function(grid, index, value){return value ? dayjs(value).format('YYYY-MM-DD') : null},
      ...
      editor: { type: 'date', datetimeFormat: 'yyyy-MM-dd', mask: { editMask: '9999-99-99', placeHolder: 'yyyy-MM-dd', includedFormat: true } } },
    { fieldName: 'CHANGE_DATE', dataType: 'text','displayCallback': function(grid, index, value){return value ? dayjs(value).format('YYYY-MM-DD') : null},
      width:'120', header: { text: t('변경일') }, width:'120', styleName: 'editable_column',
      editor: { type: 'date', datetimeFormat: 'yyyy-MM-dd', mask: { editMask: '9999-99-99', placeHolder: 'yyyy-MM-dd', includedFormat: true } } },
    ...
    { fieldName: 'FILE_ID_DEC', dataType: 'text', header: { text: t('신고서') }, styleName: 'editable_column', isFile: { value: true, align: 'right', showAlways: true },
      'displayCallback': function(grid, index, value){return value ? 'Y' : null}, editable: false },
    { fieldName: 'JAGYEC_DIV', dataType: 'text', header: { text: t('자격구분') }, width:'150', editable: false },
  ...
  columnLayout: [
    ...
    'COM_ENT_DATE','RETI_TMP_DATE','SONIM_DATE','HAEIM_DATE','CHANGE_DATE','FILE_ID_SON',
    {
      name: '선해임신고',
...
const onCellClicked = (grid, clickData) => {
  if(clickData.column === 'DIV_CD' && clickData.cellType === 'header' ){
    bunyaPop.value = true
  }
}

// file 업로드 팝업 호출
const popupOpen = (grid, index, col) => {
  fieldName.value = null
  fieldIndex.value = null
  grid.commit()
  let newRow = grdMain.value.getDataProvider().getRowState(index.dataRow)
  let rowData = grdMain.value.getDataProvider().getJsonRow(index.dataRow)

  if(index.fieldName === 'EMP_NM'){
    if(newRow === 'created'){
      addRowIndex.value = index.dataRow
      empPopup.value.openPopup({ EMP_NM: rowData.EMP_NM })
    } else {
      return false
    }

  }else if(index.fieldName === 'SONIM_DIV' || index.fieldName === 'FCTR_DIV'){
    historyPopup.value.openPopup(rowData)
  } else {
    // let data = grdMain.value.getDataProvider().getJsonRow(index.dataRow)
    // fieldName.value = col.fieldName
    // fieldIndex.value = index.dataRow
    // fileUploadPopup.value.openPopup(data[col.fieldName])
  }
}
...
const onCellDblClicked = (grid, clickData) => {
  if(clickData.column === 'SONIM_DIV' || clickData.fieldName === 'FCTR_DIV'){
    historyPopup.value.openPopup(searchParams)
  }
}
...
const beforeSave = async () => {
  ...
  if(chekedRow.length === 0){
    ...
    chekedRow.forEach( val => {
      ...
      data.CHANGE_DATE = data.CHANGE_DATE ? dayjs(data.CHANGE_DATE).format('YYYYMMDD') : null
...
const saveData = () => {
  return commonExecuteApi({ queryId: "MNGCC0010_SAVE_01", list: saveParam.value })
}

const deleteData = () => {  
  return commonExecuteApi({ queryId : 'MNGCC0010_DELETE_01', list: saveParam.value })
}
...
const onEditRowChanged = (grid, itemIndex, dataRow, field, oldValue, newValue) => {
  if( field === 6) {
    ...
    grid.commit()
    let rowData = grdMain.value.getDataProvider().getJsonRow(dataRow)
    grdMain.value.getDataProvider().setValue(itemIndex, 'JAGYEC_DIV', rowData.SONIM_JAGYEC)
  }
}
...
const printSet = () => {
  grdMain.value.getGridView().exportGrid({
    ...
    hideColumns: ['FILE_ID_SON','DEC_DATE','FILE_ID_DEC','JAGYEC_DIV','FILE_ID_JA','SUDANG_YN','EDU_DIV','EDU_INS', 'COM_ENT_DATE', 'DIV_CD', 'SONIM_INWON', 'CHANGE_DATE'
  ...
  <v-card class="pa-0 fill-height">
    <v-card-title class="pa-3 pb-0">
      <IMenuTitle
        ...
        ref="menuTitle"
        :button-list="['btnSearch', 'btnCreate', 'btnUpdate', 'btnDelete', 'btnExcel']"
    ...
    <v-card-text class="pa-3 pt-0 content-area">
      <div class="d-flex flex-column fill-height">
        ...
        <v-sheet class="searchArea d-flex">
          <div class="d-flex align-center pr-8 sonim" style="width: 550px;">
            <div class="formLabelText" style="width: 80px;">
              {{ $t('선임 구분') }}
            </div>
            <VAutocomplete 
...
::v-deep(.sonim > .v-input > .v-input__control) {
  width: 420px;
}
</style>
