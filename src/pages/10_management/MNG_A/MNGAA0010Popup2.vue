<!--
  화면명 : MNGAA0010Popup2.vue
  화면개요 :HSE 경영시스템 모니터링 -> HSE 실적 팝업
-->
<script setup>
...
const searchParams = reactive({
  ...
  HALF_QUARTER: Math.floor((dayjs().get("month")+1)/6)+1 + '',
  QUARTER: '',
  YEAR: dayjs().get("year"),
  ORGN_DIV: 'A',
  PROD_DIV: '',
...
const grdSub1Props = reactive({
  ...
  field: [
    ...
    { fieldName: "VEND_NM", dataType: "text", styleName: "left-column", editable: false, header: { text: t("협력사") } },
    { fieldName: "CNT_NONE", dataType: "text", width: '70', editable: false, header: { text: t("미작성") } },
    { fieldName: "CNT_CNFM", dataType: "text", width: '70', editable: false, header: { text: t("완료") } },
...
const searchData = () => {
  ...
  // searchParams.QUARTER = searchParams.HALF_QUARTER === '1' ? ('1,2') : ('3,4')
  searchParams.QUARTER = searchParams.HALF_QUARTER === '1' ? '1,2' : '3,4'
  console.log('searchParams.QUARTER ', searchParams.QUARTER)
  return commonSearchApi({ queryId : 'MNGAA0010_POPUP_02_SEARCH_01', param: searchParams }) 
}
...
const onGrdMainCellClicked = async (grid, oldRow, newRow) => {
  ...
  searchParams2.HALF_QUARTER = searchParams.HALF_QUARTER
  searchParams2.QUARTER = searchParams.QUARTER
  searchParams2.BSNS_NM = selectedGrdMain.BSNS_NM
   
  commonSearchApi({ queryId : 'MNGAA0010_POPUP_02_SEARCH_02', param: searchParams2 }).then( res => {  
...
watch(() => [searchParams.YEAR, searchParams.HALF_QUARTER, searchParams.ORGN_DIV, searchParams.PROD_DIV],([newValueYEAR, newValueHALF_QUARTER, newValueORGN_DIV, newValuePROD_DIV]) => {
  grdMain.value.getDataProvider().setRows()
  grdSub1.value.getDataProvider().setRows()
})
          ...
          <v-sheet class="searchArea flex-column">
            <div class="d-flex">
              ...
              <i-select 
                ...
                labelWidth="35px"
                width="170px"
                v-model="searchParams.HALF_QUARTER"
                :items="codeList.HALF_QUARTER"
                item-value="value"
                item-title="label"
              />
