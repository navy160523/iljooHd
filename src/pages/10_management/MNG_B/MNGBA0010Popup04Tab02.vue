...
import SAFCD0010Tab02Popup01 from '../../30_safety/SAF_C/SAFCD0010Tab02Popup01.vue'
...
const defaultDate = () => {
  ...
  //FROM: -6일 ~ TO : 오늘날짜
  let date = dayjs()
  let dateFrom = dayjs().subtract(6, 'day')
...
const onGrdMainCurrRowChanged = (grid, oldIdx, newIdx) => {
  ...
  commonSearchApi({
    ...
    if (res.ORESULT_CUR.length > 0) {
      ...
      let groupCode = res.ORESULT_CUR.filter((x) => userStore.authGrpCd.includes(x.GU_ID)).map(
        (x) => x.GU_ID
      ) //로그인유저 권한그룹과 계획점검에 등록된 권한을 비교하여 일치하는 것만 추출
      if (empNo.length > 0 || groupCode.length > 0) {
        //권한또는 인원에 등록되어 있으면 등록버튼 활성화
      ...
      <div class="d-flex flex-column fill-height">
        ...
        <div class="h-grow">
          ...
          <v-sheet width="60%" class="h-auto">
            <IGridTitle title="계획점검 결과"> </IGridTitle>
            <RealGrid ref="grdSub" :fields="grdSubProps.fields" :columns="grdSubProps.columns" />
          </v-sheet>
        </div>
      </div>
    </v-card-text>
