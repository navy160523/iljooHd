...
defineOptions({
  name:'10_management-MNG_E-MNGEA0020',
})
...
const deleteMainData = () =>{
  ...
  return commonExecuteApi({ queryId : 'MNGEA0020_DELETE_01', list: deleteList })
}
...
const validationChk = () =>{
  ...
  dataRows.forEach(row => {
    if(checkSys.includes(row.ALL_SYS_CD)){
      Message.warn(t(row.SYS_CDNM + '의 코드가 중복값입니다. 코드값을 수정해 주세요.'))
...
const saveMainData = () => {
  ...
  return commonExecuteApi({ queryId : 'MNGEA0020_SAVE_01', list: saveList })
}
...
const addChildCode = async () =>{
  ...
  if(getTreeData.getAncestors(currentRowIdx).length > 1){
    Message.warn(t('3레벨을 초과하는 항목은 추가할 수 없습니다.'))
...
const addCode = () =>{
  ...
  let currentRow = getTreeData.getJsonRow(currentRowIdx)
  let newRow ={
    ALL_SYS_CD: parentRow < 0 ? 'HHI' : currentRow.ALL_UP_CD,
    ALL_UP_CD : parentRow < 0 ? 'HHI' : currentRow.ALL_UP_CD,
    CODE_DESC1: '',
    CODE_DESC2: '',
    CODE_DESC3: '',
    CODE_DESC4: '',
    ...
    SYS_CD: '',
    SYS_CDNM: '',
    UP_CD: parentRow < 0 ? 'HHI' : currentRow.UP_CD,
    USE_FLAG: "Y",
    VIEW_INDEX: '',
  }
...
const searchMainData = () => {  
  ...
  return commonSearchApi({ queryId : 'MNGEA0020_SEARCH_01', param: searchParam })
}
