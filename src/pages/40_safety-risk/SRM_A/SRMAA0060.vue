...
import SRMAA0060Popup01 from '@/pages/40_safety-risk/SRM_A/Popup/SRMAA0060Popup01.vue'
import SRMAA0060Popup02 from '@/pages/40_safety-risk/SRM_A/Popup/SRMAA0060Popup02.vue'
import SRMAA0030Popup01 from '@/pages/40_safety-risk/SRM_A/SRMAA0030Popup01.vue'

defineOptions({
  name: '40_safety-risk-SRM_A-SRMAA0060',
})
...
const grdProps1 = reactive({
  ...
  fields: [
    { fieldName: 'TASK_NO_3', dataType: 'text', header: { text: t('공모전 관리번호') }, width: '125', },
    { 
      fieldName: 'THUMBNAIL', header: { text: t('썸네일') },
      renderer: { width: '290', type: 'image', imageField: 'THUMBNAIL', imageHeight: 150, },
      width: '290',
    },
    { fieldName: 'RISK_NM', dataType: 'text', header: { text: t('제목') }, styleName: 'left-column multiline-editor', width: '350', },
    { fieldName: 'INSERT_DATE', dataType: 'text', header: { text: t('등록일자') }, width: '100', },
    { fieldName: 'EXCELLENT_NM', dataType: 'text', header: { text: t('우수 제보 선정') }, width: '100', },
    { fieldName: 'CONTEST_DIFFICULTY_NM', dataType: 'text', header: { text: t('위험등급') }, width: '100', },
    { fieldName: 'TASK_SEQ', dataType: 'text', header: { text: t('통합관리번호') }, width: '100', },
    { fieldName: 'RISK_REVIEW_RESULT_NM', dataType: 'text', header: { text: t('개선 진행상태') }, width: '170', },
    { fieldName: 'EXCELLENT_REMARK', dataType: 'text', header: { text: t('안전의견 검토') }, styleName: 'left-column multiline-editor',
      width: '330', },
    { fieldName: 'CONTEST_VIEWS', dataType: 'text', header: { text: t('조회수') }, width: '80', },
  ],
  columns: [],
})
...
const searchData = () => {
  return commonSearchApi({ queryId: 'SRMAA0060_SEARCH_02', param: searchParams })
}
...
const searchOnOff = () => {
  commonSearchApi({ queryId: 'SRMAA0060_SEARCH_01', param: { CMPNY_DIV: searchParams.CMPNY_DIV } }).then(res => {
    searchParams.ONOFF = res.ORESULT_CUR[0].CODE_DESC1
  })
}

const excelData = () => {
  const excelFileNM = searchParams.FROM_DT + '~' + searchParams.TO_DT + ' 리스크 공모전 운영'
...
onMounted(() => {
  nextTick(() => {
    ...
    menuTitle.value.setBtnProperty('btnUpdate', 'text', '관리')
  ...
  <v-card class="pa-0 fill-height">
    <v-card-title class="pa-3 pb-0">
      <IMenuTitle
        ...
        :button-list="['btnSearch2', 'btnUpdate', 'btnSearch', 'btnDelete', 'btnPrint']" 
      ...
      <div class="d-flex flex-column fill-height">
        <v-sheet class="searchArea flex-column">
          <!-- <div class="d-flex justify-space-between align-center"> -->
            ...
            <!-- <div :class="`light-div light__${searchParams.ONOFF}`" />
          </div> -->
        </v-sheet>
    ...
    <SRMAA0060Popup01 ref="onOffPop" @close="closeOnOff" />
    <SRMAA0060Popup02 ref="dashPop" />
