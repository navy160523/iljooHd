<!--
  화면명 : 작업중지 해제인원 조회(팝업)
  화면개요 : 작업중지 해제인원을 조회하고 데이터를 선택하여 대상자 목록 그리드에 데이터를 추가한다.
-->
<script setup>
...
const initCodeList = () => {
  let searchParams = {
    ...
    YYYYMM : popupData.SAFEDU_YM
  }
  commonSearchApi({ queryId : 'EDUAB0030_TAB01_POP01_POP03_SEARCH_01', param : searchParams }).then(res => {
    console.log("레스 확인",res)
    popupData.SAFEDU_DT = ''
    codeList.safeduDt = res.ORESULT_CUR
    popupData.SAFEDU_DT = codeList.safeduDt[0].SAFEDU_FIN_DT
  })
}
...
const openPopup = data => {
  ...
  gridRefresh()
  popupData.SAFEDU_YM = dayjs(new Date()).format('YYYY-MM')
  //grdMainSearch()
  dialog.value = true
}
...
const grdMainSearch = () => {
  let searchParams = {
    CMPNY_DIV : userStore.cmpnyDiv,
    YYYYMM : popupData.SAFEDU_YM,
    YYYYMMDD : popupData.SAFEDU_DT,
  }
  commonSearchApi({ queryId : 'EDUAB0030_TAB01_POP01_POP03_SEARCH_02', param : searchParams }).then(res => {
    console.log("메인 그리드 조회 결과 :::: >>>>",res)
          ...
          <div style="height:100%">
            <v-sheet class="d-flex searchArea">
              <i-input 
                ...
                :label="$t('이수년월')" 
                type="month"
                v-model="popupData.SAFEDU_YM"
                @update:model-value="gridRefresh('SAFEDU_YM')"
              />
              <i-select 
                labelWidth="80px"
                width="230px"
                :label="$t('센터이수일')"
                v-model="popupData.SAFEDU_DT"
                :items="codeList.safeduDt"
                item-title="SAFEDU_FIN_DT"
                item-value="SAFEDU_FIN_DT"
                @update:model-value="gridRefresh('SAFEDU_FIN_DT')"
              />
            </v-sheet>
            <v-sheet width="100%" height="90%" class="flex-column">
              <RealGrid
                ref="grdMain"
