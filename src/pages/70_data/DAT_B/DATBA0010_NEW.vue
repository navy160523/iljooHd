...
import DATBA0010PopupReq from "./DATBA0010PopupReq.vue"
import DATBA0010PopupDetail from "./DATBA0010PopupDetail.vue"
import DATBA0010PopupValid from "./DATBA0010PopupValid.vue"
import IUploadPopup from "@/components/popup/IUploadPopup.vue"

defineOptions({
  name:'70_data-DAT_B-DATBA0010_NEW',
})
...
const dATBA0010PopupReq = ref(null)
const dATBA0010PopupDetail = ref(null)
const dATBA0010PopupValid = ref(null)
const fileUploadPopup = ref(null)
const gubun1All = ref('Y') 
...
const grdMainProps = reactive({
  ...
  fields : [ 
    { fieldName: 'GUBUN1NM', dataType: 'text', width: '100', header: { text: t('적용사업부') }, editable:false },
    { fieldName: 'GUBUN2NM', dataType: 'text', width: '80', header: { text: t('작업구분') }, editable:false },
    ...
    { fieldName: 'FILE_CNT', dataType: 'text', header: { text: t('첨부자료 [건]') },
    ...
    { fieldName: 'VAILD_STATUS_NM', dataType: 'text', width: '100', 
      editable: false,      
      header: { text: t('타당성확인') }, 
      styleCallback: function(grid,dataCell){
        let ret = {}
        let targetData = grid.getValue(dataCell.index.itemIndex, "VAILD_STATUS_NM")

        if(targetData == '타당성확인완료'){
          // 위험성평가 미실시
          ret.styleName = "button-green"
        }
        else if (targetData == '개정요망') {
          // 
          ret.styleName = "button-blue"
        }
        else if (targetData == '폐기검토 중') {
          ret.styleName = "button-orange"
        }
        else if (targetData == '폐기') {
          ret.styleName = "button-grey"
        }
        else if (targetData == '폐기반려') {
          ret.styleName = "button-green"
        }
        else if (targetData == '작성중') {
    ...
    { fieldName: 'VAILD_COLOR', dataType: 'text' , visible: false },
...
const onButtonsClick = async btn => {
  if (btn.id === 'btnRequest') {
    dATBA0010PopupReq.value.openPopup(null, 'N', 'NEW')
  } 
...
const searchData = () =>{
  ...
  if(codeList.SEARCH_GBN1.length === searchParams.GUBUN1.length){
    sendParams.GUBUN1 = ''
  }

  return commonSearchApi({ queryId : 'DATBA0010_SELECT_01', param: sendParams })
}
...
const status = () => {
  grdMain.value.getGridView().setRowStyleCallback(function(grid, item, fixed) {
    let ret = {}
    let targetData = grid.getValue(item.index, "VAILD_COLOR")

    if(targetData === 'Y'){ //타당성확인대상
...
const onCellItemClicked = (grid, index, col) => {
  ...
  else if (col.fieldName === 'REQUEST') {
    let data = grdMain.value.getDataProvider().getJsonRow(col.dataRow)

    dATBA0010PopupReq.value.openPopup(data, userLogStore.isMenuAdmin, 'REV')
  }
  else if (col.fieldName === 'FILE_CNT') {
    fileUploadPopup.value.openPopup(data.FILEID)
  }
  else if (col.fieldName === 'VAILD_STATUS_NM') {
    dATBA0010PopupValid.value.openPopup(data, userLogStore.isMenuAdmin)
  }
}
...
const onCellDblClicked = (grid, clickData) => {
  ...
  if (clickData.cellType !== "header" && clickData.fieldName != "REQUEST" && clickData.fieldName != "FAVORITE_CHK") {
    dATBA0010PopupDetail.value.openPopup(row, userLogStore.isMenuAdmin)
  }
}

const gubun1AllClick = e => {
  grdMain.value.getDataProvider().setRows([])
  if(gubun1All.value === 'Y'){
    gubun1All.value = 'N'
  } else {
    gubun1All.value = 'Y'
  }
...
const gubun1SelectAll = () => {
  grdMain.value.getDataProvider().setRows([])
  if(gubun1All.value === 'Y'){
    searchParams.GUBUN1 = []
    searchParams.GUBUN1 = codeList.SEARCH_GBN1
  } else {
    searchParams.GUBUN1 = []  
  }
}
  ...
  <v-card class="pa-3 fill-height">
    <v-card-title class="pa-2 py-0">
      <IMenuTitle
        ...
        :button-list="['btnRequest', 'btnSearch']"
    ...
    <v-card-text class="pa-2 pt-0 content-area">
        ...
        <v-sheet class="searchArea flex-column">
          <div class="d-flex">
            ...
            <i-select
              ...
              width="235px"
            />          
            <div class="d-flex align-center ml-2" style="width:500px">
              ...
              <VAutocomplete
                ...
                multiple
                clearable
                @update:model-value="handleSelect('GUBUN1')"                             
              >
                <template v-slot:prepend-item>
                  <v-list-item 
                    :class="gubun1All === 'Y' ? 'select' : 'unselect'"
                    @click="gubun1AllClick"
                  >
                    <v-list-item-title>
                      <VRow align="center">
                        <VCol cols="auto pl-1">
      ...
      <!-- 안전지침서관리 팝업 -->      
      <DATBA0010PopupDetail
        ref="dATBA0010PopupDetail" 
        @closed="afterPopupClosed"
      />      
      <!-- 안전지침서 제개정 요청 팝업 -->
      <DATBA0010PopupReq
        ref="dATBA0010PopupReq"
        editor-id="300"
        @closed="afterPopupClosed"
      />
      <IUploadPopup 
