<!-- 
  화면명 : 대관/대노조 공문관리
  화면개요 : 대관/대노조 공문 자료 조회및 추가, 수정, 삭제, 등록, 메일발송 화면
-->
<script setup>
import { ref, reactive, onMounted, getCurrentInstance } from "vue"
import { getCodeList, commonSendApi, getCompanyList } from "@hiway/api/commonApi"
...
import DATCA0080Popup from "./DATCA0080Popup.vue"
import DATCA0080Popup02 from "./DATCA0080Popup02.vue"
defineOptions({
  name: "70_data-DAT_C-DATCA0080",
})
...
const initCodeList = () => {
  ...
  Promise.all([
    ...
    codeList.company.unshift({COD: '', TXT: '전체'})
  })
}
...
const onButtonsClick = (btn) => {
  if (btn.id === "btnSearch") {
    ...
    Popup.value.openPopup({ CMPNY_DIV: searchParams.CMPNY_DIV })
    ...
    MailPopup.value.openPopup(popupData)
  }
}

const searchData = () => {
  console.log(searchParams)
  return commonSearchApi({ queryId: "DATCA0080_SEARCH_01", param: searchParams })
}
...
<template>
  <v-card>
    <v-card-title>
      <IMenuTitle
        ...
        :button-list="['btnSearch', 'btnCreate', 'btnSendMail']"
  ...
  <DATCA0080Popup ref="Popup" @after-search="() => onButtonsClick({ id: 'btnSearch' })"></DATCA0080Popup>
  <DATCA0080Popup02 ref="MailPopup" @after-search="() => onButtonsClick({ id: 'btnSearch' })"></DATCA0080Popup02>
</template>
