...
const beforeSave = () => {
  ...
  if (!saveParam[0].YOUTUBE_LINK) {
    Message.warn("유튜브 링크를 입력 해주세요.")
    return false
  }
  return true
}
...
const linkCopy = () => {
  let isCopyLink = document.execCommand('copy')

  if(isCopyLink){
    Message.success(t('링크가 복사 되었습니다.'))
  }
}
        ...
        <div class="d-flex flex-column fill-height">
          <v-sheet class="searchArea flex-column">
            ...
            <div class="mt-2 d-flex">
              <i-textarea
                :label="$t('유튜브 링크')"
                v-model="saveParam[0].YOUTUBE_LINK"
                topLabel
                width="95%"
                :row="3"
                :readonly="!isAdmin"
              ...
              <div class="align-self-center">
                <v-icon class="cursor-pointer" @click="linkCopy">
                  mdi-content-copy
                </v-icon>
              </div>
            </div>
          </v-sheet>
          <v-sheet class="flex-column" v-if="!props.isReadonly">
            <div class="d-flex mt-2">
              <v-file-input
