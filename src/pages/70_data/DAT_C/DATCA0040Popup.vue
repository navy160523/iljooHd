...
const cmpnyGubun = reactive([
  { COD: "MNZFEC11001Y", TXT: "우리회사" },
  { COD: "MNZFEC11002Y", TXT: "동종업종" },
])
...
const openPopup = (gbn) => {
  if (gbn && (gbn.NOTICE_DIV === "MNZFEC11002Y" || gbn.NOTICE_DIV === "MNZFEC11001Y")) {
    criticalcase.value = true
    cmpnyDivReadOnlyValue.value = true
  ...
  } else if (gbn && (gbn.NOTICE_DIV === "MNZFEA11001Y" || gbn.NOTICE_DIV === "MNZFED11001Y")) {
    ...
    /* ********************* 신규 등록 ******************* */
    if (typeof gbn === "boolean") {
      criticalcase.value = true
    } else if (gbn === "MNZFEA11001Y") { // 사고즉보
      visibleType.value = true
    }
    if (typeof gbn === "string") {
      saveParam[0].NOTICE_DIV = gbn
    }
...
const search = () => {
  ...
  commonRequest("/hse/data/DATCA0030_SEARCH_02", { queryId: "DATCA0030_SEARCH_02", param: searchParam })
    ...
    .finally(() => {
      /* ****************** 조회건수 저장 ****************** */
      commonExecuteApi({ queryId: "DATCA0040_SAVE_02", list: saveParam })
...
const beforeSave = () => {
  ...
  if (!saveParam[0].NOTICE_DIV) {
    Message.warn("종류를 입력 해주세요.")
    return false
  }
...
const saveData = () => {
  ...
  let result = commonRequest("/hse/data/DATCA0030_SAVE_01", { queryId: "DATCA0040_SAVE_01", list: saveParam })
  result.then((res) => {
    /* **************************** 신규 등록일때 담당자한테 메일 발송 (사용안함) **************************** */
    // if (isCreate.value && toUpper(saveParam[0].NOTICE_DIV) === "MNQ00010" && res.NOTICE_NO) {
    //   commonSearchApi({ queryId: "DATCA0040_SEARCH_04", param: { CMPNY_DIV: userStore.cmpnyDiv } }).then((res2) => {
...
/* ********** 삭제 ********** */
const deleteData = () => {
  return commonExecuteApi({ queryId: "DATCA0040_DELETE_01", list: saveParam })
}
...
const closePopup = () => {
  ...
  emit("after-search")
  cmpnyDivReadOnlyValue.value = false
  visibleType.value = false
  criticalcase.value = false
        ...
        <div class="d-flex flex-column fill-height">
          <v-sheet class="searchArea flex-column">
            ...
            <div class="d-flex mt-4">
              ...
              /> -->
              <i-select
                :readonly="cmpnyDivReadOnlyValue"
                required
                v-if="criticalcase"
                width="200px"
                label-width="80px"
