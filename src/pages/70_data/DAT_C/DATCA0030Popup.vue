...
const openPopup = (gbn) => {
  if (gbn) {
    ...
    setDisableButton(gbn.NOTICE_REG_EMPNO, gbn.KOR_NM)
    if (userStore.userId === gbn.NOTICE_REG_EMPNO || userStore.userName === gbn.KOR_NM) {
...
const search = () => {
  commonRequest("/hse/data/DATCA0030_SEARCH_02", { queryId: "DATCA0030_SEARCH_02", param: searchParam })
    ...
    .finally(() => {
      /* ****************** 조회건수 저장 ****************** */
      commonExecuteApi({ queryId: "DATCA0030_SAVE_02", list: saveParam })
...
/* ********** 삭제 ********** */
const deleteData = () => {
  return commonExecuteApi({ queryId: "DATCA0030_DELETE_01", list: saveParam })
}
...
const setDisableButton = (userId, KOR_NM) => {
  if (userId != userStore.userId || userStore.userName != KOR_NM) {
