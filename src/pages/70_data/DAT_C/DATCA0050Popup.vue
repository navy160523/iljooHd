...
const openPopup = (gbn) => {
  ...
  if (gbn) {
    ...
    saveParam[0].NOTICE_DIV = props.id
    isCreate.value === true
  }
  dialog.value = true
...
const search = () => {
  commonRequest("/hse/data/DATCA0030_SEARCH_02", { queryId: "DATCA0050_SEARCH_02", param: searchParam })
    ...
    .finally(() => {
      /* ****************** 조회건수 저장 ****************** */
      commonExecuteApi({ queryId: "DATCA0050_SAVE_02", list: saveParam })
      checkFileId()
    })
}
...
const saveData = () => {
  ...
  let result = commonRequest("/hse/data/DATCA0030_SAVE_01", { queryId: "DATCA0050_SAVE_01", list: saveParam })
  result.then((res) => {
    /* **************************** 신규 등록일때 담당자한테 메일 발송 (사용안함) **************************** */
    // if (isCreate.value && toUpper(saveParam[0].NOTICE_DIV) === "MNQ00010" && res.NOTICE_NO) {
    //   commonSearchApi({ queryId: "DATCA0050_SEARCH_04", param: { CMPNY_DIV: userStore.cmpnyDiv } }).then((res2) => {
...
/* ********** 삭제 ********** */
const deleteData = () => {
  return commonExecuteApi({ queryId: "DATCA0050_DELETE_01", list: saveParam })
}
...
const sortValidation = () => {
  saveParam[0].SORT_NO = saveParam[0].SORT_NO.replace(/[^0-9]$/g, "")
}
</script>
