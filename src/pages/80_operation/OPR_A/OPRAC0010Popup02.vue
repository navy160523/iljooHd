...
const grdMainProps = reactive({
  ...
  fields : [ 
    { fieldName: 'SYS_CD', dataType: 'text', width: '120', header: { text: t('시스템 코드') }, styleName: 'left-column', width : '100' },
    { fieldName: 'SYS_CDNM', dataType: 'text', width: '120', header: { text: t('장소명') }, styleName: 'left-column', width : '170',
    } ,    
...
const grdSubProps = reactive({
  ...
  keys: [],
  fields: [
    { fieldName: 'SYS_CD', dataType: 'text', width: '80', header: { text: t('시스템코드') }, editable: false },
    { fieldName: 'SYS_CDNM', dataType: 'text', width: '120', header: { text: t('장소명') }, styleName: 'left-column', width : '170', editable: false },    
    ...
    { fieldName: 'HND_PHN', dataType: 'text', header: { text: t('휴대폰') }, visible: false },
    { fieldName: 'TRS', dataType: 'text', header: { text: t('무전기코드') }, visible: false },
    { fieldName: 'LOC_HEADER_NO', dataType: 'text', header: { text: t('지역장') }, visible: false },
...
const openPopup = (param) => {
  ...
  searchParams[0].HND_PHN = ''
  searchParams[0].TRS = ''
  searchParams[0].LOC_HEADER_NO = ''
...
const searchMainData = () => { 
  let searchParam = { CMPNY_DIV: userStore.cmpnyDiv, REL_DEPT_CD: '', REL_GWA_CD:'', REL_TEAM_CD: '', NIGHT_YN: ''}
  searchParam.REL_DEPT_CD = searchParams[0].REL_DEPT_CD
  searchParam.REL_GWA_CD = searchParams[0].REL_GWA_CD
  searchParam.REL_DEPT_CD = searchParams[0].REL_DEPT_CD
  searchParam.NIGHT_YN = searchParams[0].NIGHT_YN

  // if(searchParams[0].REL_TEAM_CD != null && searchParams[0].REL_TEAM_CD != '' ) searchParam.REL_TEAM_CD = searchParams[0].REL_TEAM_CD
  // else if(searchParams[0].REL_GWA_CD != null && searchParams[0].REL_GWA_CD != '' ) searchParam.REL_TEAM_CD = searchParams[0].REL_GWA_CD
  return commonSearchApi({ queryId : 'OPRAC0010_SEARCH_01', param: searchParam })
}
...
const onCellClicked = (grid, clickData) => {
  ...
  const index = selectList.nm.indexOf(data.ALL_SYS_CD + '||' + data.SYS_CDNM)
  
  // 상위레벨 클릭시 return
  if(data.ALL_SYS_CD.length !== 12) return
  ...
  if(index !== -1) {
    ...
    //selectList.nm.push(data.ALL_SYS_CD + '||' + data.SYS_CDNM)
...
const saveData = () => {
  ...
  return commonExecuteApi({ queryId : 'OPRAC0010_SAVE_01', list: saveParams })
}
...
const onItemChecked = (grid, itemIndex, checked) => {
  ...
  for(var d of selectList.nm) {
    const dt = d.split('||')
    grdData.push({
      SYS_CD: dt[0].substring(9,12),
      SYS_CDNM: dt[1],
      CMPNY_DIV: userStore.cmpnyDiv,
      WK_LPLC: dt[0].substring(7,8),
      WK_MPLC: dt[0].substring(8,9),
      WK_SPLC: dt[0].substring(9,12),
      ...
      HND_PHN: searchParams[0].HND_PHN,
      LOC_HEADER_NO: searchParams[0].LOC_HEADER_NO,
        ...
        <v-sheet class="searchArea d-flex flex-column mb-2">
          <div >
            ...
            <div class="d-flex mt-2">
              ...
              <VCheckbox
                class="mt-5"
                v-model="searchParams[0].LOC_HEADER_NO"
                :label="$t('지역장')"
              />
            </div>
          </div>
          ...
          <div class="d-flex flex-column fill-height">
            <div>
              <IGridTitle 
                ...
                :title="$t('장소 목록')"
                :use-permission="false"
                :button-list="['btnExpand', 'btnCollapse']"
                @click-button="onButtonsClick"
              />
            </div>
