...
const mapCenter4326 = [129.430331, 35.504824]

// const mapCenter4326 = [129.4067622245301, 35.47818446202955]
...
// 일레일 경우에는 200Z010로 변경해야 지도에 표시됨. (2024.12.30)
const areaOrg = ref('200Z010')    //현재 조직유형 기본:전사

const serious = ref([])           //중대성 사고  Y 데이터 저장
const jungdae = ref([])           //중대재해  Y 데이터 저장
const sago_div_l_d = ref([])        // 물적사고
...
const areaDataTo3 = ref([])        //안전수칙 전체
const areaDataTo4 = ref([])        // 안전작업지시 전체

let areaDataAreaAll = ref([])   // 장소코드 전체
...
// 모바일 안전작업지시 현황 
const areaDataMobile2 = ref([])

const areaDataMobile = ref({           
  work_cnt: '0',
  start_cnt: '0',
  dgr_cnt: '0',
})

const areaDataMobileCnt1 = ref([])     // 발행건수
const areaDataMobileCnt2 = ref([])     // 확인건수
const areaDataMobileCnt3 = ref([])     // 위험건수
...
const setComParams = type => {
  ...
  if(type === 'inspect') {
    ...
    // 안전작업지시 현황
    if(orgnWorkDiv.value === 'date') {
      from = formWorkDate.value
      to = formWorkDate.value
    } else if(orgnWorkDiv.value === 'yyyymm') {
      if(typeWorkDiv.value === 'acc') {
        from = dateWorkYear.value + '-01-01'
        to = dateWorkYear.value + '-' + dateWorkMonth.value + '-' + toDay
      } else {
        from = dateWorkYear.value + '-' + dateWorkMonth.value + '-01'
        to = dateWorkYear.value + '-' + dateWorkMonth.value + '-' + toDay
      }
    }
  } else {
...
const setSAFFB0010 = async () => {
  ...
  if(response.ORESULT_CUR.length > 0) {
    ...
    sliData.value = [
      ...
      { BSNS_NM: '전사', TOT_SVG: 0, TOTAL_GRADE: '' },
      { BSNS_NM: '울산1공장', TOT_SVG: 0, TOTAL_GRADE: '' },
      { BSNS_NM: '울산2공장', TOT_SVG: 0, TOTAL_GRADE: '' },
      { BSNS_NM: '선암공장', TOT_SVG: 0, TOTAL_GRADE: '' },      
    ]
  }
...
const SAFAB0010 = async () => {
  ...
  let response = await commonSearchApi({ queryId: "OPRAG0010_ELEC_SAFAB0010_01", param: setComParams('accident') })
...
const setSAFAB0010 = async res => {
  ...
  res.forEach(item => {
    ...
    if(item.SAGO_DIV_L !== 'C' && item.SAGO_DIV_L !== 'F') {
      ...
      // 물적사고 건수 취합
      if(item.SAGO_DIV_L === 'D') {
        sago_div_l_d.value.push(item)
      }
      ...
      if(item.SAGO_TYPE_CD === 'SJE') {
        areaDataDe1_1.value.push(item)
        item.TYPE = "안전사고(산재)"
      } else if(item.COME_SERIOUS_YN === 'N1') {
        areaDataDe1_2.value.push(item)
        item.TYPE = "경미상1"
      } else if(item.COME_SERIOUS_YN === 'N2') {
        areaDataDe1_3.value.push(item)
        item.TYPE = "경미상2"
      }
...
// 안전작업지시 
const setMobileSaftyWorkDetail = async areaData => {
  //console.log('setSAFAB0010Detail',areaData.value)
  await setAreaCount(areaData.value)
  setAreaLevel(areas.value, areaLv.value, areaColor.value)
}
...
const SAFDC0010 = async () => {
  let response = await commonSearchApi({ queryId: "OPRAG0010_ELEC_SAFDC0010_01", param: setComParams('rules') })
  response = response.ORESULT_CUR
...
const setSAFDC0010 = async res => {
  areaDataTo3.value = res // 안전수칙 전체 데이터
  areaEtc3_1.value = []
  areaEtc3_2.value = []
  areaDataDe3_1.value = []
  areaDataDe3_2.value = []
  ...
  res.forEach(item => {
    // HHIZ010AJ 군산 데이터로 보임. 일렉에서는 사용하지 않음.
    if(item.LV4 !== null && item.LV4.indexOf('HHIZ010AJ')  > -1) {
...
const SAFCD0010 = async () => {
  ...
  let response = await commonSearchApi({ queryId: "OPRAG0010_ELEC_SAFCD0010", param: setComParams('inspect') })
  response = response.ORESULT_CUR
...
const setAreaCount = areaData => {
  ...
  area1.value.forEach(areaItem => {
    ...
    areaData.forEach(dataItem => {
      ...
      // 모바일 안전작업지시 일 경우
      if (workorderCard.value) {
        findCode = dataItem.area_l + dataItem.area_m + dataItem.area_s + dataItem.area_d
        // 모바일 안전작업지시 장소코드에 해당되는 장소코드 찾기
        let findItem = areaCDList.value.find(t => t.SAFTY_AREA_CD === findCode)
        // console.log("item", item.LV4)
        dataItem = findItem
      }
      if(dataItem != undefined && dataItem.LV1 !== null && areaItem.ALL_SYS_CD === dataItem.LV1) {
        ...
        if(inspectCard.value ) {
          ...
          // 미진행, 진행, 완료 건수를 표시하는 것 같음. (2025.01.20)
        ...
        if (workorderCard.value) {
          ...
          c_cnt = ++ c_cnt          
          // 확인건수
          if (dataItem.started_worker != '0') {
            p_cnt = ++ p_cnt
          }
          // 고위험 건수
          if (dataItem.dgr_yn == 'Y') {
            n_cnt = ++ n_cnt
          }
        }
      }
  ...
  area2.value.forEach(areaItem => {
    ...
    areaData.forEach(dataItem => {
      ...
      if(dataItem != undefined && dataItem.LV2 !== null && areaItem.ALL_SYS_CD === dataItem.LV2) {
        count = ++count
        if(inspectCard.value ) {
    ...
    areaData.forEach(dataItem => {
      ...
      if (workorderCard.value) {
        ...
        if (dataItem) {
          ...
          // console.log("findItem.LV3", findItem.LV3)
        }
        
      }

      if(dataItem != undefined && dataItem.LV3 !== null && areaItem.ALL_SYS_CD === dataItem.LV3) {
        count = ++count
        console.log("area3 exist")
        if(inspectCard.value ) {
  ...
  area4.value.forEach(areaItem => {
    ...
    areaData.forEach(dataItem => {
      ...
      if(dataItem != undefined && dataItem.LV4 !== null && areaItem.AREA_CD === dataItem.LV4) {
        count = ++count
        if(inspectCard.value ) {
          if(dataItem.STATUS === 'C')  c_cnt = ++ c_cnt
          if(dataItem.STATUS === 'P')  p_cnt = ++ p_cnt
...
const getInitArea  = async () => {
  ...
  let response = await commonSearchApi({ queryId: "OPRAG0020_ELEC_SEARCH_06", param: acdtParms })
  // 추후 response는 areaDataAreaAll로 변경할것.
  response = response.ORESULT_CUR
  areaDataAreaAll = response.ORESULT_CUR
  ...
  await setSAFAB0010(response1) // 안전사고 발생현황 건수 표시
  await setSAFCD0010(response2) // 안전점검
  await setSAFDC0010(response3) // 안전수칙

  // 일렉용 모바일 안전작업 지시 추가 (2024.12.21)
  await setMobileSaftyWork() // 모바일 안전작업지시 (작업자확인/발행건수, 고위험 작업건수)
...
// 장소코드 모두 조회하기
const setAreaCDList = async () => {
  let params = {
    CMPNY_DIV: '200',    
  }
  console.log("setAreaCDList ")
  let response = await commonSearchApi({ queryId: "OPRAG0010_ELEC_AREACDLIST", param: params })//setComParams() })
  //console.log('setMNGAA0060 response',response.ORESULT_CUR)
  if(response.ORESULT_CUR.length > 0) {
    console.log("> 0")
    console.log("ORESULT_CUR.length", response.ORESULT_CUR.length)
    areaCDList.value = response.ORESULT_CUR
  }   
...
//상세정보 Tooltip
// 안전점검, 안전작업지시현황에서 건수 클릭시 아이콘 형식으로 (미완료, 진행중, 점검완료) 건수를 보여준다.
...
const setAreaLevel = (area, lv, color) => {  
  ...
  for (var i in area) {
    ...
    if(area[i].LAT != null && area[i].ALL_SYS_CD.indexOf(areaOrg.value) > -1) {
      ...
      // 안점점검, 안전작업지시 일 경우에만 표시한다.
      if((inspectCard.value || workorderCard.value) && (mapMakersVisable.value.ptw || mapMakersVisable.value.plan || mapMakersVisable.value.workorder)) { //Number(lv) > 2) {
        ptwFold[i] = true
...
const formWorkDate = ref(dayjs().format('YYYY-MM-DD'))
...
const dateWorkYear = ref(dayjs().format('YYYY'))
const dateWorkMonth = ref(dayjs().format('MM'))
...
const mapMakersVisable = ref({
  ...
  general: false,
  workorder: false,
  workstart: false, // 안전작업지시 발행건수 
  workdgr: false, // 안전작업지시 고위험 건수
})
...
const clickCard = async type => {
  ...
  if(type === 'accident') {
    ...
    rulesCard.value = inspectCard.value = workorderCard.value = false
    ...
    accidentCard.value = rulesCard.value = workorderCard.value = false
    ...
    accidentCard.value = inspectCard.value = workorderCard.value = false
    ...
    // 안전작업지시 현황
    workorderCard.value = true
    rulesCard.value = accidentCard.value = inspectCard.value = false
    areaColor.value = '#ffd700'

    // 일렉용 모바일 안전작업 지시 추가 (2024.12.21)
    await setMobileSaftyWork() // 모바일 안전작업지시 (작업자확인/발행건수, 고위험 작업건수)

    // 안전사고 건수를 지도에 표시한다.
    await setAreaCount(areaDataTo4.value)  
    ...
    // 장소 데이터를 표시한다. 현재레벨 장소 데이터, 현재레벨, 색깔
    setAreaLevel(areas.value, areaLv.value, areaColor.value)
  }
...
function clickDetail(type) {
  ...
  } else if (type === 'workstart') {
    // 모바일 안전작업지시 현황 - 확인/발행건수
    //setSAFCD0010Detail(areaDataDe2_1)
    setMobileSaftyWorkDetail(areaDataMobileCnt1)
  } else if (type === 'workdgr') {
    // 모바일 안전작업지시 현황 - 고위험 건수
    //setSAFCD0010Detail(areaDataDe2_1)
    setMobileSaftyWorkDetail(areaDataMobileCnt3)
  }

  if(type === 'ptw' || type === 'plan' || type === 'workorder')
...
// 모바일 안전작업지시 디테일
const setMobileSaftyWork = async () => {  
  console.log("setMobileSaftyWork start")

  areaDataMobileCnt1.value = [] // 발행건수
  areaDataMobileCnt2.value = [] // 확인건수
  areaDataMobileCnt3.value = [] // 고위험건수

  // 조회구분 (1:월누적, 2:월별, 3:일별)
  // 모바일 안전작업지시는 데이터가 많아서, 일별로만 조회함.
  let halfType = '1'
  if (orgnWorkDiv.value === 'date') {
    // 일별
    halfType = '3'
  } else if (orgnWorkDiv.value === 'yyyymm' && typeWorkDiv.value === 'acc') {
    // 월 누적
    halfType = '1'
  } else {
    // 월
  ...
  console.log("orgnWorkDiv.value", orgnWorkDiv.value)

  const dataParam = {
    
    // 안전작업지시는 일별로만 조회함.
    year: orgnWorkDiv.value === 'yyyymm' ? dateWorkYear.value : formWorkDate.value.substring(0, 4),
    month: orgnWorkDiv.value === 'yyyymm' ? dateWorkMonth.value : formWorkDate.value.substring(5, 7),
    // deptCode: 'WX50',
    day: orgnWorkDiv.value === 'date' ? formWorkDate.value.substring(8, 10) : '',
    searchType: halfType, 
  }
  ...
  // dataParam.month = "02"
  // dataParam.day = "07"
  // dataParam.searchType = "3"

  // 모바일 안전작업지시 현황 조회(왼쪽 카드)
  await elecMobileRequestDetail(dataParam, meta).then((res) => {
    ...
    if (res.result.status == '200') {
      ...
      //let jstring = JSON.stringify(res.body.toString())

      //areaDataMobile2.value.push(res)

      if (res.data != null ) {
        console.log('res.data', res.data)
          res.data.forEach((item) => {          
          
            console.log("item.dgr_yn=" + item.dgr_yn)
            console.log("item.started_worker=" + item.started_worker)
            
            // 발행건수
            areaDataMobileCnt1.value.push(item)
            if (item.started_worker != '0') {
              // 확인건수
              areaDataMobileCnt2.value.push(item)
            }
            if (item.dgr_yn == 'Y') {
              // 고위험 건수
              areaDataMobileCnt3.value.push(item)
            }

            // 전체 데이터를 설정한다.
            areaDataTo4.value = res.data

            // areaDataMobile.value.work_cnt = el.work_cnt  // 확인건수
            // areaDataMobile.value.start_cnt = el.start_cnt // 발행건수
            // areaDataMobile.value.dgr_cnt = el.dgr_cnt // 위험 건수
         })
      }
...
// 모바일 안전작업지시 현황 (사용안함)
const setMobileSaftyWorkOld = async () => {  
  ...
  // 모바일 안전작업지시 현황 조회(왼쪽 카드)
  await elecMobileRequest(dataParam, meta).then((res) => {
...
const elecMobileRequest = (data) => {  
  ...
  // if (isProd) {
  //   // 운영
  //   callUrl ="http://safety-work.hd-hyundaielectric.com/wsx01/interface/dashboard/integrate/satfey?searchType=" + data.halfYearType + "&year=" + data.year + "&month=" + data.month
  // } else {
  //   // 개발용
  //   // callUrl = "http://10.100.45.203:49567/wsx01/interface/dashboard/hazard/assessment?year=2024&deptCode=WX50&halfYearType=1"
  //   callUrl = "http://10.100.45.203:49567/wsx01/interface/dashboard/integrate/safety?searchType=" + data.halfYearType + "&year=" + data.year + "&month=" + data.month
  // }

  if (isUseAPI()) {
    return request({

      url: "/hse/fileElec/elecMobileSafetyList",      
      method: 'post',
      headers: {
...
* 일렉 안전작업지시 데이터를 조회함. 원칙적으로 request API파일을 따로 분리할려고 했으나, 추후 일렉용으로 개발시 빼기로 함.
*/
const elecMobileRequestDetail = (data) => {  
  // let isProd = true
  var hostname = window.location.hostname
  console.log("hostname", hostname)
  console.log("data", data)
  ...
  if (isUseAPI()) {
    return request({
      ...
      url: "/hse/fileElec/elecMobileSafetyDetail",      
      method: 'post',
      headers: {
        'Content-Type': 'application/json',        
      },
...
* 일렉 안전작업지시 데이터를 조회함. 원칙적으로 request API파일을 따로 분리할려고 했으나, 추후 일렉용으로 개발시 빼기로 함.
*/
const elecMobileRequest_Old2 = (data) => {  
  let isProd = true
  var hostname = window.location.hostname
  console.log("hostname", hostname)
...
const btnClick = async () => {
  ...
  setSAFFB0010() // SLI 데이터
  setMNGAA0060() // 부문별 안전 KPI 현황 데이터
  ...
  // 일렉용 모바일 안전작업 지시 추가 (2024.12.21)
  setMobileSaftyWork() // 모바일 안전작업지시 (작업자확인/발행건수, 고위험 작업건수)
...
const areaBtnList  = ref([
  ...
  // 울산1공장 : 35.504824 	129.430331
  // 울산2공장 : 35.533767 	129.433448 
  // 선암공장 : 35.508807	129.331467
  { text: '울산1공장', id: 'HHI', loc: [129.430331, 35.504824], selected: true },
  { text: '울산2공장', id: 'HHI', loc: [129.433448, 35.533767], selected: false },
  { text: '선암공장', id: 'HHI', loc: [129.331467, 35.508807], selected: false },
  //{ text: '해양', id: 'OCN', loc: [129.406218, 35.480363], selected: false },
])
...
const orgBtnList = ref([
  ...
  // 조직유형 버튼 전사를 제외하고 주석처리함.
  // { text: '울산1공장', id: 'HHIZ012A', selected: false },
  // { text: '울산2공장', id: 'HHIZ012B', selected: false },
  // { text: '선암공장', id: 'HHIZ012C', selected: false },
          ...
          >
            ...
            >
              ...
              <div class="d-flex justify-space-between">
                <v-row no-gutters>
                  ...
                  >
                    ...
                    >
                      <span class="dashLabel">경미상1</span>
                      <div class="dashLabel d-flex align-center">
                    ...
                    >
                      <span class="dashLabel">경미상2</span>
                      <div class="dashLabel d-flex align-center">
              ...
              >
                ...
                <span class="dashLabel">물적사고</span>
                <div class="dashLabel d-flex align-center">
                  {{ sago_div_l_d.length }} 건
                </div>                
              </v-sheet>
              ...
              <section style="height: 70%">
                ...
                >
                  ...
                  >
                    ...
                    >
                      <span class="dashLabel">PTW점검</span>
                      <div class="dashLabel d-flex align-center">
            ...
            <v-card
              color="dashCard"
              class="checkCard"
              :class="{ checked: workorderCard }"
              variant="flat"
              min-height="132"
              height="12vh"
              @click="clickCard('workorder')"
            >
              <section class="dataView mb-4">
                <div class="cardTitle">
                  <v-sheet
                  ...
                  >
                    <svg-icon
                      :key="workorderCard ? 'checked' : 'unChecked'"
                      width="20"
                      :name="workorderCard ? 'checked' : 'unChecked'"
                    />
                  </v-sheet>
                  안전작업지시 현황<span style="font-size: 12px;">* 승인완료기준</span>
                </div>
                <!--
                  <div>
                  <v-btn
              ...
              <section style="height: 50%">
                ...
                >
                  ...
                  >
                    <v-btn
                      :disabled="!workorderCard"
                      density="comfortable"
                      flat
                      color="dashCard"
                      class="dataBtn"
                      :class="{ selected: mapMakersVisable.workorder }"
                      @click.stop="clickDetail('workstart')"
                    >
                      <span class="dashLabel">작업자 확인/발행건수</span>
                      <div class="dashLabel d-flex align-end">
                        <span class="dataNum pr-2">{{ areaDataMobileCnt2.length }} / {{ areaDataMobileCnt1.length }}</span>건                        
                        <v-sheet
                          width="20"
                          color="transparent"
                        >
                    ...
                    <v-btn
                      ...
                      class="dataBtn"
                      :class="{ selected: mapMakersVisable.workorder }"
                      @click.stop="clickDetail('workdgr')"
                    >
                      <span class="dashLabel">고위험 작업건수</span>                      
                      <div class="dashLabel d-flex align-right">
                        <span class="dataNum pr-2">{{ areaDataMobileCnt3.length }}</span>건
                        <v-sheet
                          width="20"
                          color="transparent"
                        >
          ...
          >
            ...
            >
              <p class="cardTitle pb-3">
                {{ detailCType === 'ptw' ? 'PTW 점검 진행 현황' : detailCType === 'plan' ? '부서관리작업 점검 진행 현황' : '안전작업지시 현황' }}
              </p>
              <v-sheet
                color="transparent"
                class="chartContents"
              >
                <section>
                  ...
                  -->
                  <span class="dashLabel">{{detailCType === 'ptw' || detailCType === 'plan' ? '점검미완료' : '발행건수'}}</span>
                  <div>
                ...
                <section>
                  <svg-icon name="cs02" />
                  <span class="dashLabel">{{detailCType === 'ptw' || detailCType === 'plan' ? '점검 진행중' : '확인건수'}} </span>
                  <div>
                ...
                <section>
                  <svg-icon name="cs03" />
                  <span class="dashLabel">{{detailCType === 'ptw' || detailCType === 'plan' ? '점검 완료' : '고위험건수'}}</span>
                  <div>
            ...
            <div class="d-flex align-center px-8 h-full w-full flex-auto">
              ...
              >
                ...
                <p v-else-if="workorderCard">                  
                  ...
                  >
                    <VRadio
                      v-for="(item, i) in orgnWorkDivList"
                      :key="i"
                      :value="item.code"
                      :label="item.text"
                      class="mr-2"
                ...
                <div class="d-flex algin-center">
                  ...
                  <template v-else-if="workorderCard">
                    <template v-if="orgnWorkDiv === 'date'">
                      <i-input
                    ...
                    <template v-else>
                      <!-- 안전작업지시 조회는 해당 일자만 조회하는 것으로 변경함. -->
                      <i-input
                        v-model="formWorkDate"
                        type="Date"
                        width="155px"
