<!--
  화면명 : 안전요원 등록/관리 TAB 1
  화면개요 : 장소별 안전요원 등록/관리 화면
-->
<script setup>
...
import BulkCreate from '@/pages/80_operation/OPR_A/OPRAC0010Popup02.vue'
...
const getCode = () => {
  ...
  Promise.all([
    ...
    commonSearchApi({ queryId : 'OPRAC0010_SEARCH_14', param : {  } }), // 호선번호
    commonSearchApi({ queryId : 'OPRAC0010_SEARCH_15', param : { USER_ID: userStore.userId } }), // 과 코드
  ]).then(res => {
    var tempCodeList = []    
    ...
    codeList.SHIP_NO.unshift({ TXT:'', COD:'' })

    codeList.DEPT_CD2 = allCodeList.DEPT_CD
    codeList.GWA_CD2 = allCodeList.GWA_CD
    codeList.TEAM_CD2 = allCodeList.TEAM_CD
    // codeList.DEPT_CD2.unshift({ DEPT_NM:'전체', DEPT_CD:'' })
    // codeList.GWA_CD2.unshift({ ASGN_NM:'전체', ASGN_CD:'' })
    // codeList.TEAM_CD2.unshift({ ASGN_NM:'전체', ASGN_CD:'' })

    grdMain.value.getGridView().editOptions.movable = true
    grdEmp.value.getGridView().editOptions.movable = true
    gwa_cd.value = res[4].ORESULT_CUR[0].GWA_CD
    setDept(res[4].ORESULT_CUR[0].GWA_CD)
...
const grdMainProps = reactive({
  gridViewOption : { checkBar: { visible: true }, stateBar: { visible: false }, edit: { editable: true }, header: { height: 75 } },
  keys : ['ALL_SYS_CD', 'SYS_CD'],
  fields : [ 
    { fieldName: 'SYS_CDNM', dataType: 'text', width: '150', sortable: false, header: { text: t('장소명') }, styleName: 'left-column', width : '170', editable: false,
    ...
    { fieldName: 'REL_DEPT_NM', dataType: 'text', sortable: false, width: '80', header: { text: t('안전부') }, editable: false, visible: true },
    { fieldName: 'REL_GWA_NM', dataType: 'text', sortable: false, width: '80', header: { text: t('안전과') }, editable: false, visible: true },
    { fieldName: 'REL_TEAM_NM', dataType: 'text', sortable: false, width: '80', header: { text: t('안전팀') }, editable: false, visible: true },
    { fieldName: 'REL_EMP_NO', dataType: 'text', sortable: false, width: '60', header: { text: t('사번') }, editable: true,styleName: 'editable_column', visible: false,
      'styleCallback': function(grid, dataCell){
        var ret = {editable : false, styleName : 'middle_column', enabled: false, style: {background:'#dfdfdf'}}
        var data = grid.getValue(dataCell.index.itemIndex, 'TMP_ALL_SYS_CD')

        if(data == null) return ret
        if(data.length < 12) return ret
        
        if(grid.getValue(dataCell.index.itemIndex, 'c') == '1') {
    ...
    { fieldName: 'REL_EMP_NM', dataType: 'text', sortable: false, width: '60', header: { text: t('안전요원') }, editable: true,styleName: 'editable_column',
      button:'action',
    ...
    { fieldName: 'HND_PHN', dataType: 'text', sortable: false, width: '80', header: { text: t('휴대폰번호') }, editable: true,styleName: 'editable_column',
      'styleCallback': function(grid, dataCell){
        var ret = {editable : false, styleName : 'middle_column', enabled: false, style: {background:'#dfdfdf'}}
        var data = grid.getValue(dataCell.index.itemIndex, 'TMP_ALL_SYS_CD')
    ...
    { fieldName: 'TEL_NO', dataType: 'text', sortable: false, width: '60', header: { text: t('내선번호') }, editable: true,styleName: 'editable_column',
      'styleCallback': function(grid, dataCell){
        var ret = {editable : false, styleName : 'middle_column', enabled: false, style: {background:'#dfdfdf'}}
        var data = grid.getValue(dataCell.index.itemIndex, 'TMP_ALL_SYS_CD')
    ...
    { fieldName: 'TRS', dataType: 'text', sortable: false, width: '60', header: { text: t('무전기코드') }, editable: true, styleName: 'editable_column',
      'styleCallback': function(grid, dataCell){
        return setEditable(grid,dataCell)
      }
    },
    { fieldName: 'LOC_HEADER_NO', dataType: 'boolean', sortable: false, width: '40', header: { text: t('지역장/\n호선장') , styleName: 'multiline-editor-pre' }, editable: false, styleName: 'editable_column change_back_check', renderer: { type: "check" },
      'styleCallback': function(grid, dataCell){
        var ret = {editable : false, styleName : 'middle_column', enabled: false, style: {background:'#dfdfdf'}}
        var data = grid.getValue(dataCell.index.itemIndex, 'TMP_ALL_SYS_CD')
      ...
      'displayCallback': function (grid, index, value) {
        var data = grid.getValue(index.itemIndex, 'TMP_ALL_SYS_CD')
        if(data == null) return ''
        if(data.length < 12) return ''
      }
    },

    { fieldName: 'SHIP_NO', dataType: 'text', sortable: false, width: '60', header: { text: t('호선/\n프로젝트') , styleName: 'multiline-editor-pre' }, editable: true, styleName: 'editable_column', visible: true,
      editor: { type: 'text' }, lookupDisplay: true,
      'styleCallback': function(grid, dataCell){
         
        var ret = {editable : true, styleName : 'editable_column', enabled: true, style: {background:'#dfdfdf'}}
        var dropList = { COD: [], TXT: [] }

        var cmpnyDiv = grid.getValue(dataCell.index.itemIndex, 'CMPNY_DIV')
        var deptCd = grid.getValue(dataCell.index.itemIndex, 'REL_DEPT_CD')
        var gwaCd = grid.getValue(dataCell.index.itemIndex, 'REL_GWA_CD')
        var teamCd = grid.getValue(dataCell.index.itemIndex, 'REL_TEAM_CD')
        var data = grid.getValue(dataCell.index.itemIndex, 'TMP_ALL_SYS_CD')

        for(let ship of codeList.SHIP_NO) {
          if (ship.DEPT_CD === deptCd && ship.GWA_CD === gwaCd && ship.TEAM_CD === teamCd){
            dropList.COD.push(ship.COD)
            dropList.TXT.push(ship.TXT)
          }
        }
        ...
        if(data.length < 12) { 
          ...
          if (grid.getValue(dataCell.index.itemIndex, 'REL_EMP_NO') == '' || grid.getValue(dataCell.index.itemIndex, 'REL_EMP_NO')== null){
            if(grid.getValue(dataCell.index.itemIndex, 'EXCLUDE_WORK') == '1'){
              ret.style.background = undefined
            } else {
            ...
            if(grid.getValue(dataCell.index.itemIndex, 'EXCLUDE_WORK') == '1') {
              ...
              // 도크(02) 또는 안벽(03) 만 호선 번호 입력 가능하게
              // 창정비 셀타 구역 입력 가능하게 수정
              if(grid.getValue(dataCell.index.itemIndex, 'CODE_DESC6') == '02' || grid.getValue(dataCell.index.itemIndex, 'CODE_DESC6') == '03'
              || grid.getValue(dataCell.index.itemIndex, 'TMP_ALL_SYS_CD') == 'HHIZ010AC240' || grid.getValue(dataCell.index.itemIndex, 'TMP_ALL_SYS_CD') == 'HHIZ010AC250'){
                ret.style.background = undefined
    ...
    { fieldName: 'UPDATE_USER_ID', width: '60', sortable: false, dataType: 'text', header: { text: t('수정자') }, editable: false,
      'styleCallback': function(grid, dataCell){
        var ret = {editable : false, styleName : 'middle_column', enabled: false, style: {background:'#dfdfdf'}}
        var data = grid.getValue(dataCell.index.itemIndex, 'TMP_ALL_SYS_CD')
    ...
    { fieldName: 'UPDATE_DATE', width: '80', sortable: false, dataType: 'text', header: { text: t('수정일자') }, editable: false, 
      'styleCallback': function(grid, dataCell){
        var ret = {editable : false, styleName : 'middle_column', enabled: false, style: {background:'#dfdfdf'}}
        var data = grid.getValue(dataCell.index.itemIndex, 'TMP_ALL_SYS_CD')
    ...
    { fieldName: 'ADD_EMP', width: '45', sortable: false, dataType: 'text', header: { text: t('요원추가') }, editable: false, styleName: 'cursor_pointer',
      renderer: {
        type: "image",
        imageCallback: function (grid, cell) {
          if(grid.getValue(cell.index.itemIndex, 'TMP_ALL_SYS_CD') == null) return
          if(grid.getValue(cell.index.itemIndex, 'TMP_ALL_SYS_CD').length < 12) return
          return getImg('AddBtn')
        },
        imageHeight: 20
      },
    },
    { fieldName: 'SUB_EMP', width: '45', sortable: false, dataType: 'text', header: { text: t('요원삭제') }, editable: false, styleName: 'cursor_pointer',
      renderer: {
        type: "image",
        imageCallback: function (grid, cell) {
          if(grid.getValue(cell.index.itemIndex, 'TMP_ALL_SYS_CD') == null) return
    ...
    // 저장, 삭제시 필요 데이터
    { fieldName: 'SYS_CD', dataType: 'text', header: { text: t('시스템 코드') }, styleName: 'left-column', visible: false },
    { fieldName: 'CMPNY_DIV', dataType: 'text', header: { text: t('사업장구분') }, visible: false },
    { fieldName: 'REL_DEPT_CD', dataType: 'text', width: '80', header: { text: t('안전부') }, editable: false, styleName: '', visible: false,
      ...
      'styleCallback': function(grid, dataCell){
        ...
        if(data == null) return ret
        if(data.length < 12) {
          ret.style.background = '#dfdfdf'
          return ret
        }
        else {
          ret.styleName = 'middle_column'
        } 
        return ret
      }
    },
    { fieldName: 'REL_GWA_CD', dataType: 'text', width: '80', header: { text: t('안전과') }, editable: false, styleName: '', lookupDisplay: true, visible: false,
      'styleCallback': function(grid, dataCell){
        var ret = {editable : false, styleName : 'middle_column', enabled: false, style: {background:undefined}}
        ...
        if(grid.getValue(dataCell.index.itemIndex, 'TMP_ALL_SYS_CD') == null) return ret
        if(grid.getValue(dataCell.index.itemIndex, 'TMP_ALL_SYS_CD').length < 12) {
          ret.style.background = '#dfdfdf'
          return ret
        }
        else {
    ...
    { fieldName: 'WK_LPLC', dataType: 'text', header: { text: t('장소_대분류') }, visible: false },
    { fieldName: 'WK_MPLC', dataType: 'text', header: { text: t('장소_중분류') }, visible: false },
    { fieldName: 'WK_SPLC', dataType: 'text', header: { text: t('장소_소분류') }, visible: false },
    { fieldName: 'ALL_SYS_CD', dataType: 'text', header: { text: t('ALL_SYS_CD') }, visible : false },
    { fieldName: 'TMP_ALL_SYS_CD', dataType: 'text', header: { text: t('TMP_ALL_SYS_CD') }, visible : false },
    { fieldName: 'ALL_UP_CD', dataType: 'text', header: { text: t('ALL_UP_CD') }, visible : false },
    { fieldName: 'CODE_DESC6', dataType: 'text', header: { text: t('CODE_DESC6') }, visible : false },
    { fieldName: 'UP_CD', dataType: 'text', header: { text: t('UP_CD') }, visible : false  },
    ...
    { fieldName: 'WK_DT', dataType: 'datetime', datetimeFormat: 'yyyy-MM-dd', header: { text: t('근무일자') }, visible: false, styleName: 'editable_column',
      'editor': {
        'type': 'date',
        'commitOnSelect': true,
        'mask': {
    ...
    { fieldName: 'INSERT_USER_ID', dataType: 'text', header: { text: t('등록자') }, visible: false },
    { fieldName: 'INSERT_DATE', dataType: 'text', header: { text: t('등록일자') }, visible: false },
  ...
  columnLayout: [
    'SYS_CDNM',
    {
          ...
          items: [ 'REL_DEPT_NM', 'REL_GWA_NM', 'REL_TEAM_NM', 'REL_EMP_NO', 'REL_EMP_NM', 'HND_PHN', 'TEL_NO', 'TRS', 'LOC_HEADER_NO', 'SHIP_NO'],
...
const grdEmpProps = reactive({
  ...
  keys : [],
  fields : [
    { fieldName: 'EMP_NO', sortable: false, dataType: 'text', header: { text: t('사번') }, editable: false },
...
const onButtonsClick = (btn) => {
  ...
  if (btn.id === 'btnSearch') {
    ...
    if(selectData.TMP_ALL_SYS_CD.length != 12) {
      return Message.warn(t('해당 행은 복사할 수 없습니다'))
    }
    ...
    let newRow ={
      ...
      LOC_HEADER_NM: copyData[0].LOC_HEADER_NM,
      TMP_ALL_SYS_CD: selectData.TMP_ALL_SYS_CD,
      WK_LPLC: selectData.WK_LPLC,
      WK_MPLC: selectData.WK_MPLC,
      WK_SPLC: selectData.WK_SPLC,
    ...
    let currentRowIdx = grdMain.value.getGridView().getCurrent().dataRow
    const selectData = grdMain.value.getDataProvider().getJsonRow(currentRowIdx)
    if(selectData.TMP_ALL_SYS_CD.length < 12) return Message.warn(t('해당 노드에서는 추가할 수 없습니다.'))
    ...
    let newRow ={
      ...
      WRITE_YN: 'Y',
      SHIP_NO: '',
      EXCLUDE_WORK: selectData.EXCLUDE_WORK,
      CODE_DESC6: selectData.CODE_DESC6,
      TMP_ALL_SYS_CD: selectData.TMP_ALL_SYS_CD,
    }
    ...
    if (selectData.SYS_CDNM == null || selectData.SYS_CDNM == '') { // 최하위 레벨
      let addRowIdx = getTreeData.addChildRow(getTreeData.getParent(currentRowIdx), newRow, null, false)
      grdMain.value.getGridView().setCurrent({ dataRow: addRowIdx, column: 'SYS_CDNM' })
    ...
    toDateCopy.value.openPopup(searchParams, 1)
  } else if (btn.id === 'btnAddEmp') {
    addEmp()
    ...
    setTeam.value.openPopup(searchParams)
  }
} 
...
const addEmp = async () => {
  ...
  let newRow ={
    ...
    REL_GWA_CD: grdEmp.value.getGridView().getValue(empRow, 'GWA_CD'),
    ...
    IDX: '',
    WRITE_YN: 'Y',
    CODE_DESC6: selectData.CODE_DESC6, // 요원추가 버튼으로 행 추가 시 호선 컬럼 활성화하기 위해 추가
  }
  ...
  if (selectData.REL_EMP_NO == null || selectData.REL_EMP_NO == '') { // 최하위 레벨
    if(selectData.TMP_ALL_SYS_CD.length < 12) return Message.warn(t('해당 노드에는 안전요원을 추가할 수 없습니다.'))
    ...
    grdMain.value.getDataProvider().setValue(currentRowIdx,'TRS', newRow.TRS)
    ...
    grdMain.value.getDataProvider().setValue(currentRowIdx,'WK_LPLC', newRow.WK_LPLC)
    grdMain.value.getDataProvider().setValue(currentRowIdx,'WK_MPLC', newRow.WK_MPLC) 
    grdMain.value.getDataProvider().setValue(currentRowIdx,'WK_SPLC', newRow.WK_SPLC)
    grdMain.value.getDataProvider().setValue(currentRowIdx,'TMP_ALL_SYS_CD', selectData.TMP_ALL_SYS_CD)
    gridView.checkRow(currentRowIdx, true, false, false)
  } else if(selectData.SYS_CDNM == null) {
    if(!checkDup(selectData.TMP_ALL_SYS_CD, newRow.REL_EMP_NO)) return
    let addRowIdx = getTreeData.addChildRow(getTreeData.getParent(currentRowIdx), newRow, null, false)
...
// 체크된 행(수정된 행 있는지 확인) 
const checkUpdate = () => {
  if(grdMain.value.getGridView().getCheckedRows(true).length === 0) return false
  else return true
}
...
const deleteMainData = () => {
  ...
  return commonExecuteApi({ queryId : 'OPRAC0010_DELETE_01', list: deleteList })
}
...
const resetMainData = () => {
  let deleteList = []
  let param = { CMPNY_DIV: userStore.cmpnyDiv, USER_ID : userStore.userId, WK_DT: '',  REL_TEAM_CD: '', NIGHT_YN: searchParams.NIGHT_YN }
  // 일자
  ...
  return commonExecuteApi({ queryId : 'OPRAC0010_DELETE_02', list: deleteList })
}
...
const beforeSave = () => {
  ...
  if(checkedRows.length == 0) return Message.warn(t('선택된 행이 없습니다.'))
  return true
}
...
const saveData = () => {
  ...
  for (let rowIdx of checkedRows) {
    ...
    data.NIGHT_YN = searchParams.NIGHT_YN
    if(data.CMPNY_DIV == '') data.CMPNY_DIV = userStore.cmpnyDiv
    saveParams.push(data)
  }
  return commonExecuteApi({ queryId : 'OPRAC0010_SAVE_03', list: saveParams })
}
...
const setEmp = async (row) => {
  ...
  let gwaNm = await getCodeNm(row.GWA_CD)
  ...
  grdMain.value.getDataProvider().setValue(currentRowIdx, 'REL_GWA_CD', row.GWA_CD)
...
const searchMainData = () => {
  ...
  return commonSearchApi({ queryId : 'OPRAC0010_SEARCH_06', param: searchParams })
}
...
const afterMainSearch = res =>{
  ...
  deleteRow.rows = []
  for(let idx of grdMain.value.getGridView().getChildren(0)) {
    nodeOrganize(idx)
  }
  grdMain.value.getDataProvider().removeRows(deleteRow.rows)
...
const nodeOrganize = (index) => {
  let gridView = grdMain.value.getGridView()
  const data = gridView.getValue(index, 'ALL_SYS_CD')
  let result = false
  if(data == null) return false
  if(data.length >= 12) return true

  for(let idx of gridView.getChildren(index)) {
    if(nodeOrganize(idx)) result = true
  }

  if(result) return true
  deleteRow.rows.push(index + 1)
  return false
}
...
const searchSub = () => {  
  ...
  subSearchParam.NIGHT_YN = searchParams.NIGHT_YN
  ...
  return commonSearchApi({ queryId : 'OPRAC0010_SEARCH_07', param: subSearchParam })
}
...
const searchEmp = () => {  
  ...
  //   REL_GWA_CD:  gwa_cd.value, // 외업안전 2팀코드만 임시 변경
  //   REL_TEAM_CD: userStore.asgnCd,                //'N1E11', userStore.asgnCd // 안전팀 (임시)
    
  //   // 2024.12.12 윤찬양 책임 요청으로 조회조건의 부서 조회 X 로그인 유저 부서로 조회
  //   // REL_DEPT_CD: searchParams.REL_DEPT_CD,
  //   // REL_GWA_CD: searchParams.REL_GWA_CD,
  //   // REL_TEAM_CD: searchParams.REL_TEAM_CD,
  ...
  searchParams2.RETI_DATE = searchParams.TMP_WK_DT.replaceAll('-','')

  return commonSearchApi({ queryId : 'OPRAC0010_SEARCH_13', param: searchParams2 })
}
...
//조회 함수
const searchEmpData = () =>{
  // if(searchParams2.REL_DEPT_CD == '' && searchParams2.REL_TEAM_CD == '' && searchParams2.REL_GWA_CD == '') {
...
const deleteMan = () => {
  ...
  if(selectData.TMP_ALL_SYS_CD == null) return
  if(selectData.TMP_ALL_SYS_CD.length < 12) return

  if(selectData.SYS_CDNM == null || selectData.SYS_CDNM == '') {
    let deleteList = []
    deleteList.push(selectData)
    if(selectData.IDX != null && selectData.IDX != '') commonExecuteApi({ queryId : 'OPRAC0010_DELETE_01', list: deleteList })
    ...
    if(selectData.IDX != null && selectData.IDX != '') {
      commonExecuteApi({ queryId : 'OPRAC0010_DELETE_01', list: deleteList }).then(res => {
      })
    } 
    
    changeDeleteRow(selectData.TMP_ALL_SYS_CD, currentRowIdx)    
  }
  reRendering()
}

// 부모노드에 위치한 요원은 삭제되지만 자식노드에 있는 요원이 있을 시 자리 바꿈
const changeDeleteRow = (allSysCd, currentRowIdx) => {
  for(var i = 1; i <= grdMain.value.getDataProvider().getRowCount(); i++) {
    let data = grdMain.value.getDataProvider().getJsonRow(i)
    if(data == null) continue
    if(data.TMP_ALL_SYS_CD == allSysCd && (data.REL_EMP_NO != '' && data.REL_EMP_NO != null) && data.WRITE_YN == 'Y') {
      grdMain.value.getDataProvider().setValue(currentRowIdx,'WRITE_YN', 'Y')
      ...
      grdMain.value.getDataProvider().setValue(currentRowIdx,'LOC_HEADER_NO', data.LOC_HEADER_NO)
...
const onSubCellClicked = (grid, clickData) => {  
  ...
  if(grdSub.value.getDataProvider().getRowCount() == 0) return
  if(day == 'GBN') return
  if(day < 10) day = '0' + day
...
watch(() => searchParams.REL_DEPT_CD, (newValue, oldValue) => {
  ...
  .then(res => {
    ...
    // // 야간 시 안전기획과로 고정
    // if(searchParams.NIGHT_YN === 'Y') {
    //   searchParams.REL_GWA_CD = 'N1E6'
    //   return
    // }
...
watch(() => searchParams.REL_GWA_CD, (newValue, oldValue) => {
  ...
  commonSearchApi({ queryId : 'OPRAC0010_SEARCH_05', param : { CMPNY_DIV: userStore.cmpnyDiv, BSNS_CD: '', ASGN_CD: searchParams.REL_GWA_CD } })
    .then(res => {
      ...
      // // 야간 시 야간안전팀으로 고정
      // if(searchParams.NIGHT_YN === 'Y') {
      //   searchParams.REL_TEAM_CD = 'N1E61'
      //   return
      // }
...
// 안전부 변경 시 부서 변경 이벤트
watch(() => searchParams2.REL_DEPT_CD, (newValue, oldValue) => {
  if(searchParams2.REL_DEPT_CD === '') {
    searchParams2.REL_GWA_CD = ''
    searchParams2.REL_TEAM_CD = ''
  }
  
  commonSearchApi({ queryId : 'OPRAC0010_SEARCH_04', param : { CMPNY_DIV: userStore.cmpnyDiv, BSNS_CD: '', DEPT_CD: searchParams2.REL_DEPT_CD, USE_DIV:'Y' } })
  .then(res => {
    codeList.GWA_CD2 = res.ORESULT_CUR
    codeList.GWA_CD2.unshift({ ASGN_NM:'전체', ASGN_CD:'' })
    ...
    if (newValue !== oldValue) {
      searchParams2.REL_GWA_CD = codeList.GWA_CD2[0].ASGN_CD
    }
  })
})

// 안전과 바뀌면 안전팀 데이터 가져옴
watch(() => searchParams2.REL_GWA_CD, (newValue, oldValue) => {
  searchParams2.REL_TEAM_CD = ''
  commonSearchApi({ queryId : 'OPRAC0010_SEARCH_05', param : { CMPNY_DIV: userStore.cmpnyDiv, BSNS_CD: '', ASGN_CD: searchParams2.REL_GWA_CD } })
    .then(res => {
      codeList.TEAM_CD2 = res.ORESULT_CUR
      // // 야간 시 야간안전팀으로 고정
      // if(searchParams.NIGHT_YN === 'Y') {
      //   searchParams.REL_TEAM_CD = 'N1E61'
      //   return
      ...
      if(codeList.TEAM_CD2.length == 1) {
        searchParams2.REL_TEAM_CD = res.ORESULT_CUR[0].ASGN_CD
        codeList.TEAM_CD2.unshift({ ASGN_NM:'전체', ASGN_CD:'' })
        return
      }

      codeList.TEAM_CD2.unshift({ ASGN_NM:'전체', ASGN_CD:'' })
      if (codeList.TEAM_CD2.length !== 0) {
        searchParams2.TEAM_CD = ''
      }
    })
})

watch(() => searchParams.NIGHT_YN, (newValue, oldValue) => {
  if(newValue === 'Y') {
...
<template>
  <v-card class="pa-0 fill-height">
    <v-card-text class="pa-3 pt-0 content-area">
      <div class="d-flex flex-column fill-height">
        <v-sheet class="searchArea d-flex flex-column mb-0">
          ...
          <div class="d-flex">
            ...
            <!-- 윤찬양책임 요청으로 야간 선택시 부서 선택 불가였지만 선택 가능하도록 수정 - 2024.09.06 -->
            <i-select
              :label="$t('안전부')"
            ...
            <v-radio-group 
              class="mt-1"
              v-model="searchParams.NIGHT_YN"
              inline>
              <v-radio class="mr-5" :value="'N'" color="primary" label="주간"/>
              <v-radio :value="'Y'" color="primary" label="야간" />
            </v-radio-group>
          </div>
        </v-sheet>
          ...
          <IGridTitle 
            ...
            <template #editors>
                <span style="font-size: 14px; ">
                  ※ 다른 안전팀으로 배정된 장소에 소속 인원 배정 필요 시, 해당 안전팀으로 조회 후 해당 장소에 추가해 주시기 바랍니다.
                </span>
            </template>
          </IGridTitle>
          ...
          <v-sheet class="h-auto" width="15%">
            ...
            <v-sheet class="searchArea d-flex flex-column mb-0">
              <div>
                <i-select
                  :label="$t('부서')"
                  v-model="searchParams2.REL_DEPT_CD"
                  :items="codeList.DEPT_CD2"
                  item-value="DEPT_CD"
                  item-title="DEPT_NM" 
                  width="250px"
                />
                <i-select 
                  :label="$t('과')"
                  v-model="searchParams2.REL_GWA_CD"
                  :items="codeList.GWA_CD2"
                  item-value="ASGN_CD"
                  item-title="ASGN_NM" 
                  width="237px"
                ...
                <i-select 
                  :label="$t('팀')"
                  v-model="searchParams2.REL_TEAM_CD"
                  :items="codeList.TEAM_CD2"
                  item-value="ASGN_CD"
                  item-title="ASGN_NM" 
                  width="237px"
                  style="margin-left: 13px;"
      ...
      <SetTeam ref="setTeam" @after-search="() =>{
        searchShipNo()
        onButtonsClick({ id: 'btnSearch' })
      }"/>          <!-- 안전팀배정(호선) -->
    </v-card-text>
