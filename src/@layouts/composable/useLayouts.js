...
export const useLayouts = () => {  
  const tagviewType = computed({
    get() {
      return config.tagview.type.value
    },
    set(value) {
      config.tagview.type.value = value
    },
  })  
  ...
  const appContentLayoutNav = computed({
    ...
    set(val) {
      ...
      localStorage.setItem(`${config.app.title}-contentLayoutNav`, val)
    },
  })
  ...
  const _layoutClasses = computed(() => (windowWidth, windowScrollY) => {
    ...
    return [
      ...
      `layout-tagview-${tagviewType.value}`,
